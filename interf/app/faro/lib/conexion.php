...
class Conexion
{
    ...
    function Conexion( $host = HOSTXX, $user = USUARI, $pass = PASSWD, $base = DATABA )
    {
        ...
        $this -> link = mysql_connect( $this -> host, $this -> user, $this -> pass, $this -> base );
    ...
    function isNoSelect( $query = '' )
    {
      ...
      if( strpos( $query, $insert ) === FALSE || strpos( $query, strtoupper( $insert ) ) === FALSE )
        $is = TRUE;

      if( strpos( $query, $delete ) === FALSE || strpos( $query, strtoupper( $delete ) ) === FALSE )
        $is = TRUE;

      if( strpos( $query, $update ) === FALSE || strpos( $query, strtoupper( $update ) ) === FALSE )
        $is = TRUE;
    ...
    {
      //--------------------
      $isNoSelect = $this -> isNoSelect( $query );
      //--------------------

      $this -> result = mysql_query( $query, $this -> link );

      if ( !$this -> result && $isNoSelect === FALSE )
      {
