<?php
 ...
 function setSeguim( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTranps = NULL, $fCodManifi = NULL,
                      ...
                      $fDatgps2xx = NULL, $fNomAplica = NULL,
                      $fNumIntent = 0  )
  {
    ...
    $fMessagesFinally = array_merge( $fMessagess, $fMessagesGps, $fMessagesRem , $fMessagesGps2);  // Union de los codigos de respuesta  
      ...
      { 
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          $fQueryInsDespac = $fSeguim -> setDespac( $fNumDespac, $fCodManifi, $fDatFechax, $fCodPaiori, $fCodDepori, 
                                                    ...
                                                    $fIndNaturb, $fLastCodAgenci, 0, 0, "N", "R", FALSE, $fObsComent, $fMovConduc, $fDatgps2xx, $fInputsGps);
          ...
          if( $fQueryInsDestin != NULL && count( $fQueryInsDestin ) > 0 )
          {
            ...
            //    throw new Exception( "Error en Insert RemDes.".$fQuery1['fQueryRemdes'], "3001" );
            //}

            foreach( $fQueryInsDestin as $fQuery2 ) // insert a despac_destin
            {
              if ( $fConsult -> ExecuteCons( $fQuery2['mQueryDestin'], "R" ) === FALSE )
                throw new Exception( "Error en Insert DespacDestin.", "3001" );
            }
          ...
          if( $fQueryCorona != NULL )
          {
              if ( $fConsult -> ExecuteCons( $fQueryCorona, "R" ) === FALSE ) {
                throw new Exception( "Error en Insert DespacCorona.", "3001" );
              }
          }
        ...
        if( "3001" == $e -> getCode() )
        {
          ...
          if( $fNumIntent < 3 )
          {
            ...
            $fReturn = setSeguim( $fNomUsuari, $fPwdClavex, $fCodTranps, $fCodManifi,
                                  ...
                                  $mCodGpsxxx, $mCodRemesa, $fBinHuella, $fNumViajex, $fDatgps2xx, $fNomAplica, $fNumIntent );
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          if( count( $conduc ) > 0 )
          {
            $fQueryInsert = "INSERT IGNORE INTO tab_transp_tercer ( cod_transp, cod_tercer, ind_estado, fec_creaci, usr_creaci ) VALUES ('".$fCodTranps."','".$fCodCondu2."','1', NOW(), 'FTP Syscom' ) ";
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        else
        {
          ...
          throw new Exception( "No se encuentra numero de despacho PC.", "1999" );
          break;
        }
...
//              throw new Exception( "Error en Insert.", "3001" );
  ...
  ************************************************************************/
       ...
       try {
        ...
        $mSelect = "(
                      SELECT  
                          ...
                          FROM  ".BASE_DATOS.".tab_despac_vehige a INNER JOIN
                                ".BASE_DATOS.".tab_despac_contro b ON a.num_despac = '{$fNumDespac}' AND a.num_despac = b.num_despac   INNER JOIN
                                ".BASE_DATOS.".tab_genera_noveda e ON b.cod_noveda = e.cod_noveda INNER JOIN 
                                ".BASE_DATOS.".tab_genera_contro c ON b.cod_contro = c.cod_contro INNER JOIN
                                ".BASE_DATOS.".tab_despac_sitio  d ON b.cod_sitiox = d.cod_sitiox 
                    )
                    UNION ALL
                    (
                      SELECT  
                          ...
                          b.usr_creaci
                        FROM  ".BASE_DATOS.".tab_despac_vehige a INNER JOIN
                              ".BASE_DATOS.".tab_despac_noveda b ON a.num_despac = '{$fNumDespac}' AND a.num_despac = b.num_despac INNER JOIN 
                              ".BASE_DATOS.".tab_genera_noveda e ON b.cod_noveda = e.cod_noveda INNER JOIN
                              ".BASE_DATOS.".tab_genera_contro c ON b.cod_contro = c.cod_contro          
                    )
  ...
  {
    //comentado por Harold Peñates - 2021-12-16
