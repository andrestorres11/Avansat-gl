<?php
 ...
 function setSeguim( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTranps = NULL, $fCodManifi = NULL,
                      ...
                      $mCodGpsxxx = NULL, $mCodRemesa = NULL, $fBinHuella = NULL, $fNumViajex = NULL, $fNumIntent = 0  )
  {
    ...
    if( $mCodContrs -> ControSeguim !== NULL && count( $mCodContrs -> ControSeguim ) > 0  )
    {
      $i = 0;
      foreach ( $mCodContrs -> ControSeguim  as $mCodContr)
      { 
    ...
    //----------------------------------------------------------------------
    if( $mCodAgenci -> DataAgencia !== NULL && count( $mCodAgenci -> DataAgencia ) > 0  )
    {
    ...
    if( $mCodGpsxxx -> DataGps !== NULL && count( $mCodGpsxxx -> DataGps ) > 0  )
    {
    ...
    if( $mCodRemesa -> DataRem !== NULL && count( $mCodRemesa ) > 0  )
    {
      $i = 0;
      ...
      { 
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          $fQueryInsDespac = $fSeguim -> setDespac( $fNumDespac, $fCodManifi, $fDatFechax, $fCodPaiori, $fCodDepori, 
                                                    ...
                                                    2, $fLastCodAgenci, 0, 0, "N", "R", FALSE, $fObsComent, $fMovConduc);
          ...
          if( $mCodGpsxxx -> DataGps !== NULL && count( $mCodGpsxxx ) > 0  )
          {
          	$fQueryInsGps = $fSeguim -> SetGps($fNumDespac, $mCodGpsxxx -> DataGps, $fNomUsuari);  //Gps
        ...
        if( "3001" == $e -> getCode() )
        {
          ...
          if( $fNumIntent < 3 )
          {
            ...
            $fReturn = setSeguim( $fNomUsuari, $fPwdClavex, $fCodTranps, $fCodManifi,
                                  ...
                                  $mCodGpsxxx, $mCodRemesa, $fBinHuella, $fNumViajex, $fNumIntent );
...
//         return $fMessageFinally;
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          if( !$fTercer -> tercerExists( $fCodTransp ) )
            throw new Exception( "La Transportadora no esta registrada.".$fCodTransp, "6001" );
  ...
  ************************************************************************/
  function setNovedadPC( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTransp = NULL, $fNumManifi = NULL,
                         ...
                         $fNomSitiox = NULL, $fNumViajex = NULL, $fBinFotcon = NULL )
  {
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          {       
              foreach ($fRow AS $key => $Datax) {
               
                $fArrHomolo[$i]['cod_pcxfar'] = $Datax -> cod_pcxfar;
                $fArrHomolo[$i]['cod_pcxbas'] = $Datax -> cod_pcxbas;
                $fArrHomolo[$i]['ind_estado'] = $Datax -> ind_estado;
              } 
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          mail("nelson.liberato@intrared.net", "getHomoloData faro2", var_export($fHomoloData, true));
          
          if( NULL !== $fHomoloData )
          {
            $fReturn['cod_rutfar'] = "ddd";#$fHomoloData[0]['cod_rutfar'];
            $fReturn['cod_rutbas'] = "sss";#$fHomoloData[0]['cod_rutbas'];
            $fReturn['arr_homolo'] =  $fHomoloData;
            $fReturn['cod_respon'] = '1000';
