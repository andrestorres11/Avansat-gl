...
/* @author Andrés Felipe Malaver                                                             */
/*********************************************************************************************/  
//ini_set('display_errors', true);
//error_reporting(E_ALL & ~E_NOTICE);
die();
class TrazabilidadFARO
{
  var $db4 = NULL;
  var $PDF = NULL;
  ...
  function __construct()
  {
    ...
    include_once("fpdf/multicell_fpdf.php");
  ...
  function GeneratePDF( $arr_client, $key )
  {
    ...
    $this -> PDF -> Cell(10,4,"Fecha",1,0,'C',1);    //Fecha
    $this -> PDF -> Cell(10,4,"Duración",1,0,'C',1); //Duración
    $this -> PDF -> Cell(7,4,"Días",1,0,'C',1);     //Días
    ...
    foreach($pedidos as $pedido)
    {
      ...
      $obs_despac = $inf_despac['obs_noveda'];
  ...
  {
    $mQuery = "( SELECT a.fec_contro, UPPER( b.nom_sitiox )
  ...
  {
    $mQuery = "SELECT ema_client, cod_client
    ...
    if( $_email[0][0] == NULL || $_email[0][0] == '' )
    {
       $mQuery = "SELECT b.dir_emailx, a.cod_client
                 FROM ".BASE_DATOS.".tab_despac_despac a,
                      ".BASE_DATOS.".tab_tercer_tercer b
                WHERE a.num_despac = '".$num_despac."'
                  AND a.cod_client = b.cod_tercer";
      ...
      if( $_email[0][0] == NULL || $_email[0][0] == '' )
      {
        $mQuery = "SELECT ema_client, num_docume
