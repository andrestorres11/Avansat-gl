<?php
  ...
  ************************************************************************/
  ...
  include_once( "/var/www/html/ap/interf/app/faro/notifi.class.inc");
  include_once( "/var/www/html/ap/interf/app/faro/sat.class.inc");
  ...
  function setSoliciRuta($arr_datsol,$arr_rutsol){
    try{
      ...
      $fNomInstance = $arr_datsol->nom_instan;
      ...
      if( $fNomInstance != NULL) // SI NO ES FARO , CARGA EL CLIENTE SATT_TQALA, EJEMPLO
        {
          if(file_exists('/var/www/html/ap/generadores/'.$fNomInstance.'/constantes.inc') )
          {
            include_once('/var/www/html/ap/generadores/'.$fNomInstance.'/constantes.inc');
          }
          else if(file_exists('/var/www/html/ap/integradorgps/'.$fNomInstance.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/integradorgps/'.$fNomInstance.'/constantes.inc');
          }          
          else if(file_exists('/var/www/html/ap/'.$fNomInstance.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/'.$fNomInstance.'/constantes.inc');
          }
          else if(file_exists('/var/www/html/ap/latino/'.$fNomInstance.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/latino/'.$fNomInstance.'/constantes.inc');
          }
          else if(file_exists('/var/www/html/ap/centroa/'.$fNomInstance.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/centroa/'.$fNomInstance.'/constantes.inc');
          }
          else if(file_exists( AplKon ) )
          {
            ...
            throw new Exception( "Aplicacion GENERADOR O NTEGRADOR GPS ".$fNomInstance." encontrada ", "1999" );
          }
        }
        else if(file_exists( AplKon ) ){
          include_once( AplKon );
      ...
      $objSoliciSolici->nom_viaxxx="'".$arr_rutsol->nom_viaxxx."'";
      ...
      $dataMail = (object) array(
              ...
              'asunto' => "Creacion Ruta: ".getTransSolici($objSoliciSolici->num_solici, $fConsult,"1")[0]['origen']." - ".getTransSolici($objSoliciSolici->num_solici, $fConsult, "1")[0]['destino']." Via ".getTransSolici($objSoliciSolici->num_solici, $fConsult)[0]['nom_viaxxx'],
                            ...
                            'mailTo'  =>  getTransSolici($objSoliciSolici->num_solici, $fConsult)[0]['dir_usrmai'].",".$objSoliciDatosx->dir_usrmai.",".FarMai.",supervisores@faro.com.co",
                              );
  ...
  function sendMailSolifa($data = NULL)
  {
      ...
      { 
        ...
        $mailToS = str_replace("'", "", $mailToS);
        mail( $mailToS, "sol. ".$data->asunto, '<div name="_faro_07">' . $mHtmlxx . '</div>', $mCabece, "-f $from -r $from" );
          //mail($to,$subject,$message,"From: <$from>","-f $from -r $from");
      }
      catch(Exception $e)
      {
        return "code_resp:".$e->getCode()."; msg_resp:".$e->getMessage();
 ...
 function setSeguim( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTranps = NULL, $fCodManifi = NULL,
    ...
    if( $fCodTranps == '900243606' )
  {
    ...
    $fInputs = array( "nom_usuari" => $fNomUsuari, "pwd_clavex" => $fPwdClavex, "cod_tranps" => $fCodTranps, 
                      ...
                      "cod_agedes" => $mCodAgedes, "cod_operad" => $fCodoperad, "inf_remesa" => $mCodRemesa,
                      "inf_gpsxxx" => $mCodGpsxxx);

    //Log Muestra de creaci?n de despacho
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\n---------------------------------------------------" );
    $reg = fwrite( $file, "\nExecution Date-Time: ".date( "Y/m/d H:i:s" )."" );
    $reg = fwrite( $file, "\nData Created: ".var_export($fInputs,true)."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 1: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 2: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 3: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    // -------------------------------------------------------------------------------
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 4: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 5: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 6: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado Messages: ".var_export( $fMessagesFinally, true )."" );
    $close = fclose( $file );
    ...
    $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
    $file = fopen( $mLogFile, "a+" );
    $reg = fwrite( $file, "\nProcesado PARTE 6.1: ".date( "Y/m/d H:i:s" )."" );
    $close = fclose( $file );
    ...
    if( $flagCode === 0  )//AND count($mNumMessageGps) ===  1
    {
      ...
      { 
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 7: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 8: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );

        if( $fNomAplica != NULL) // SI NO ES FARO , CARGA EL CLIENTE SATT_TQALA, EJEMPLO
        {
          if(!include_once('/var/www/html/ap/'.$fNomAplica.'/constantes.inc') )
          {
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 9: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 10: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 11: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 12: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
          ...
          $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nProcesado PARTE 13: ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
          ...
          $cabeceras  = 'MIME-Version: 1.0' . "\r\n";
                ...
                $cabeceras .= 'From: SAT EMPRESARIAL <no-reply@intrared.net>' . "\r\n";
          $interfazIntegrador = getInterfaceStanda(59, $fCodTranps, $fExcept);
         
          if( FALSE !== $interfazIntegrador){
                $fCodMarcax = $fCodLineax == '' ? 99999 : $fCodMarcax;
                $fCodLineax = $fCodLineax == '' ? 1 : $fCodLineax;
                
                $fusrInterf= $interfazIntegrador["NomUsuari"];
                $fconInterf = $interfazIntegrador["ClvUsuari"];
                $urlApi = $interfazIntegrador["UrlWebSer"];
                $codToken = $interfazIntegrador["CodTokenx"];
                unset( $interfazIntegrador );
                $mDespac =  [];
                $mDespac['dispatch']['transporter_id'] = $fCodTranps;
                $mDespac['dispatch']['manifest'] = $fInputs['cod_manifi'];
                $mDespac['dispatch']['origin'] = $fInputs["cod_ciuori"];
                $mDespac['dispatch']['destiny'] = $fInputs["cod_ciudes"];
                $mDespac['dispatch']['observation'] = $fInputs['obs_coment'];
                $mDespac['dispatch']['route_id'] = '';
                $mDespac['dispatch']['route_name'] = '';
                $mDespac['dispatch']['mobilization_id'] = 2;
                $mDespac['dispatch']['order'] = $fInputs['cod_manifi'];
                $mDespac['dispatch']['date'] = date('Y-m-d H:i:s');
                $mDespac['dispatch']['plate'] = $fInputs['cod_placax'];
                $mDespac['dispatch']['ws'] = 1;  
                //No validar agencia
                $mDespac['dispatch']['no_val_agenci'] = 1;

                $mDespac['vehicle']['plate'] = $fInputs['cod_placax'];
                $mDespac['vehicle']['brand'] = $fCodMarcax;
                $mDespac['vehicle']['line'] = $fCodLineax;
                $mDespac['vehicle']['model'] = $fInputs['num_modelo'];
                $mDespac['vehicle']['color'] = $fInputs['cod_colorx'];
                $mDespac['vehicle']['configuration'] = $fInputs['num_config'];
                $mDespac['vehicle']['bodywork_id'] = $fInputs['cod_carroc'];

                $mDespac['vehicle']['driver']['document_id'] = $fInputs['cod_conduc'];
                $mDespac['vehicle']['driver']['first_name'] = $fInputs['nom_conduc'];
                $mDespac['vehicle']['driver']['second_name'] = ''; //OJO
                $mDespac['vehicle']['driver']['first_last_name'] = ''; //OJO
                $mDespac['vehicle']['driver']['second_last_name'] = ''; //OJO
                $mDespac['vehicle']['driver']['city'] = $fInputs['ciu_conduc'];
                $mDespac['vehicle']['driver']['address'] = $DirConduc;
                $mDespac['vehicle']['driver']['landline'] = $fInputs['tel_conduc'];
                $mDespac['vehicle']['driver']['cell_phone_operator'] = '1';
                $mDespac['vehicle']['driver']['cell_phone'] = $fInputs['mov_conduc'];
                $mDespac['vehicle']['driver']['licence'] = $fInputs['cod_conduc'];
                $mDespac['vehicle']['driver']['license_category'] = $fInputs['cat_licenc'];
                $mDespac['vehicle']['driver']['photo'] = '';

                $mDespac['vehicle']['owner']['document_type'] = 'N';
                $mDespac['vehicle']['owner']['document_id'] = $fCodPoseed;
                $mDespac['vehicle']['owner']['abbreviation'] =  $fNomPoseed;
                $mDespac['vehicle']['owner']['first_name'] = $fNomPoseed;
                $mDespac['vehicle']['owner']['second_name'] = '';
                $mDespac['vehicle']['owner']['first_last_name'] = '';
                $mDespac['vehicle']['owner']['second_last_name'] = '';
                $mDespac['vehicle']['owner']['city'] = $fCiuPoseed;
                $mDespac['vehicle']['owner']['address'] = $DirPoseed == NULL ? 'N/A' : $DirPoseed;

                if(sizeof($fInputsGps)>0){
                    $mDespac['vehicle']['devicegps']['document_number'] = $fInputsGps['cod_opegps'];
                    $mDespac['vehicle']['devicegps']['name'] = $fInputsGps['nom_operad'];
                    $mDespac['vehicle']['devicegps']['user'] = $fInputsGps['nom_usrgps'];
                    $mDespac['vehicle']['devicegps']['password'] = $fInputsGps['clv_usrgps'];
                    $mDespac['vehicle']['devicegps']['url'] = '';
                }else{
                    $mDespac['vehicle']['devicegps']['document_number'] = NULL;
                    $mDespac['vehicle']['devicegps']['name'] = NULL;
                    $mDespac['vehicle']['devicegps']['user'] = NULL;
                    $mDespac['vehicle']['devicegps']['password'] = NULL;
                    $mDespac['vehicle']['devicegps']['url'] = NULL;
                }

                //Valida las dos maneras de los clientes de enviar la informaci�n de las remesas.
                if(isset($remesa->inf_client) && isset($remesa->inf_remite) && isset($remesa->inf_destin)){
                  foreach ($mCodRemesa as $index => $remesa) {
                      $infClient = $remesa->inf_client;
                      $infRemite = $remesa->inf_remite;
                      $infDestin = $remesa->inf_destin;
                      
                      $clientData = json_decode($infClient, true);
                      $remiteData = json_decode($infRemite, true);
                      $destinData = json_decode($infDestin, true);

                      $mDespac['remitances'][$index]['remitance'] = $remesa->cod_remesa;
                      $mDespac['remitances'][$index]['weight'] = $remesa->pes_cargax;
                      $mDespac['remitances'][$index]['volume'] = $remesa->vol_cargax;
                      $mDespac['remitances'][$index]['package_name'] = $remesa->nom_empaqu;
                      $mDespac['remitances'][$index]['commodity'] = $remesa->abr_mercan;
                      $mDespac['remitances'][$index]['estimated_delivery_date'] = $remesa->fec_citdes;
                      $mDespac['remitances'][$index]['arrival_date'] = '0000-00-00 00:00:00';
                      $mDespac['remitances'][$index]['departure_date'] = '0000-00-00 00:00:00';
                      
                      
                      $mDespac['remitances'][$index]['client']['document_type'] = $clientData['document_type'];
                      $mDespac['remitances'][$index]['client']['document_id'] = $clientData['document_id'];
                      $mDespac['remitances'][$index]['client']['first_name'] = $clientData['first_name'];
                      $mDespac['remitances'][$index]['client']['city'] = $clientData['city'];
                      $mDespac['remitances'][$index]['client']['address'] = $clientData['address'];

                      
                      $mDespac['remitances'][$index]['sender']['document_type'] = $remiteData['document_type'];
                      $mDespac['remitances'][$index]['sender']['document_id'] = $remiteData['document_id'];
                      $mDespac['remitances'][$index]['sender']['first_name'] = $remiteData['first_name'];
                      $mDespac['remitances'][$index]['sender']['city'] = $remiteData['city'];
                      $mDespac['remitances'][$index]['sender']['address'] = $remiteData['address'];
                      $mDespac['remitances'][$index]['sender']['latitude'] = $remiteData['latitude'];
                      $mDespac['remitances'][$index]['sender']['longitude'] = $remiteData['longitude'];

                      $mDespac['remitances'][$index]['recipient']['document_type'] = $destinData['document_type'];
                      $mDespac['remitances'][$index]['recipient']['document_id'] = $destinData['document_id'];
                      $mDespac['remitances'][$index]['recipient']['first_name'] = $destinData['first_name'];
                      $mDespac['remitances'][$index]['recipient']['city'] = $destinData['city'];
                      $mDespac['remitances'][$index]['recipient']['address'] = $destinData['address'];
                      $mDespac['remitances'][$index]['recipient']['latitude'] = $destinData['latitude'];
                      $mDespac['remitances'][$index]['recipient']['longitude'] = $destinData['longitude'];
                  }
                }else{
                  foreach ($mCodRemesa as $index => $remesa) {
                    $mDespac['remitances'][$index]['remitance'] = $remesa->cod_remesa;
                    ...
                    $mDespac['remitances'][$index]['arrival_date'] = '0000-00-00 00:00:00';
                    $mDespac['remitances'][$index]['departure_date'] = '0000-00-00 00:00:00';
                    $mDespac['remitances'][$index]['client']['document_type'] = NULL;
                    $mDespac['remitances'][$index]['client']['document_id'] = $remesa->cod_client;
                    $mDespac['remitances'][$index]['client']['first_name'] = $remesa->abr_tercer;
                    $mDespac['remitances'][$index]['client']['city'] = NULL;
                    $mDespac['remitances'][$index]['client']['address'] = NULL;

                    $mDespac['remitances'][$index]['sender']['document_type'] = NULL;
                    $mDespac['remitances'][$index]['sender']['document_id'] = $remesa->cod_client;
                    $mDespac['remitances'][$index]['sender']['first_name'] = $remesa->nom_remite;
                    $mDespac['remitances'][$index]['sender']['address'] = NULL;
                    $mDespac['remitances'][$index]['sender']['latitude'] = NULL;
                    $mDespac['remitances'][$index]['sender']['longitude'] = NULL;

                    $mDespac['remitances'][$index]['recipient']['document_type'] = NULL;
                    $mDespac['remitances'][$index]['recipient']['document_id'] = $remesa->cod_client;
                    $mDespac['remitances'][$index]['recipient']['first_name'] = $remesa->nom_destin;
                    $mDespac['remitances'][$index]['recipient']['city'] = $remesa->ciu_destin;
                    $mDespac['remitances'][$index]['recipient']['address'] = $remesa->dir_destin;
                    $mDespac['remitances'][$index]['recipient']['latitude'] = NULL;
                    $mDespac['remitances'][$index]['recipient']['longitude'] = NULL;
                  }
                }
                ...
                $mDespac['agency']['agency_id'] = $fInputs['cod_agenci'];
                $mDespac['agency']['agency_name'] = $fInputs['nom_agenci'];
                $mDespac['agency']['city'] = $fInputs['cod_ciudad'];
                $mDespac['agency']['address'] = $fInputs['dir_agenci'];
                $mDespac['agency']['landline'] = $fInputs['tel_agenci'];
                $mDespac['agency']['contact_name'] = $fInputs['con_agenci'];
                $mDespac['agency']['mail'] = $fInputs['dir_emailx'];
                $mDespac['agency']['fax'] = $fInputs['num_faxxxx'];

                //Log Muestra de creaci?n de despacho para integrador gps puente.
                $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguimIntegrador_".date( "Y_m_d" ).".txt";
                $file = fopen( $mLogFile, "a+" );
                $reg = fwrite( $file, "\n---------------------------------------------------" );
                $reg = fwrite( $file, "\nExecution Date-Time: ".date( "Y/m/d H:i:s" )."" );
                $reg = fwrite( $file, "\nData Created: ".var_export($mDespac,true)."" );
                $close = fclose( $file );     
                ...
                $mDataDespacObj = json_encode($mDespac, JSON_NUMERIC_CHECK | JSON_PRETTY_PRINT);
                $mDataDespacObjDeco = json_decode($mDataDespacObj);
                $mDataDespacObjDeco -> dispatch -> manifest = $mDespac['dispatch']['manifest'] ;
                $mDataDespac = json_encode($mDataDespacObjDeco, JSON_PRETTY_PRINT);

                $mResponse = sendApiNuevoGL($fCodTranps, $fExcept, $mDataDespac, 'dispatch');
                
                //Log Muestra de creaci?n de despacho para integrador gps puente.
                $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
                $file = fopen( $mLogFile, "a+" );
                $reg = fwrite( $file, "\nRespuesta Avansat GL : ".var_export($mResponse,true));
                $close = fclose( $file ); 
                if($mResponse['cod_respon'] != 1000){ 

                }else{
                  $token = $mResponse['dat_respon']['data']['token'];
                  //Arma y organiza la data
                  $mCodResp = $mResponse['cod_respon'];
                  $mMsgResp = $mResponse['msg_respon'];

                  return "code_resp:".$mCodResp."; msg_resp:".$mMsgResp;
                }
          }
          ...
          // validaci�n para enviar despacho a la central para la app, pero para fajober
      ...
      catch( Exception $e )
      {
        ...
        $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setSeguim_".date( "Y_m_d" ).".txt";
        $file = fopen( $mLogFile, "a+" );
        $reg = fwrite( $file, "\nERROR DE CATCH: ".$e -> getMessage()." - ".$e -> getLine()." _ ".date( "Y/m/d H:i:s" )."" );
        $close = fclose( $file );
      }
      return $fReturn;
    }
...
//         return $fMessageFinally;
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          if( 0 != $fConsult -> RetNumRows() )
          {
            ...
            if( FALSE !== $interfazIntegrador ){
              ...
              $mResponse = curl([
                'Url' => $urlApi."/authenticate?userId=".$fusrInterf."&pwd=".$fconInterf,
                'Headers' => [
                                'app-id: 13f5049d-1dec-11eb-9e04-005056013943',
                                'Authorization: '.$codToken,
                ...
                if($mResponse['cod_respon'] != 1000){ 
                  ...
                  //Arma y organiza la data
                  $mDespac =  [];
                  $mDespac['dispatch_id'] = NULL;
                  $mDespac['transporter_id'] = $fCodTranps;
                  $mDespac['manifest'] = $fCodManifi;
                  $mDespac['plate'] = $fNumPlacax;
                  $mDespac['finish_trip'] = date('Y-m-d H:i:s');
                                     
                  //Log Muestra de creaci?n de despacho para integrador gps puente.
                  $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setFinalizaIntegrador_".date( "Y_m_d" ).".txt";
                  $file = fopen( $mLogFile, "a+" );
                  $reg = fwrite( $file, "\n---------------------------------------------------" );
                  $reg = fwrite( $file, "\nExecution Date-Time: ".date( "Y/m/d H:i:s" )."" );
                  $reg = fwrite( $file, "\nData Created: ".var_export($mDespac,true)."" );
                  ...
                  $mDataDespacObj = json_encode($mDespac, JSON_NUMERIC_CHECK | JSON_PRETTY_PRINT);
                  $mDataDespacObjDeco = json_decode($mDataDespacObj);
                  $mDataDespacObjDeco -> manifest = $mDespac['manifest'] ;
                  $mDataDespac = json_encode($mDataDespacObjDeco, JSON_PRETTY_PRINT);
                    
                  $mResponse = curl([
                    'Url' => $urlApi."/finishtrip",
                    'Headers' => [
                                  'app-id: 13f5049d-1dec-11eb-9e04-005056013943',
                                  'Authorization: '.$codToken,
                                  'token: '.$token,
  ...
  * @return string respuesta del webservice.                             *
  ************************************************************************/  
  function setDespachoAPP($fNitFajobe = '', $fNumDespac = '', $fConsult )  
  {
    try {
          ...
          if(sizeof($mMatriz) > 0)
          {
            ...
            if ( $mMatriz[0]['ind_estado']) 
            {
              ...
              $response    = $controlador->registrar($fConsult, $fNumDespac, $fNitFajobe);    
  ...
  ************************************************************************/
  function setNovedadGPS( $fCodTransp = NULL, $fNumDespac = NULL, $fCodNoveda = NULL, $fFecNoveda = NULL, 
                          ...
                          $fNumPlacax = NULL, $fNomAplica = NULL)
  {
      ...
      try
      {
       //if( $fNomAplica == 'satt_faro' )
        ...
        if( $fNomAplica != NULL) // SI NO ES FARO , CARGA EL CLIENTE SATT_TQALA, EJEMPLO
        {
          ...
          else if(file_exists('/var/www/html/ap/integradorgps/'.$fNomAplica.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/integradorgps/'.$fNomAplica.'/constantes.inc');
          }          
          else if(file_exists('/var/www/html/ap/'.$fNomAplica.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/'.$fNomAplica.'/constantes.inc');
          }
          else if(file_exists('/var/www/html/ap/latino/'.$fNomAplica.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/latino/'.$fNomAplica.'/constantes.inc');
          }
          else if(file_exists('/var/www/html/ap/centroa/'.$fNomAplica.'/constantes.inc'))
          {
            include_once('/var/www/html/ap/centroa/'.$fNomAplica.'/constantes.inc');
          }
          else if(file_exists( AplKon ) )
          {
            include_once( AplKon );
            ...
            throw new Exception( "Aplicacion GENERADOR O INTEGRADOR GPS ".$fNomAplica." encontrada ", "1999" );
          }
        }
        else if(file_exists( AplKon ) ){
          include_once( AplKon );
        ...
        if(($fNomLlavex == NULL && $fNomUsuari == NULL && $fPwdClavex == NULL) || ($fNomLlavex != NULL && $fNomUsuari != NULL && $fPwdClavex != NULL) )       
          throw new Exception( "Debe enviar solo la LLAVE &oacute; USUARIO y CLAVE para continuar. ".$fNomLlavex, "1002" );        
        ...
        // -----------------------------------------------------  Fin Validaci�n Usuarios y Llave  ---------------------------------------------------------
        // -------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        {   
            ...
            if( $fCodOperad != NULL && $fNomUsuari != NULL && $fPwdClavex != NULL )
            {
              ...
              //Validaci�n del numero del codigo de  manifiesto y placa
              ...
              */
              ...
              {              
                throw new Exception( "No se encontro el despacho No: ".$fNumDespac.", con placa: ".$fNumPlacax.", codigo de transportadora:".$fCodTransp." en Sat Trafico(".BASE_DATOS.").", "6001" ); 
              }              
            }
            ...
            //Se obtiene el ultimo consecutivo del despacho
            $query = "SELECT b.zon_horari
                       FROM ".BD_STANDA.".tab_opegps_paisxx a
                            INNER JOIN ".BD_STANDA.".tab_genera_paises b ON a.cod_paisxx = b.cod_paisxx
                      WHERE a.cod_operad = '".$fCodOperad."' ";
            $consulta = $fConsult -> ExecuteCons( $query );
            $zonhoraria = $fConsult -> RetMatrix( 'a' );

            if(sizeof($zonhoraria)>0){
              $fecha_hora_colombia = new DateTime();
              $fecha_hora_otra_region = clone $fecha_hora_colombia;
              // Establecer la zona horaria deseada (por ejemplo, Panam�)
              $zona_horaria_otra_region = new DateTimeZone($zonhoraria[0]['zon_horari']);
              $fecha_hora_otra_region->setTimezone($zona_horaria_otra_region);
              // Obtener la fecha y hora en el formato YYYY-mm-dd H:i:s
              $fecha_servidor = $fecha_hora_otra_region->format('Y-m-d H:i:s');
            }else{
              //Si no esta configurado toma la fecha y hora de Colombia por defecto.
            ...
            $mUpdateSet[] = "fec_modifi = '".$fecha_servidor."'";
  ...
  ************************************************************************/
  function setNovedadPC( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTransp = NULL, $fNumManifi = NULL,
                         ...
                         $fTimSegpun = NULL, $fTimUltpun = NULL, $KmsVehicux = NULL, $fNomAplica = NULL )


  {
    // Se ampl�a la memoria ya que generar error en el cliente, desde el TMS
      ...
      try
      {
        ...
        if( $fNomAplica != NULL) // SI NO ES FARO , CARGA EL CLIENTE SATT_TQALA, EJEMPLO
        {
          ...
          else if(file_exists( AplKon ) )
          {
            ...
            throw new Exception( "Aplicacion GENERADOR O NTEGRADOR GPS ".$fNomAplica." encontrada ", "1999" );
          }
        }
        else if(file_exists( AplKon ) ){
          include_once( AplKon );
        ...
        $interfazIntegrador = getInterfaceStanda(59, $fCodTransp, $fExcept);
        if( FALSE !== $interfazIntegrador){
          $mData = [];
          $mData['dispatch_id'] = NULL;
          $mData['transporter_id'] = $fCodTransp;
          $mData['manifest'] = $fNumManifi;
          $mData['plate'] = $fNumPlacax;
          $mData['novelty_id'] = $fCodNoveda;
          $mData['date_novelty'] = $fFecNoveda;
          $mData['observation'] = $fDesNoveda;
          $mData['extra_time'] = $fTimDuraci;
          $mData['checkpoint_id'] = '';
          $mData['destination_code'] = '';
          $mData['last_point_time'] = '';
          $mData['vehicle_speed'] = '';
          $mData['latitude'] = '';
          $mData['longitude'] = '';
          array_walk_recursive($mData, function( &$value, $key){
            $value = utf8_encode($value);
          });
          $mDataNovedadObj = json_encode($mData, JSON_NUMERIC_CHECK | JSON_PRETTY_PRINT);
          $mDataNovedadObjDeco = json_decode($mDataNovedadObj);
          $mDataNovedadObjDeco -> manifest = $fNumManifi;
          $mData = json_encode($mDataNovedadObjDeco, JSON_PRETTY_PRINT);
          $mResponse = sendApiNuevoGL($fCodTransp, $fExcept, $mData, 'novelty-on-site');

          if($fCodTransp=='830104929'){
            $msg_return = "code_resp:".$mCodResp."; msg_resp:".$mMsgResp;
            return $msg_return;
          }
        ...
        else
        {
          ...
          throw new Exception( "No se encuentra numero de despacho PC. Transp  ".$fCodTransp." Manifi  ".$fNumManifi." Placa  ".$fNumPlacax." fNomAplica ".$fNomAplica." BASE_DATOS ".BASE_DATOS, "1999" );
          break;
        }
        ...
        if($fNoveda['cod_noveda'] == '9266XXX')
        {
          ...
          // valida si encuentra algo , para colocar el PC homologado que est�n en el plan de ruta para poder registrar la novedad en sitio con el PC Hijo
        ...
        // ------------------------------------------------------------------------------------------------------------------------------------
        // parche para no correr tiempo cuando el despacho tiene como ultima novedad la pernoctaci�n ------------------------------------------
          $mQuery = 'SELECT 
                        -- Cuenta los incios y los fin de pernoc y valida si  los incios son mayores a lof finaliza, es porque est� pernoctando, si son iguales es porque ya no est� pernoctando
        ...
        if($fNumViajex != NULL){         
          ...
          $fConsult -> CloseConection( $fNomUsuari." - Despacho: ".$fNumDespac." - Codigo verificaci�n: ".$fCodVerifi." - Desde setNovedadPC" );       
  ...
  { 
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        else if($fNomAplica != "satt_faro") // SI NO ES FARO , CARGA EL CLIENTE SATT_TQALA, EJEMPLO
        {
          if(!include_once('/var/www/html/ap/generadores/'.$fNomAplica.'/constantes.inc') )
          {
            throw new Exception( "Aplicacion GL ".$fNomAplica."  en sat trafico no encontrada", "1999" );
        ...
        if( FALSE !== $interfazIntegrador){
          ...
          $mDataNovedadObjDeco -> manifest = $fNumManifi;
          $mData = json_encode($mDataNovedadObjDeco, JSON_PRETTY_PRINT);
          $mResponse = sendApiNuevoGL($fCodTransp, $fExcept, $mData, 'novelty-before-of-site');
          $mCodResp = $mResponse['cod_respon'];
          $mMsgResp = $mResponse['msg_respon'];
          if($fCodTransp=='830104929'){
            $msg_return = "code_resp:".$mCodResp."; msg_resp:".$mMsgResp;
        ...
        */
        ...
        //Parche para traer los parametros configurados de la novedad
        if(BASE_DATOS=='satt_faro'){
          $query = "SELECT ind_manala, ind_notsup, nom_noveda, nov_especi
                      FROM ".BASE_DATOS.".tab_genera_noveda
                    WHERE cod_noveda = '".$fCodNoveda."' ";
        }else{
          $query = "SELECT a.ind_manale, a.ind_notsup, b.nom_noveda, a.ind_novesp
                      FROM ".BASE_DATOS.".tab_parame_novseg a
                INNER JOIN ".BASE_DATOS.".tab_genera_noveda b ON a.cod_noveda = b.cod_noveda
                    WHERE a.cod_noveda = '".$fCodNoveda."' AND a.cod_transp = '".$fCodTransp."' ";
        }
        $consulta = $fConsult -> ExecuteCons( $query );
        $ind_manala = $fConsult -> RetMatrix( );
        ...
        if($ind_novesp == 1 AND BASE_DATOS == 'satt_intgps'){
          $mNotifi = new Notificacion($fConsult, $fNomUsuari, $fCodTransp, $fNumDespac, 
                                      $fCodNoveda, $fFecNoveda, $fDesNoveda, $fNomSitiox);         
          $mRespon = $mNotifi -> getResponse();    
        }
        ...
        {
          // ---------------------------------------------------------------------------------------------------------------------------------------------------------------
          // parche para no correr tiempo cuando el despacho tiene como ultima novedad la pernoctaci�n APLICA PARA NOVEDAD DE APP ------------------------------------------
          $mQuery = 'SELECT 
                        -- Cuenta los incios y los fin de pernoc y valida si  los incios son mayores a lof finaliza, es porque est� pernoctando, si son iguales es porque ya no est� pernoctando
                        IF( (SUM( IF( a.cod_noveda IN (6, 245, 258, 9008) , 1 , 0 ) ) ) != SUM( IF( a.cod_noveda IN (211, 9025) , 1 , 0 ) ), 0 , 1 ) AS ind_modifi_alarma
                      FROM
        ...
        $fQueryInsNoved = "INSERT INTO ".BASE_DATOS.".tab_despac_contro ".
                            "( num_despac , cod_rutasx , cod_contro , cod_consec , ".
                              ...
                              "cod_sitiox, usr_creaci ,$fields kms_vehicu, fec_creaci ) ".
                          "VALUES ( '".$fNumDespac."', '".$fDesp["cod_rutasx"]."', ".
                                   "'".$fCodContro."', '".$fConse."', ".
                                   "'INTERFAZ - ".str_replace("'", "", $fDesNoveda)." ', '".$fNoveda["cod_noveda"]."', ".
                                   "'".$tieadi."', '".$fFecNoveda."', ".
                                   "'".$maxsit."', '".$fNomUsuari."',$values '".$KmsVehicux."', NOW() ) ON DUPLICATE KEY UPDATE fec_creaci = DATE_ADD( VALUES(fec_creaci), INTERVAL 1 SECOND ) ";
        if( $fConsult -> ExecuteCons( $fQueryInsNoved, "R" ) === FALSE ) {
          throw new Exception( "Error en Insert 2.".$fCodTransp." Pre: ".$fQueryInsNoved, "3001" );
        }*/

        //Consulta si ya se han registrado novedades con caracteristicas similares aplica unicamente intgps
        $HisNoved = 0;
        if(BASE_DATOS=='satt_intgps'){
          $mQuery = 'SELECT cod_consec FROM '.BASE_DATOS.'.tab_despac_contro
                         WHERE num_despac = "'.$fNumDespac.'" AND fec_contro="'.$fFecNoveda.'"
                         AND cod_noveda = "'.$fNoveda["cod_noveda"].'" ORDER BY fec_creaci DESC';
          $consulta = $fConsult -> ExecuteCons( $mQuery );
          $HisNoved = count($fConsult -> RetMatrix( 'a' ));
          if($HisNoved > 0 ){
          }
        }
        
        //Consulta si ya se han registrado novedades con caracteristicas similares - faro
        /*
        $fValidRegNoved = "SELECT cod_consec FROM ".BASE_DATOS.".tab_despac_contro 
                              WHERE 
                                num_despac = '".$fNumDespac."' AND cod_contro = '".$fCodContro."' AND cod_rutasx = '".$fDesp["cod_rutasx"]."' 
                            AND obs_contro = 'INTERFAZ - ".str_replace("'", "", $fDesNoveda)." ' AND cod_noveda = '".$fNoveda["cod_noveda"]."' AND cod_sitiox = '".$maxsit."' 
                            AND fec_contro = '".$fFecNoveda.":00' ";
        $consulta = $fConsult -> ExecuteCons( $fValidRegNoved );
        $HisNovedReg = count($fConsult -> RetMatrix( 'a' ));
        if($HisNovedReg>0){
          throw new Exception( "Error al registar la novedad: La novedad ya esta registrada".$fCodTransp." Pre: ".$fValidRegNoved, "3001" );
        }*/
        ...
        $fQueryInsNoved = "INSERT INTO ".BASE_DATOS.".tab_despac_contro ".
                          ...
                          "VALUES ( '".$fNumDespac."', '".$fDesp["cod_rutasx"]."', ".
                                   "'".$fCodContro."', '".$fConse."', ".
                                   "'INTERFAZ - ".corregirTildes(str_replace("'", "", $fDesNoveda))." ', '".$fNoveda["cod_noveda"]."', ".
                                   "'".$tieadi."', '".$fFecNoveda."', ".
...
//              throw new Exception( "Error en Insert.", "3001" );
        ...
        if( $ind_manala == '0' )
          ...
          //Hace el commit si la novedad no esta repetida. Evita multiples registros gps.
          /*if(BASE_DATOS== 'satt_intgps'){
            if($HisNoved==0){
              $fConsult -> Commit();
            }
        ...
        if ($fNomAplica == 'satt_fortec'){
          ...
          //Ejecuta clase que registra la homologaci�n de la novedad y la inserta para enviar correo con el estado del despacho
          $mtrackin = new TrackingDespac();
          $mInsert = $mtrackin -> setTrackingDespac($fNumDespac, null , 2, $fNoveda["cod_noveda"], $fNomUsuari);
      ...
      catch( Exception $e )
      {
        ...
        $mFile = fopen("/var/www/html/ap/interf/app/faro/logs/setNovedadNC-".date("Ymd").".txt", "a+");
        fwrite( $mFile, var_export($fReturn, true)."\n");
        fwrite( $mFile, "----------------------");
        fclose($mFile);
  ...
  /***********************************************************************
  * Funcion Inserta una novedad de UBICACION GPS                         *
  * @fn setNovedadUbicacionGPS.                                          *
  * @brief Inserta una novedad NC en una aplicacion SAT.                 *
  * @param $fNomUsuari: string Usuario.                                  *
  * @param $fPwdClavex: string Clave.                                    *
  * @param $fCodTransp: string Codigo de la transportadora.              *
  ...
  * @return string mensaje de respuesta.                                 *
  ************************************************************************/
  function setNovedadUbicacionGPS( $fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTransp = NULL, $fNumManifi = NULL,
                         $fNumPlacax = NULL, $fCodNoveda = NULL, $fCodContro = NULL, $fValLatitu = NULL,
                         $fValLongit = NULL, $fTimDuraci = NULL, $fFecNoveda = NULL, $fDesNoveda = ""  ,
                         $fNomNoveda = NULL, $fNomContro = NULL, $fNomSitiox = NULL, $fNumViajex = NULL,
                         $fFotNoveda = NULL, $fCodRemdes = NULL, $fTimSegpun = NULL, $fTimUltpun = NULL,
                         $KmsVehicux = NULL, $fNomAplica = NULL)
  { 
    $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
    $fExcept -> SetUser( $fNomUsuari );
    $fExcept -> SetParams( "Faro", "setNovedadNC" );
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        */
        ...
        if($fValLongit != NULL && $fValLongit != '' && $fValLatitu != NULL && $fValLatitu !='' ){
          $fields = " val_latitu,val_longit, ";
          $values = " '$fValLatitu','$fValLongit' , ";
        }
...
//              throw new Exception( "Error en Insert.", "3001" );
  ...
  ************************************************************************/
       ...
       //$mTipFecha = 'fec_cumdes';
        ...
        if( in_array($mTipFecha,  array('fec_inicar','fec_fincar') ) )
        {
          // fec_inicar, fec_fincar
            ...
            //mail("luis.manrique@eltransporte.org", "Prueba despacho ".$mNumDespac." Actualizaci�n fechas", $mUpdate);
        }
  ...
  function setStatusItinerary($fNomUsuari = NULL ,$fPwdClavex = NULL ,$fNomAplica = NULL, $fCodTranps = NULL ,$fCodManifi = NULL ,$fNumPlacax = NULL ,
                              $fCodItiner = NULL ,$fMsgItiner = NULL ,$fFecItiner = NULL, $fMapTokenx = NULL ,$fGeodestin = NULL, $fIndOrigen = NULL ,
                              $fCodRespon = NULL )
  {
    $fInputs = array( "nom_usuari" => $fNomUsuari, "pwd_clavex" => $fPwdClavex, "cod_tranps" =>  $fCodTranps, "cod_manifi" => $fCodManifi, 
    ...
    if( "1000" === $fMessages["code"] )
    {
      try
      {
        ...
        if( getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) )
        {
          ...
          if( 0 != $fConsult -> RetNumRows() )
          {
            ...
            $fMapTokenx = ($fMapTokenx == '' || $fMapTokenx == NULL ? 'NULL' : "'".$fMapTokenx."'" );
            $fCodRespon = ($fCodRespon == '' || $fCodRespon == NULL ? 'NULL' : "'".$fCodRespon."'" );

            $query = "UPDATE ".BASE_DATOS.".tab_despac_vehige
                         SET  cod_itiner = IF(cod_itiner IS NULL, ".$fCodItiner." , cod_itiner) ,
                              msg_itiner = '".addslashes($fMsgItiner)."',
                              fec_itiner = IF(fec_itiner IS NULL, ".$fFecItiner." , fec_itiner) ,
                              cod_integr = IF(cod_integr IS NULL, ".$fCodItiner." , cod_integr) ,
                              cod_respon = IF(cod_respon IS NULL, ".$fCodRespon." , cod_respon) ,
                              map_tokenx = IF(map_tokenx IS NULL, ".$fMapTokenx." , map_tokenx)
                        WHERE num_despac = '".$fNumDespac[0]["num_despac"]."'"; 
            ...
            $mConectSat = new Sat($fConsult  , $fNomUsuari);
            $mInterface = $mConectSat -> getInterfaceSAT(53, $fCodTranps);

            if(sizeof($mInterface)>0){
              $nFecItiner = str_replace("'","",$fFecItiner); 
              $mParams =  [ 
                  'nom_usuari'  => $mInterface['nom_usuari'],
                  'pwd_clavex'  => $mInterface['clv_usuari'],
                  'nom_aplica'  => $mInterface['nom_operad'],
                  'cod_transp'  => $fCodTranps,
                  'cod_manifi'  => $fCodManifi,
                  'num_placax'  => $fNumPlacax,
                  'cod_itiner'  => $fCodItiner,
                  'msg_itiner'  => cleanArray($fMsgItiner), 
                  'fec_itiner'  => $nFecItiner,
                  'map_tokenx'  => '',
                  'geo_destin'  => $fGeodestin
              ];
              $sendTMS = $mConectSat -> setInfoItinerary($mInterface['url_webser'], $mParams);
            }
      ...
      catch( Exception $e )
      {
        ...
        {
          $fReturn = ["cod_respon" => $e -> getCode(), "msg_respon" =>  $e -> getMessage()." Line:". $e->getLine() ];
  ...
  /*! \fn: getDatosSolicitudes
    ...
    */  
    function getDatosSolicitudes($fNomUsuari,$fPwdClavex,$fCodClient){
      ...
      try{
          ...
          $mSelect = "SELECT 
                      ...
                      FROM ".BASE_DATOS.".tab_asiste_carret a 
                      INNER JOIN ".BASE_DATOS.".tab_formul_asiste b ON a.tip_solici = b.id 
                      WHERE a.est_solici = 6 AND a.cod_client = '$fCodClient'";
          $fConsult -> ExecuteCons( $mSelect, "R" ) ;
    ...
    function getTipoSolicitud($fNomUsuari,$fPwdClavex){
      ...
      /* \fn: getListaAsegurad
      *  \brief: Consulta los diferentes tipos de solicitudes
      *  \author: Arnulfo Casta�eda Yaima
      *  \date: 20/04/2022
      *  \param: fNomUsuari  String  Usuario
      *  \param: fPwdClavex  String  Clave
      *  \return: Matriz
      */ 
      ...
      function getListaAsegurad($fNomUsuari,$fPwdClavex){
      
        $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
        $fExcept -> SetUser( $fNomUsuari );
        $fExcept -> SetParams( "Faro", "getListaAsegurad" );
        ...
        try{
            ...
            $mSelect = "SELECT a.cod_tercer,a.abr_tercer
                        FROM ".BASE_DATOS.".tab_tercer_tercer a,
    ...
    function getCosTra($fNomUsuari,$fPwdClavex,$fcodCiuOri,$fcodCiuDes){
      ...
      try{
          ...
          $fReturn['cos_trayec'] = $costTra;
            ...
            $fReturn['msg_respon'] = "No se encontro costo del servicio de acompa�amiento entre las ciudades origen y destino";
    ...
    /*! \fn: setNuevaAsistencia
      ...
      *  \param: fNomUsuari  String  Usuario
      *  \param: fPwdClavex  String  Clave
      *  \param: fTipAsiste  Int Codigo tipo de asistencia 1: Asistencia, 2: Acompa�amiento
      ...
      *  \param: fConVehicu  String Contrase�a usuario gps
      ...
      *  \param: fObsAcompa  String Observacion Solicitud de Acompa�amiento
    ...
    function setNuevaAsistencia(  $fNomUsuari = NULL, $fPwdClavex = NULL, $fTipAsiste = NULL,
                                  ...
                                  $fDirCiuDes = NULL, $fObsAcompa = NULL, $fCiuOrigins = NULL,
                                  $fDirCiuins = NULL, $fFecInst = NULL, $fCiuDesin = NULL,
                                  $fDirCiudesi = NULL, $fFecdesin = NULL, $fNumConte = NULL, 
                                  $fObsCands = NULL,$servicios){
      ...
      $mSelect = "INSERT INTO ".BASE_DATOS.".tab_asiste_carret(
                    ...
                    '".$fObsAcompa."', '".$fCiuOrigins."', '".$fDirCiuins."', 
                    '".$fFecInst."', '".$fCiuDesin."', '".$fDirCiudesi."',
                    '".$fFecdesin."', '".$fNumConte."', '".$fObsCands."',
                    '".$fNomUsuari."',NOW()
        )";
      $fConsult -> ExecuteCons( $mSelect, "R" ) ;
      ...
      // Registra Tarifa de acompa�amiento como servicio
        if($fTipAsiste == 2){
          $mSelect="SELECT a.val_tarifa
          FROM ".BASE_DATOS.".tab_tarifa_acompa a
    ...
    function getInfoSolicitud($fNomUsuari,$fPwdClavex,$fnumSolicit,$fCodClient){
      ...
      $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
      $fExcept -> SetUser( $fNomUsuari );
      $fExcept -> SetParams( "Faro", "getInfoSolicitud" );
      ...
      try{
          ...
          $mSelect = "SELECT *, 
                        ...
                        f.cod_ciudad as 'cod_ciuori', 
                        f.nom_ciudad as 'nom_ciuori', 
                        g.cod_ciudad as 'cod_ciudes', 
                        g.nom_ciudad as 'nom_ciudes',
    ...
    function getDataFormulario($fNomUsuari,$fPwdClavex,$fvalBusqueda){
      ...
      try{
          ...
          $fConsult -> ExecuteCons( $mSelect, "R" ) ;
          if($fConsult -> RetNumRows()>0){
          $mSelect = "SELECT a.cod_campox,a.tex_respue,c.nom_campox,c.ind_tipoxx FROM ".BASE_DATOS.".tab_formul_respue a
    ...
    function getDataImgAsis($fNomUsuari,$fPwdClavex,$fCodSolici,$fCodServic){
      $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
      $fExcept -> SetUser( $fNomUsuari );
      $fExcept -> SetParams( "Faro", "getCosTra" );
      ...
      try{
          ...
          $rut_imagen = 'https://avansatgl.intrared.net/ap/satt_faro/files/asicar/';

          $mSelect = "SELECT cod_solici, cod_servic, CONCAT('".$rut_imagen."', nam_archi1) as 'rut_imagen', des_archiv
                      FROM ".BASE_DATOS.".tab_asiste_eviden
                      WHERE cod_solici='".$fCodSolici."'
                      AND cod_servic='".$fCodServic."';";
          $fConsult -> ExecuteCons( $mSelect, "R" ) ;
          $mDataFor = cleanArray($fConsult -> RetMatrix('a'));
        ...
        catch( Exception $e )
        {
          ...
          else
          {
            ...
            $fExcept -> CatchError( $e -> getCode(), $e -> getMessage(), $e -> getFile(),
                                    $e -> getLine(), $fNomAplica, $fCodSolici );
          }
        }
        return json_encode($fReturn);
    }
    ...
    function setInfoAdicioAsiste($fNomUsuari,$fPwdClavex,$fNumSolici,$fObsSolici,$fCodAuthus){
      
      $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
      $fExcept -> SetUser( $fNomUsuari );
      $fExcept -> SetParams( "Faro", "setInfoAdicioSoliciAsiste" );
      ...
      try{
          ...
          if($fNumSolici!='' && $fObsSolici!=''){
            $mSelect ="INSERT INTO ".BASE_DATOS.".tab_seguim_solasi(
              ...
              VALUES 
            (
                '".$fNumSolici."', '4','".$fObsSolici."',
                '".$fCodAuthus."', NOW()
            )";
            
            if($fConsult -> ExecuteCons( $mSelect, "R" ) != FALSE){
              $fReturn['cod_respon'] = "1000";
              $fReturn['msg_respon'] = "Se Realizo la operacion exitosamente.";
              
            }else{
              $fReturn['cod_respon'] = "2000";
              $fReturn['msg_respon'] = "Hubo un error al guardar la informaci�n.";
            };
          }else{
            $fReturn['cod_respon'] = "2000";
            $fReturn['msg_respon'] = "No se ha enviado la informaci�n correctamente.";
          }
        }
        catch( Exception $e )
        {
    ...
    */
    ...
    /********************************************************************************
    * Funcion que valida si un viaje (Manifiesto) existe, esta acivo y esta en ruta.  *
    * @fn getExisteViaje.                                                        *
    * @brief Inserta una novedad GPS en una aplicacion SAT.                       *
    * @param $fNomUsuari: string Nombre aplicacion.                               *
    * @param $fPwdClavex: integer Numero del despacho.                            *
    * @param $fCodTransp: integer Codigo de la novedad.                           *
    * @param $fNumManifi: string Fecha de la novedad.                             *
    * @param $fNumPlacax: string Descripcion de la novedad.                       *
    **********************************************************************************/
    function getExisteViaje($fNomUsuari = NULL, $fPwdClavex = NULL, $fCodTransp = NULL, $fNumManifi = NULL, $fNumPlacax = NULL, $fNomAplica = NULL)
    {

        $fExcept = new Error( array( "dirlog" => LogDir, "notlog" => TRUE, "logmai" => NotMai ) );
        $fExcept -> SetUser( $fNomUsuari );
        $fExcept -> SetParams( "GL INTEGRADOR", "getExisteViaje" );
        ...
        try
        {
          ...
          if( !getAutentica( $fNomUsuari, $fPwdClavex, $fExcept ) ) {
            ...
            //Busca si el manifiesto y placa esta activo y en ruta
            $fQueryExist = "SELECT a.num_despac 
                            FROM ".BASE_DATOS.".tab_despac_despac a, 
                                 ".BASE_DATOS.".tab_despac_vehige b 
                           WHERE a.num_despac = b.num_despac 
                             AND a.cod_manifi = '".$fNumManifi."' 
                             AND b.cod_transp = '".$fCodTransp."' 
                             AND b.num_placax = '".$fNumPlacax."' 
                             AND b.ind_activo = 'S' 
                             AND a.fec_llegad IS NULL";

            $fConsult -> ExecuteCons( $fQueryExist );
            if( 0 != $fConsult -> RetNumRows() ) {
             $mRetunDespac = $fConsult -> RetMatrix( "a" );
             $fNumDespac = $mRetunDespac[0]["num_despac"];                
            }
            ...
            {              
             throw new Exception( "El viaje ".$fNumManifi." con placa ".$fNumPlacax." no esta en ruta ", "1999" );  
            }              
          
            return "code_resp:1000; msg_resp:El viaje ".$fNumManifi." con placa ".$fNumPlacax." esta disponible para recibir novedades o reportes.";
          }
          catch( Exception $e )
          {
            if( "3001" == $e -> getCode() )
    ...
    * @param $fNumPlacax: string Descripcion de la novedad.                       *
    **********************************************************************************/
    function sendApiNuevoGL($fCodTranps = NULL, $fExcept = NULL, $mData = NULL, $mEndPoint = NULL, $mMethod = 'POST')
    {
      $interfazIntegrador = getInterfaceStanda(59, $fCodTranps, $fExcept);
          if( FALSE !== $interfazIntegrador ){
                $fusrInterf= $interfazIntegrador["NomUsuari"];
                ...
                if($mResponse['cod_respon'] != 1000){ 
                  ...
                  //Arma y organiza la data
                  //Log Muestra de creaci?n de despacho para integrador gps puente.
                  $mLogFile = '/var/www/html/ap/interf/app/faro/logs/'."setNovedadesIntegrador_".date( "Y_m_d" ).".log";
                  $file = fopen( $mLogFile, "a+" );
                  $reg = fwrite( $file, "\n---------------------------------------------------" );
                  $reg = fwrite( $file, "\nExecution Date-Time: ".date( "Y/m/d H:i:s" )."" );
                  $reg = fwrite( $file, "\nData Created: ".var_export($mData,true)."" );
                  $close = fclose( $file );    

                  
                  $mResponse = curl([
                    'Url' => $urlApi."/".$mEndPoint,
                    'Headers' => [
                                  'app-id: 13f5049d-1dec-11eb-9e04-005056013943',
                                  'Authorization: '.$codToken,
                                  'token: '.$token,
...
if( $mNuSoap != true)
{
  try
  {
    ...
    $server -> addFunction( "setNovedadUbicacionGPS" );
    ...
    $server -> addFunction( "getListaAsegurad" );
    ...
    $server -> addFunction( "getExisteViaje" );
