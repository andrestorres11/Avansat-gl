<?php 
  ...
  *******************************************************************************/  
  ...
  $fExcept -> SetUser( 'CronGpsFaro5' );
  $fExcept -> SetParams( "Faro", "Cron para Gps de despacho" );
  $fLogs = array();
  ...
  {    
    ...
    echo $fQueryDespacRuta  = "SELECT g.num_despac, 
                                 d.num_placax,
                           ...
                           WHERE a.num_despac = d.num_despac AND
                                 ...
                                 a.ind_planru = 'S'
                                 GROUP BY 1 ";     
    $db4 -> ExecuteCons( $fQueryDespacRuta );
    $DespachosRutaGps = $db4 -> RetMatrix(  );
    echo "<pre>"; print_r($DespachosRutaGps); echo "</pre>";
    
    if( 0 != $db4 -> RetNumRows() )
    {
      $i = 0;
      echo "<b>Despachos en ruta y con GPS:</b> <br>";
      echo "<BR>";
      foreach($DespachosRutaGps as $fVehiculo)
      { 
        echo "______________________________DESPACHO ".$i."_ N°_".$fVehiculo[0]."____________________<br>";
        ...
        {
          $TOKENX = getToken( $db4, $fVehiculo['usr_gpsxxx'], base64_decode( $fVehiculo['clv_gpsxxx'] ) );
          ...
          echo "\nRespuesta<br>".htmlspecialchars( $str, ENT_QUOTES );
        ...
        elseif(  $fVehiculo['cod_operad'] == '900014002' )
        {
          ...
          try
          {
            ...
            $mParam = array( "user"  => $fVehiculo['usr_gpsxxx'],
                             "pws"   => base64_decode( $fVehiculo['clv_gpsxxx'] ),
                             "placa" => $fVehiculo['num_placax']
                            );
        ...
        echo "</pre>";
        $db4 ->StartTrans();
        echo "Comienza la trasferencia.......<br>";
        /*
        ...
        if( $db4 -> ExecuteCons( $fQueryUpdVehiGps, "R" ) === FALSE )
             throw new Exception( "Error en UPDATE.", "3001" );
        */
        if( isset( $novedaGPS['all_infgps'] ) && $novedaGPS['all_infgps'] !== NULL  && $novedaGPS['val_longit'] !== NULL && $novedaGPS['val_latitu'] !== NULL )
        {
        
          if( $fVehiculo['cod_transp'] == '830096376' )
          {
            $oSoapClient = new soapclient( 'http://www.colombiasoftware.net/base/webservice/reportegps.php?wsdl', array( "trace" => "1", 'encoding'=>'ISO-8859-1' ) );
            $parametros = array( "empresa" => "humadea", 
                               "usuario" => 'OET', 
                               "clave" => 'oet', 
                               "sentido" => 0, 
                               "direccion" => $novedaGPS['det_ubicac'], 
                               "tipo_evento" => 0, 
                               "kilometraje" => 0, 
                               "velocidad" => $novedaGPS['val_veloci'],
                               "novedad" => 0,
                               "longitud" => $novedaGPS['val_longit'],
                               "latitud" => $novedaGPS['val_latitu'],
                               "hora" => date( "H:i:s", strtotime( $novedaGPS['fec_noveda'] ) ),
                               "fecha" => date( "Y-m-d", strtotime( $novedaGPS['fec_noveda'] ) ),
                               "placa" => $fVehiculo['num_placax'],
                               "manifiesto" => $fVehiculo['cod_manifi'] );
                               //"longitud" => $novedaGPS['val_longit'],
                               //"latitud" => $novedaGPS['val_latitu'],
                                                                 
            $mResult = $oSoapClient -> __call( "gps", $parametros );    
          ...
          $parametros = array( "cod_transp" => $fVehiculo['cod_transp'], 
          ...
          if( "1000" != $mCodResp[1] )
          {
            ...
            if( strpos($mMsgResp[1], 'no se encuentra en ruta, o no esta registrado') !== false || strpos($mMsgResp[1], 'Aplicacion no encontrada') !== false || strpos($mMsgResp[1], 'Fecha de la novedad') !== false )
            {
              ...
              if( $db4 -> ExecuteCons( $fQueryDelVehiGps, "R" ) === FALSE )
        ...
        {
          echo"No se pudo obtener reporte GPS";
        }
        $db4 -> Commit();
        echo "________________________________________________________________________________________<br>";
        echo "________________________________FIN DEL DESPACHO ".$i."_________________________________<br><br><br>";
        $i++;
      }
    }
    else
    {
      echo "No hay despachos en ruta con Seguimiento GPS";
    }
  }
  catch( Exception $e )
  {
  ...
  {
    echo $mQuery = "SELECT MAX(cod_consec), num_tokenx, fec_regist 
                 FROM ".BD_STANDA.".t_genera_token 
    ...
    if( !trim($tokenx[0]['num_tokenx']) )
    {
     ...
     echo "<pre>";
      ...
      $mQuery = "INSERT INTO ".BD_STANDA.".t_genera_token ( cod_consec, num_tokenx, fec_regist ) 
