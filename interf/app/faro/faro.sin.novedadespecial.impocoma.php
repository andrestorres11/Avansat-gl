    ...
    function getDestin( $mUser = NULL, $mPass = NULL, $mNumDespac = NULL )
    {
      ...
      try {
        ...
        $mSql = "  SELECT a.nom_destin, c.abr_ciudad, a.dir_destin,
                          ...
                          IF(a.ind_cumdes IS NULL, 1, 0)  AS ind_descar,
    ...
    function setFecAditio( $mUser = NULL, $mPass = NULL, $mNumDespac = NULL, $mTipFecha = NULL, $mNumFecha = NULL, $mNumFactur = NULL)
    {
      ...
      try {
        ...
        {           
            ...
            $mIndCumpli = "SELECT IF(NOW() < (SELECT CONCAT(fec_citdes,' ',hor_citdes)  
                           FROM  ".BASE_DATOS.".tab_despac_destin 
                           WHERE num_despac = '".$mNumDespac."' LIMIT 1),'S','N') AS ind_cumpli";
            ...
            $mCodnoVeda = $mIndCumpli == 'S' ? '256' : '208';
            ...
            $mUpdate = "UPDATE ".BASE_DATOS.".tab_despac_destin 
                           SET  
                                ...
                                fec_cumdes = NOW(),
                                obs_cumdes = 'Reporta llegada cliente',
                                fec_modifi = NOW(),
                                usr_cumdes = '".$mUser."',
                                not_mobile = '1'
                        WHERE 1 = 1 AND num_despac = '".$mNumDespac."' AND num_docume IN ( ".$mNumFactur." ) ";                     
        }

       /* $mUpdate = "UPDATE ".BASE_DATOS.".".((!in_array($mTipFecha,  array('fec_inicar','fec_fincar')) && $filter = " AND num_docume IN ( ".$mNumFactur." ) " && ( $mTipFecha == 'fec_cumdes' && $set = ", ind_cumdes = IF(NOW() < (SELECT CONCAT(fec_citdes,' ',hor_citdes) FROM ".BASE_DATOS.".tab_despac_destin WHERE num_despac = '".$mNumDespac."'),1,0), fec_cumdes = NOW(),nov_cumdes = IF(NOW() < (SELECT CONCAT(fec_citdes,' ',hor_citdes) FROM ".BASE_DATOS.".tab_despac_destin WHERE num_despac = '".$mNumDespac."'),256,208), obs_cumdes = 'Reporta llegada cliente' ") ) ? "tab_despac_destin" : "tab_despac_despac")."
                        SET ".$mTipFecha." = '".$mNumFecha."',
                            usr_modifi = '".$mUser."',
                            fec_modifi = NOW()
                            ".$set."
        ...
        { 
          $fReturn['cod_respon'] = "1000";
          $fReturn['msg_respon'] = "Se Realizo la operacion exitosamente.".$mCodnoVeda;
