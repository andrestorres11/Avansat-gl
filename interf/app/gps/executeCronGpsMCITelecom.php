 ...
 * @brief Cron inserta Novedades de MCI TELECOM                         *
 * @version 0.1                                                         *
...
try {
    ...
    $url = "https://api.3dtracking.net/api/v1.0/";
    

    //Prueba
    /*
    $usrConexion = "cellspa";
    $pasConexion = "password2016";
    $autenticacion = setCurl([
        'Url' => $url."Authentication/UserAuthenticate?UserName=".$usrConexion."&Password=".$pasConexion,
        'Headers' => [
    ...
    if($resp['Result']=='ok'){
        $UserIdGuid = $data['Result']['UserIdGuid'];
        $SessionId = $data['Result']['SessionId'];
        $Uid = "0FAABA";
        $reportData= setCurl([
            'Url' => $url."Units/LatestPositionsList?UserIdGuid=".$UserIdGuid."&SessionId=".$SessionId."&UnitUid=".$Uid,
            'Headers' => [
                        'Content-Type: application/json'
                        ],
            'Verbo' => 'GET',
        ...
        if($resp['Result']=='ok'){
            $registro = $data['Result'][0];
            $Position = $registro['Position'];
            //Organizamos la data
            $novedaGPS['fec_noveda'] = (string) date('Y-m-d H:i:s',strtotime($registro['LastReportedTimeLocal']));
            $novedaGPS['val_veloci'] = (string) $Position['Speed'];
            $novedaGPS['val_longit'] = (string) $Position['Longitude'];
            $novedaGPS['val_latitu'] = (string) $Position['Latitude'];
            $novedaGPS['det_ubicac'] = (string) $Position['Address'];
