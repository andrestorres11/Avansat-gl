...
try {
    ...
    $dateNow = date('Y/m/d');
    $url = "http://chile.latitudtelematics.cl/api/api.php?api=user&key=1E45C685FA9E572EED980895FA71E233&cmd=OBJECT_GET_LOCATIONS,";
    

    if ($db5->RetNumRows() > 0) {
        foreach ($fRecorVehiculos as $fVehiculo) {

            $url_consulta= $url.$fVehiculo['gps_adicio'];
            $json_data = file_get_contents($url_consulta);

            // Decodificar el JSON a un array asociativo
            $data = json_decode($json_data, true);

            // Verificar si la decodificaci�n fue exitosa
            if ($data !== null && isset($data[$fVehiculo['gps_adicio']])) {

                $specific_object = $data[$fVehiculo['gps_adicio']];

                $id = key($data);
                $novedaGPS['fec_noveda'] = (string) date('Y-m-d H:i:s',strtotime($data[$id]['dt_tracker']));
                $novedaGPS['val_veloci'] = (string) $data[$id]['speed'];
                $novedaGPS['val_longit'] = (string) $data[$id]['lng'];
                $novedaGPS['val_latitu'] = (string) $data[$id]['lat'];
                ...
                // Puedes seguir mostrando otros valores de la misma manera
            } else {
                // Manejar el caso en que no se pudo decodificar el JSON
                echo "No se pudo obtener o decodificar la informaci�n JSON.";
            }
