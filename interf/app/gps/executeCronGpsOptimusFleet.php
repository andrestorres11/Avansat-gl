...
try {
    ...
    echo $fQuerySelVehiculos = "SELECT a.cod_operad, a.cod_transp, a.nom_aplica, a.num_despac, " .
        ...
        "AND a.fec_creaci >= DATE_SUB( NOW(), INTERVAL 10 DAY ) " .
    ...
    if ($db5->RetNumRows() != 0) {
        ...
        foreach ($fRecorVehiculos as $fVehiculo) {
            ...
            foreach ($result["data"] as $item) {
                if ($item["code"] === $fVehiculo['num_placax']) {
                    $filteredData[] = $item;
                }
            }
            unset($novedaGPS);
            ...
            $validarInfo = TRUE;
            if($result['result'] === 'Error' || is_null($result)) {
                $validarInfo = FALSE;
            }
            ...
            if ($validarInfo) {
                ...
                if(sizeof($ApiInfo) > 0){
                    ...
                    $mDespac['manifest']         = $info['dispatch_info'][0]['cod_manifi'];
                    $mDespac['plate']            = $fVehiculo['num_placax'];
                    $mDespac['novelty_id']       = '9273';
                    $mDespac['date_novelty']     = date("Y-m-d H:i:s"); 
                    $mDespac['observation']      = $result['message'];
                    ...
                    $mDespac['latitude']         = NULL;
                    $mDespac['longitude']        = NULL;
