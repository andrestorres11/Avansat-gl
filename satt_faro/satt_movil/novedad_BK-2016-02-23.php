	...
	function Registrar()
	{
		...
		$query = "SELECT b.cod_transp, a.cod_manifi, c.ind_estado, 
					...
					FROM 
						 ...
						 tab_despac_seguim c,
						 tab_despac_despac a 
				   LEFT JOIN tab_despac_noveda d ON a.num_despac = d.num_despac 
					     AND d.cod_noveda = '71' 
						 AND d.cod_contro IN ( $inPuestos )
				...
				for ($i=0; $i < $mSizeData; $i++) 
				{
					...
					if( $data )
					{
						...
						if( !$error_ )
						{
							...
							# Query para traer la imagen tomada por la EAL, de la BD
							/*$query = "SELECT bin_fotoxx, bin_fotox2 
                                                  FROM tab_despac_images 
                                                 WHERE num_despac = '".$_POST[$i]["num_despac"]."' AND 
                                                       num_consec = (SELECT MAX(num_consec) 
                                                                       FROM tab_despac_images 
                                                                      WHERE num_despac = '".$_POST[$i]["num_despac"]."'  
                                                                    )  AND
                                                       cod_contro = '".$_POST[$i]["cod_contro"]."'   ";

							$mImage = $this -> conexion -> Consultar( $query, "a" );
							*/
