	...
	function Registrar()
	{
		...
		$query = "SELECT b.cod_transp, a.cod_manifi, c.ind_estado, 
					...
					FROM 
						 tab_despac_vehige b,
						 tab_despac_seguim c,
						 tab_despac_despac a LEFT JOIN tab_despac_noveda d
					  ON a.num_despac = d.num_despac AND
						 d.cod_noveda = '71' AND
						 d.cod_contro IN ( $inPuestos )
				   WHERE b.num_placax = '$_POST[placa]' AND
		...
		$data = $this -> conexion -> Consultar( $query, "a", TRUE );
		...
		$query = "SELECT a.nom_contro
				   ...
				   WHERE a.cod_contro = '".$_POST[0][cod_contro]."'  ";

		$contro = $this -> conexion -> Consultar( $query, "a" );

		if( $data )
		{
			#Inicio IF $data["ind_estado"]
			if( $data[0]["ind_estado"] ) 
			{
				for ($i=0; $i < $mSizeData; $i++) 
				{
					//-------SE SUBE LA FOTO-----------------
					...
					if( $data )
					{
						...
						$query = "SELECT a.url_webser	
								   ...
								   WHERE a.cod_server = b.cod_server AND
										 b.cod_transp = '".$_POST[$i][cod_transp]."'
									 AND a.ind_estado = 1 
						...
						if( !$error_ )
						{
							$query = "SELECT a.cod_manifi, b.num_placax ".
									  ...
									  "WHERE a.num_despac = b.num_despac ".
										"AND a.num_despac = '".$_POST[$i]["num_despac"]."' ";
	...
	{	
		...
		$clv_filtro = "SELECT a.clv_filtro, b.nom_contro, b.cod_contro
						 FROM tab_aplica_filtro_usuari a,
							  tab_genera_contro b
						WHERE a.cod_aplica = '1' AND
							  a.cod_filtro = '7' AND
							  a.cod_usuari = '$cod_usuari' AND
							  a.clv_filtro != 0 AND
							  a.clv_filtro = b.cod_contro ";

		$pc = $this -> conexion -> Consultar( $clv_filtro, "a" );

		if( $pc )
		{
			/*
            *  \brief:  Crea una cadena separada por , 
            *			de los puestos de control hijos padres y el ligado al usuario
            *  \warning: 
            */
			$query = "  (
                            SELECT x.cod_homolo 
                              FROM tab_homolo_pcxeal x 
                             WHERE x.cod_contro = '$pc[clv_filtro]'
                        )
                        UNION
                        (
                            SELECT y.cod_contro 
                              FROM tab_homolo_pcxeal y 
                             WHERE y.cod_homolo = '$pc[clv_filtro]'
                        )  ";	
            $mArrayCodContro = $this -> conexion -> Consultar( $query, "a", TRUE );

            $mCodContros = $pc[clv_filtro];
            foreach ($mArrayCodContro as $row) {
                $mCodContros .= ', '.$row[cod_homolo];
            }

            /*
            *  \brief: Trae los despachos en ruta de la placa y el puesto de control que no tenga novedad OK(EAL) = 71
            *  \warning: 
            */
			...
			$mDespac = $this -> conexion -> Consultar( $query, "a", TRUE );
			
			/*
            *  \brief: Verifica que los despachos traido no tengan novedades en puestos posteriores 
            *  \warning: 
            */
			foreach ($mDespac as $row)
			{
				$mSqlB ="SELECT a.cod_contro 
					 ...
					 INNER JOIN tab_genera_rutcon b 
						  ...
						  WHERE a.num_despac = '{$row[num_despac]}' 
							...
							AND b.val_duraci > (
												 ...
												 INNER JOIN tab_genera_rutcon c 
														 ON d.cod_rutasx = c.cod_rutasx 
														AND d.cod_contro = c.cod_contro 
            ...
            */
            if($data != null)
            {
	            ...
	            $mSql = "SELECT b.des_texto,  b.htm_config, b.ind_requer, 
                     ...
                     INNER JOIN tab_genera_recome b 
		...
		if( $pc )
		{
			...
			<form name="form"  id="form" method="post" action="index.php" enctype="multipart/form-data">
				<table align=center >
					<tr><td align="center" colspan="2" ><h4>ESFERA: <? echo $pc["nom_contro"]; ?></h4></td></tr>
					<tr>
						<td align="right" >
				...
				<input type="hidden" name="cod_contro" value="<? echo $data["0"]["cod_contro"]; ?>" >
				<input type="hidden" name="img_foto01" id="img_foto01ID" value="<? echo $data["img_foto01"]; ?>" >
				<input type="hidden" name="img_foto02" id="img_foto02ID" value="<? echo $data["img_foto02"]; ?>" >
			</form>        
			<script>
