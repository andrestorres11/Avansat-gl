...
class Aplicacion_Seguridad
{
    ...
    //Metodos
    function Aplicacion_Seguridad()//Constructor de la clase
    {
        $this -> codigo = 1;//este c�digo es dado por el administrador de la base de datos de seguridad
        $this -> conexion = new Conexion("localhost", USUARIO, CLAVE, "".BASE_DATOS."");//Se crea la conexi�n a la base de datos
        $this -> principal();
    }
    ...
    function Principal()
    {
        ...
        if(isset($GLOBALS["window"]))
        {
            ...
            //si no se ha elegido donde ejecutar la operaci�n
            $this -> frames();
        }
    ...
    //Con esta funci�n se generan los marcos de al aplicacion.
    function frames()
    {
        echo "<html>
               <head>
      ...
      echo "<link type=\"text/css\" rel=\"stylesheet\" href=\"../".DIR_APLICA_CENTRAL."/estilos/".ESTILO."/css/dhtmlgoodies_calendar.css?random=20051112\" media=\"screen\"></LINK>\n";
        echo "<SCRIPT type=\"text/javascript\" src=\"../".DIR_APLICA_CENTRAL."/js/dhtmlgoodies_calendar.js?random=20060118\"></script>\n";
         ...
         $cod_hijo = $datos_servicio["cod_servic"];
           ...
           if(($GLOBALS["cod_servic"] == "1308" || $GLOBALS["cod_servic"] == "1315" || $GLOBALS["cod_servic"] == "1410" || $GLOBALS["cod_servic"] == "1415" || $GLOBALS["cod_servic"] == "1420")&&($GLOBALS[opcion] == '1'))
           	echo '';
           else
    ...
    //Con esta funci�n se presenta el men�
    ...
    function menuFrame()
    {
        ...
        $fila= $consulta -> ret_arreglo();
          ...
          echo "\n".'  <TABLE WIDTH="100%" CELLPADDING="0" CELLSPACING="0" BORDER="0">
            <TBODY>
              ...
              <TR>
                <TD ALIGN="RIGHT" CLASS="celda_etiqueta">Usuario:</TD><TD ALIGN="LEFT" CLASS="celda_info">'.$datos_usuario[nom_usuari].'</TD>
              </TR>
              <TR>
        ...
        //Si ya se ha desplegado una opci�n del men� se traen de la bd todos los niveles desplegados
        ...
        // Se llama a la funci�n recursiva que publica el men�
...
// Funciones para Boton Limpiar
 ...
 //                  $consulta3 = new Consulta($query3, $this -> conexion);
   ...
   //         $consulta = new Consulta($query, $this -> conexion);
    ...
    //Esta funci�n publica los items del men� desplegados cada nivel.
