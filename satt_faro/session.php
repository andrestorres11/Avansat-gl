...
class session
{
   ...
   //$this -> conexion = new Conexion("web10.intrared.net:3306", USUARIO, CLAVE, "".BASE_DATOS."");//Se crea la conexión a la base de datos
        $this -> conexion = new Conexion("bd10.intrared.net:3306", USUARIO, CLAVE, BASE_DATOS); //Se crea la conexión a la base de datos
        ...
        /*
        echo "<br /><br />usuario_aplicacion :: <br /> "; print_r($this -> usuario_aplicacion);
        echo "<br /><br />datos :: <br />"; print_r($datos_usuario); exit;
        */

        //if(!$this -> usuario_aplicacion -> activo($this -> conexion)){
        if(!$this -> usuario_aplicacion -> activo()){

					echo "<script language=\"JavaScript\" > alert('El Usuario Esta Inactivo')</script>\n";
        ...
        else
        {
          ...
          {
            $queryx = "SELECT CONCAT( `fec_suspen` , ' ', `hor_suspen` ) as timer, NOW() as now
                    FROM ".BASE_DATOS.".tab_genera_suspen a
                   WHERE a.cod_usuari = '".$GLOBALS["usuario"]."' 
                     AND a.ind_suspen = 1 ";

            $consulta = new Consulta($queryx, $this -> conexion);
            $usuari = $consulta -> ret_matriz(); 

            if(count($usuari)>0  ){
              if(strtotime($usuari[0]['now']) > strtotime($usuari[0]['timer'])){
                echo "<script language=\"JavaScript\" > alert('El Servicio ha sido suspendido')</script>\n";
                return $this -> Formulario("");
              } 
            }
            ...
            header( "Location: index.php?".$_SESSION['preload'] );
            return true;
					}
					else
					{
