 ...
 $query = "SELECT a.num_placax,Max(b.num_noveda)
  ...
  for($k = 0; $k < $totalrel; $k++)
  {
	...
	$formulario -> nueva_tabla();
	  ...
	  {
	   if($interf_gps -> getVehiculo($GLOBALS[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]))
		$activo_v = 1;
	   else	$activo_v = 0;
	  ...
	  else if($interf_gps -> ind_modind[$i][0] == CONS_MODIND_IDGPSX)
	  {
	   ...
	   if($activo_v)
	   {
	    $idgps = $interf_gps -> getIdGPS($GLOBALS[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]);
...
*/
 ...
 function Eliminar()
 {
  ...
  for($k = 0; $k < $totalrel; $k++)
  {
   $interf_gps -> Interfaz_GPS_envio($vehire[$k][0],BASE_DATOS,$GLOBALS[usuario],$this -> conexion);

   for($i = 0; $i < $interf_gps -> cant_interf; $i++)
   {
	  if($interf_gps -> getVehiculo($GLOBALS[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]))
	  {
	    if($interf_gps -> EstadoVehiculo($GLOBALS[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0],"0","".CONS_APLICA_BASICO."","".CONS_SERVID_INDICA.""))
...
*/
 if(/*$consulta = new Consulta ("COMMIT", $this -> conexion)*/true)
  {
   $link = "<b><a href=\"index.php?cod_servic=".$this -> servic."&window=central&cod_servic=".$GLOBALS[cod_servic]." \"target=\"centralFrame\">Eliminar Otro Vehiculo</a></b>";

   $mensaje = "El Vehiculo <b>".$GLOBALS[placa]."</b> se Elimino con Exito<br>".$mensaje_gps."<br>".$link;
   $mens = new mensajes();
