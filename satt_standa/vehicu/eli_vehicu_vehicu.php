...
function Buscar()
 ...
 $query = "SELECT a.num_placax,Max(b.num_noveda)
  ...
  if(!$_REQUEST[transp])
  {
   ...
   $consec = new Consulta($query, $this -> conexion);
   $vehire = $consec -> ret_matriz();

   $totalrel = sizeof($vehire);
  }
  else
  {
   $vehire[0][0] = $_REQUEST[transp];
   $totalrel = 1;
  }
  ...
  for($k = 0; $k < $totalrel; $k++)
  {
    $interf_gps -> Interfaz_GPS_envio($vehire[$k][0],BASE_DATOS,$usuario,$this -> conexion);
	...
	$formulario -> nueva_tabla();
	  ...
	  {
	   if($interf_gps -> getVehiculo($_REQUEST[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]))
		$activo_v = 1;
	   else	$activo_v = 0;
	  ...
	  else if($interf_gps -> ind_modind[$i][0] == CONS_MODIND_IDGPSX)
	  {
	   ...
	   if($activo_v)
	   {
	    $idgps = $interf_gps -> getIdGPS($_REQUEST[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]);
...
*/
  ...
  if(!$existdes)
   $formulario -> botoni("Eliminar","if(confirm('Esta Seguro de Eliminar Este Vehiculo.?')){form_vehiculos.submit()}",1);
  else
   $formulario -> linea("Imposible Eliminar Este Vehiculo, Puede Estar Asignado a un Despacho.",1,"e");
  $formulario -> cerrar();
 }//FIN FUNCION

 function Eliminar()
 {
   $query = "DELETE FROM ".BASE_DATOS.".tab_vehicu_vehicu
              WHERE num_placax = '$_REQUEST[placa]'";
  ...
  if(!$_REQUEST[transp])
  {
   ...
   $consulta = new Consulta($query, $this -> conexion);
   $vehirec = $consulta -> ret_matriz();

   $totalrel = sizeof($vehirec);
  }
  else
  {
   $vehire[0][0] = $_REQUEST[transp];
  ...
  for($k = 0; $k < $totalrel; $k++)
  {
   $interf_gps -> Interfaz_GPS_envio($vehire[$k][0],BASE_DATOS,$_REQUEST[usuario],$this -> conexion);

   for($i = 0; $i < $interf_gps -> cant_interf; $i++)
   {
	  if($interf_gps -> getVehiculo($_REQUEST[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0]))
	  {
	    if($interf_gps -> EstadoVehiculo($_REQUEST[placa],$interf_gps -> cod_operad[$i][0],$vehire[$k][0],"0","".CONS_APLICA_BASICO."","".CONS_SERVID_INDICA.""))
...
*/
 if(/*$consulta = new Consulta ("COMMIT", $this -> conexion)*/true)
  {
   $link = "<b><a href=\"index.php?cod_servic=".$this -> servic."&window=central&cod_servic=".$_REQUEST[cod_servic]." \"target=\"centralFrame\">Eliminar Otro Vehiculo</a></b>";

   $mensaje = "El Vehiculo <b>".$_REQUEST[placa]."</b> se Elimino con Exito<br>".$mensaje_gps."<br>".$link;
   $mens = new mensajes();
   $mens -> correcto("ELIMINAR VEHICULO",$mensaje);
  }
 }

}//FIN CLASE eli_vehicu_vehicu
     $proceso = new eli_vehicu_vehicu($this -> conexion, $this -> usuario_aplicacion, $this-> codigo);
?>