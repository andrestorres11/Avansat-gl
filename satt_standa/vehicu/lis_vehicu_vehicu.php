...
class Lis_vehicu_vehicu
{
 ...
 function Resultado()
 {
  ...
  $formulario = new Formulario ("index.php","post","LISTADO DE VEHICULOS","form_item");
    ...
    $_URL  = "../".DIR_APLICA_CENTRAL."/export/exp_recurs_recurs.php?op=2";
    $_URL .= "&url=".DIR_APLICA_CENTRAL;
    $_URL .= "&db=".BASE_DATOS;
    $_URL .= !empty($_REQUEST[transp]) ? "&transp=".$_REQUEST[transp] : '';
    $_URL .= !empty($_REQUEST[por_fecha])  ? "&por_fecha=".$_REQUEST[por_fecha] : '';
    $_URL .= !empty($_REQUEST[marcax])     ? "&marcax=".$_REQUEST[marcax] : '';
    $_URL .= !empty($_REQUEST[carroc])     ? "&carroc=".$_REQUEST[carroc] : '';
    $_URL .= !empty($_REQUEST[colorx])     ? "&colorx=".$_REQUEST[colorx] : '';
    $_URL .= !empty($_REQUEST[config])     ? "&config=".$_REQUEST[config] : '';
    $_URL .= !empty($_REQUEST[por_modelo]) ? "&por_modelo=".$_REQUEST[por_modelo] : '';
    $_URL .= !empty($_REQUEST[mod1]) ? "&mod1=".$_REQUEST[mod1] : '';
    $_URL .= !empty($_REQUEST[mod2]) ? "&mod2=".$_REQUEST[mod2] : '';
    $_URL .= !empty($fecha1) ? "&fecha1=".$fecha1 : '';
    $_URL .= !empty($fecha2) ? "&fecha2=".$fecha2 : '';
    

   $formulario -> linea("Se Encontraron ".sizeof($matriz)." Registros&nbsp;&nbsp;<a target='_blank' href='".$_URL."'>[ Excel ]</a>",1,"t2");
   if($_REQUEST[por_fecha])
    ...
    if($interf_gps -> cant_interf > 0)
    {
     ...
     for($i = 0; $i < $interf_gps -> cant_interf; $i++)
     {
	  if($interf_gps -> ind_modind[$i][0] == CONS_MODIND_PLACAX)
	  {
	   ...
	   if($activo_v)
	    $mensaje = "<br><small><img src=\"../".DIR_APLICA_CENTRAL."/imagenes/ok.gif\"><b>El Veh&iacute;culo Se Encuentra Activado con el Operador GPS ".$interf_gps -> nom_operad[$i][0].".</b></small>";
	   else
...
*/
  ...
  $formulario -> oculto("cod_servic","$_REQUEST[cod_servic]",0);
  $formulario -> oculto("window","central",0);
  $formulario -> botoni("Otro","form_vehiculos.submit()",1);
  $formulario -> cerrar();
 }//FIN FUNCION
// *********************************************************************************
