...
class ajax_genera_tipcto
{
  ...
  */
  private function setRegistros() {
    $mSql = " SELECT  a.cod_tipcto,
                      a.nom_tipcto,
                      a.ind_estado
                FROM  ".BASE_DATOS.".tab_genera_tipcto a";
    ...
    foreach ($mMatriz as $key => $datos) {
    	foreach ($datos as $campo => $valor) {
    		if($campo == "ind_estado"){
    			if($valor == 1){
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_tipcto'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></button>&nbsp;';
          		}else{
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_tipcto'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i></button>&nbsp;';
          		}
          			$html .='<button onclick="formRegistro(\'form\', \'xl\', this.value)" value="'.$datos['cod_tipcto'].'" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></button>';
          		$data[$key][] = $html;	
    		}else{
    			$data[$key][] = $valor;	
    		}
   ...
   /*! \fn: form
   *  \brief: Crea el formulario de para registrar información del Tipo contrato
   *  \author: Ing. Luis Manrique
   *  \date: 27/01/2020
   ...
   */
    function form(){
      try { 
      	//Valida si existe codgi de tipo de contrato para consultarlos o crearlos en vacio
        if($_REQUEST['cod_tipcto'] == ''){
          $mData = $arrayName = array('cod_tipcto' => '', 'nom_tipcto' => '');
        }else{
          $mSql = " SELECT  a.cod_tipcto,
                            a.nom_tipcto
                      FROM  ".BASE_DATOS.".tab_genera_tipcto a
                     WHERE  a.cod_tipcto = ".$_REQUEST['cod_tipcto'];
          $mMatriz = new Consulta($mSql, $this->conexion);
        ...
        //arrays que contienen la información de los campos y los titulos del formulario
          ...
          $mTittle[0]['data'] = array("cod_tipcto" => array(
          													'name' => "Codigo", 
          													'type' => "hidden",
          													'class' => "", 
          													'atribute' => array(
   ...
   */
    function updEst(){
      try {
        ...
        //Geera query
        $mQuery = "UPDATE ".BASE_DATOS.".tab_genera_tipcto 
                      SET ind_estado = $estado,
                          usr_modifi = '".$_SESSION['datos_usuario']['cod_usuari']."',
                          fec_modifi = NOW()
                    WHERE cod_tipcto = ".$_REQUEST['cod_tipcto'];
    ...
    /*! \fn: regForm
   *  \brief: Actualzia o crea registros de tipo de contrato
   *  \author: Ing. Luis Manrique
   *  \date: 28/04/2020
   *  \date modified: dia/mes/año
   *  \param: 
   ...
   */
    function regForm(){
      try {
        //Varibales Necesarias
        $return = [];
        $codigo = empty($_REQUEST['cod_tipcto']) ? '' : "cod_tipcto = ".$_REQUEST['cod_tipcto'].", ";
        //Consulta
	    $mQuery = "INSERT INTO  ".BASE_DATOS.".tab_genera_tipcto 
	                       SET  $codigo
	                            nom_tipcto = '".$_REQUEST['nom_tipcto']."',
	                            ind_estado = 1,
	                            usr_creaci = '".$_SESSION['datos_usuario']['cod_usuari']."',
	                            fec_creaci = NOW()
	   ON DUPLICATE KEY UPDATE 	nom_tipcto = '".$_REQUEST['nom_tipcto']."',
	                            usr_modifi = '".$_SESSION['datos_usuario']['cod_usuari']."',
	                            fec_modifi = NOW()";
	    $consulta = new Consulta($mQuery, $this -> conexion);   
	    if($consulta == true){
...
$proceso = new ajax_genera_tipcto();
 ?>