...
class FacturFaro 
{
    ...
    { 
        ...
        echo '<script>
                document.getElementById("transpID").value='.$GLOBALS["transp"].';
              </script>';
        ...
        $query  = "SELECT a.num_despac AS 'DESPACHO',
                    ...
                    WHERE a.num_despac = b.num_despac 
                      ...
                      AND b.ind_activo = 'S' 
                      AND a.fec_llegad BETWEEN '".$GLOBALS["fecini"]." 00:00:00' AND '".$GLOBALS["fecfin"]." 23:59:59' 
                      ";
        /* Consulta 2
        AND a.fec_llegad >= '".$GLOBALS["fecini"]." 00:00:00' 
        AND a.fec_llegad <= '".$GLOBALS["fecfin"]." 23:59:59' 
        AND (a.cod_conult ='9999' OR a.fec_llegad IS NOT NULL) 
        */
        /* consulta original
        AND b.fec_creaci >= '".$GLOBALS["fecini"]." 00:00:00' 
        AND b.fec_creaci <= '".$GLOBALS["fecfin"]." 23:59:59' 
        */

        if($GLOBALS["transp"])
            $query.= " AND b.cod_transp = '".$GLOBALS["transp"]."'";
            
        if($GLOBALS["cod_agenci"])
            $query.= " AND b.cod_agenci = '".$GLOBALS["cod_agenci"]."'";
        ...
        for($i = 0; $i < sizeof($facturas); $i++)
        {
            ...
            if($empresa != "860068121" && $GLOBALS["transp"] != '860068121'){
              $no_com =array(52,70);
            }else{              
        ...
        $formulario -> nueva_tabla();
        $exp .= "url=".NOM_URL_APLICA."&db=".BASE_DATOS."&fecini=".$GLOBALS["fecini"]."&fecfin=".$GLOBALS["fecfin"]."";
        ...
        *///$formulario -> botoni("Facturar","aceptar()",1);
        ...
        if(sizeof($factur)>0)
        {
            ...
            if($GLOBALS["transp"])
                $sql.= " AND cod_tercer = '".$GLOBALS["transp"]."'";
    ...
    function Facturar()
    {
        ...
        echo "<img src=\"../".DIR_APLICA_CENTRAL."/imagenes/ok.gif\"><b>Se Marcaron Como Facturados los Despacho desde el ".$GLOBALS["fecini"]." hasta el ".$GLOBALS["fecfin"];
