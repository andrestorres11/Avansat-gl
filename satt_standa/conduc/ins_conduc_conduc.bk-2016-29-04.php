          ...
          <?php foreach ($datos->referencias as $key => $value) { ?>
           ...
           <?php } ?>
            ...
            <tr style=" border-left: 2px solid  #8A8683;border-right:2px solid  #8A8683; border-bottom: 2px solid  #8A8683;">
              ...
              <td colspan="2" style="text-align:center"><?php if($datos->principal->dir_ultfot != "NULL" && $datos->principal->dir_ultfot != NULL && $datos->principal->dir_ultfot != ""){ ?><img  width="128px"; height="128px" src="../<?= NOM_URL_APLICA ?>/<?= $datos->principal->dir_ultfot ?>"/> <?php }else { ?> <img src="../<?= DIR_APLICA_CENTRAL ?>/imagenes/silueta.png"/> <?php } ?></td>
          ...
          $mHtml->Row("td");
            ...
            if( $total != 1 ){
              ...
              $mHtml->OpenDiv("id:DatosBasicosID; class:accordion");
                $mHtml->SetBody("<h2 class='fuente'><center>Agregar Conductores</center></h2>");
                $mHtml->OpenDiv("id:sec1;");
                  $mHtml->OpenDiv("id:form1; class:contentAccordionForm");
                    $mHtml->Table("tr");
                        $mHtml->Label("<h3>Transportadora:</h3>", "width:35%; :1;");
              ...
              $mHtml->OpenDiv("id:datos; class:accordion");
                $mHtml->SetBody("<h2 class='fuente'><center>Listado de Conductores</center></h2>");
              ...
              $mHtml->OpenDiv("id:form1; class:contentAccordionForm");
                $mHtml->Table("tr");
                  $mHtml->Label("Tipo de Documento:", "width:25%; *:1;");
                    ...
                    $mHtml->Input(array("type" => "numeric", "name" => "conduc[cod_tercer]", "id" => "cod_tercerID", "onblur"=>"comprobar()", "width" => "10%", "obl" => "1", "minlength" => "6", "maxlength" => "10", "validate" => "numero", "value" =>  $datos->principal->cod_tercer, $disabled, "end" => true));
