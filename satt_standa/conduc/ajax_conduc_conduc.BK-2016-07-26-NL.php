 ...
 ***********************************************************************************/
    ...
    private function registrar(){
      ...
      if(!$indter){
        ...
        $query = "SELECT cod_transp FROM ".BASE_DATOS.".tab_transp_tercer WHERE cod_transp = '$conduc->cod_transp' AND cod_tercer = '$conduc->cod_tercer'";
          ...
          #verifico si esta en la tabla de conductores 
           $sql = "SELECT cod_tercer FROM ".BASE_DATOS.".tab_tercer_conduc WHERE cod_tercer = '$conduc->cod_tercer'";
           $consulta = new Consulta($sql, self::$cConexion);
          $existe2 = $consulta->ret_matriz("a");
          if(!$existe2){
    ...
    ******************************************************************************/
    private function inactivar(){
        $conduc = (object) $_POST['conduc']; //objeto para la tabla tab_tercer_tercer
        $fec_actual = date("Y-m-d H:i:s");
        $usuario = $_SESSION['datos_usuario']['cod_usuari'];
        $conduc->usr_modifi = $usuario;
        $conduc->fec_modifi = $fec_actual;
