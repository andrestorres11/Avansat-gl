...
class conduc {
    ...
    function __construct($co = null, $us = null, $ca = null) {
        ...
        if ($_REQUEST[Ajax] === 'on' || $_REQUEST[Ajax] === 'on') {
    ...
    private function registrar() {
        ...
        $conduc = (object) $_REQUEST['conduc']; #datos principales del conductor
        ...
        if (!$indter) {
            ...
            $empresas = $_REQUEST['empresa']; #arreglo con los nombres de las empresas
            $telefonos = $_REQUEST['telefono']; #arreglo con los numeros de telefono
            $viajes = $_REQUEST['viajes']; #arreglo con los numeros de viajes
            $antiguedades = $_REQUEST['antiguedad']; #arreglo con las antiguedades
            $mercancias = $_REQUEST['mercancia']; #arreglo con las mercancias
            ...
            $query = "SELECT cod_transp 
                        FROM " . BASE_DATOS . ".tab_transp_tercer 
                       WHERE cod_transp = '$conduc->cod_transp' 
                         AND cod_tercer = '$conduc->cod_tercer'";
            $consulta = new Consulta($query, self::$cConexion);
            $existe = $consulta->ret_matriz("a");
    ...
    private function activar() {
        ...
        $conduc = (object) $_REQUEST;
        $fec_actual = date("Y-m-d H:i:s");
        $usuario = $_SESSION['datos_usuario']['cod_usuari'];
        $conduc->usr_modifi = $usuario;
        $conduc->fec_modifi = $fec_actual;
        if (!$conduc->cod_tercer) {
            $conduc->cod_tercer = $_REQUEST['cod_conduc'];
            $conduc->abr_tercer = $_REQUEST['nom_conduc'];
        }
    ...
    private function inactivar() {
        $conduc = (object) $_REQUEST; //objeto para la tabla tab_tercer_tercer
        $fec_actual = date("Y-m-d H:i:s");
        $usuario = $_SESSION['datos_usuario']['cod_usuari'];
        $conduc->usr_modifi = $usuario;
        $conduc->fec_modifi = $fec_actual;
    ...
    private function modificar($operacion = null) {
        ...
        $existConduc = $consulta->ret_matriz("a");
        ...
        if( !$existConduc ) {
            ...
            $query = "INSERT INTO " . BASE_DATOS . ".tab_tercer_conduc(
              ...
              VALUES ('$conduc->cod_tercer','$conduc->cod_tipsex','$conduc->cod_grupsa','$conduc->num_licenc','$conduc->num_catlic','$conduc->fec_venlic',
                      '$conduc->cod_califi','$conduc->nom_epsxxx','$conduc->nom_arpxxx','$conduc->nom_pensio','$conduc->nom_refper','$conduc->tel_refper',
                      '$conduc->cod_operad','$conduc->usr_modifi', '$conduc->fec_modifi')";
        } else {
    ...
    public function getDatosConductor($cod_tercer = '0') {
        ...
        $query = "SELECT a.cod_tercer, a.cod_tipdoc, a.nom_apell1, 
                         a.nom_apell2, a.nom_tercer, a.abr_tercer,
                         a.dir_domici, a.num_telef1, a.num_telef2,
                         a.num_telmov, a.cod_paisxx, a.cod_depart,
                         a.cod_ciudad, a.cod_estado, a.dir_ultfot,
                         a.obs_tercer, b.cod_tipsex, b.cod_grupsa,
                         b.num_licenc, b.num_catlic, b.fec_venlic,
                         b.cod_califi, b.nom_epsxxx, b.nom_arpxxx,
                         b.nom_pensio, b.nom_refper, b.tel_refper,
                         b.cod_operad, 
                         CONCAT( UPPER(c.abr_ciudad), '(', LEFT(d.nom_depart, 4), ') - ', LEFT(e.nom_paisxx, 3) ) abr_ciudad
                 FROM " . BASE_DATOS . ".tab_tercer_tercer a
        ...
        $datos->principal->nom_apell1 = utf8_encode($datos->principal->nom_apell1);
        $datos->principal->nom_apell2 = utf8_encode($datos->principal->nom_apell2);
        $datos->principal->nom_tercer = utf8_encode($datos->principal->nom_tercer);
        $datos->principal->abr_tercer = utf8_encode($datos->principal->abr_tercer);

        #por ultimo consulta todas las referencias laborales del conductor
        $query = "SELECT nom_empre,tel_empre,num_viajes,num_atigue,nom_mercan 
                    FROM " . BASE_DATOS . ".tab_conduc_refere 
