	...
	{	
		...
		$query = "SELECT a.cod_contro, UPPER( a.nom_contro ), UPPER( b.nom_ciudad ), a.nom_encarg,
				  ...
				  WHERE a.ind_estado = '1' AND
						...
						a.cod_contro = '$_REQUEST[cod_contro]' ";
		...
		{
			$puestos = $this -> getPuestos( $_REQUEST[cod_contro] );	
		...
		if( !$listados ) $listados = $_REQUEST[cod_contro];
		...
		$formulario -> oculto( "cod_contro", $_REQUEST[cod_contro] );
		...
		{
			$link = "<a href='index.php?cod_servic=$_REQUEST[cod_servic]&window=central&opcion=2&cod_contro=$row[0]' >$row[0]</a>";
