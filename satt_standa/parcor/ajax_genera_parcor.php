  ...
  private function setRegistros() {
    ...
    foreach ($mMatriz as $key => $datos) {
    	foreach ($datos as $campo => $valor) {
    		if($campo == "id"){
                $html = '<button onclick="delReg(this)" value="'.$datos['id'].'" data-estado="'.$datos['id'].'" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></button>&nbsp;';
          			$html .='<button onclick="formRegistro(\'form\', \'xl\', this.value)" value="'.$datos['id'].'" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></button>';
          		$data[$key][] = $html;	
    		}else{
    			$data[$key][] = $valor;	
    		}
      ...
      try { 
        ...
        if($_REQUEST['cod_regist'] == ''){
          ...
          $mSql = " SELECT  a.dir_emailx,
                            ...
                            a.id
                      FROM  ".BASE_DATOS.".tab_genera_parcor a
                     WHERE  a.id = ".$_REQUEST['cod_regist'];
          $mMatriz = new Consulta($mSql, $this->conexion);
   ...
   */
    function darCampos($datos=""){
      $campos = '
                        ...
                        <input type="hidden" name="correoID" value="'.$datos['id'].'">
        ';
   ...
   */
    function delReg(){
      try {
        ...
        //Geera query
        $mQuery = "DELETE FROM ".BASE_DATOS.".tab_genera_parcor WHERE id = '".$_REQUEST['cod_regist']."';";
