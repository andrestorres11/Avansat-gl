...
//********METODOS
 ...
 function Resultado()
 {
   ...
   else
   {
    ...
    if($matriz[0][6] == '1') $formulario -> caja ("Notificar Agencias:","ind_notage","1",1,1);
		...
		if($matriz[0][19] == '1') $formulario -> caja ("Seguimiento Cargue:","ind_segcar","1",1,1);
		else $formulario -> caja ("Seguimiento Cargue:","ind_segcar","1",0,1);
		
		if($matriz[0][20] == '1') $formulario -> caja ("Seguimiento Transito:","ind_segtra","1",1,1);
		else $formulario -> caja ("Seguimiento Transito:","ind_segtra","1",0,1);
		
		if($matriz[0][21] == '1') $formulario -> caja ("Seguimiento Descargue:","ind_segdes","1",1,1);
		else $formulario -> caja ("Seguimiento Descargue:","ind_segdes","1",0,1);
		
		if($matriz[0][ind_camrut] == '1') $formulario -> caja ("Cambio Plan de Ruta:","ind_camrut","1",1,1);
		else $formulario -> caja ("Cambio Plan de Ruta:","ind_camrut","1",0,1);
    ...
    if($matriz[0][ind_calcon] == '1') $formulario -> caja ("Transmitir Despachos Finalizados y Calificaci&oacute;n de Conductores a la RIT:","ind_calcon","1",1,1);
		else $formulario -> caja ("Transmitir Despachos Finalizados y Calificaci&oacute;n de Conductores a la RIT:","ind_calcon","1",0,1);
		#nuevo campo para sabr si se permite o no duplicar un manifiesto
		if($matriz[0][dup_manifi] == '1') {$val = 1;}else{$val = 0;}
	$formulario -> caja ("Permitir duplicar manifiesto:","dup_manifi","1",$val,1);
	if($matriz[0][ind_biomet] == '1') {$val = 1;}else{$val = 0;}
	$formulario -> caja ("Contratar Biometr&iacute;a:","ind_biomet","1",$val,1);
    /***********************************************************************************************************************************************************************/ 
    
    /**    SECCION TIEMPOS PENDIENTES CARGUE ***********************************************/
    $formulario -> nueva_tabla();
    $formulario -> linea("Configuraci&oacute;n Tiempos Control de Cargue",1,"h");
    //a.,a., a., a. 
    $formulario -> nueva_tabla();
    ...
    $formulario -> texto("Despachos Importaci&oacute;n(Min)", "text", "tie_carimp\" id=\"tie_carimpID\" onChange=\" BlurNumeric(this);", 1, 2, 3, "", $matriz[0][10] );
    $formulario -> texto("Despachos Exportaci&oacute;n(Min)", "text", "tie_carexp\" id=\"tie_carexpID\" onChange=\" BlurNumeric(this);", 1, 2, 3, "", $matriz[0][11] );
    $formulario -> texto("Despachos Tramo 1(Min)", "text", "tie_cartr1\" id=\"tie_cartr1ID\" onChange=\" BlurNumeric(this);", 1, 2, 3, "", $matriz[0][tie_cartr1] );
    $formulario -> texto("Despachos Tramo 2(Min)", "text", "tie_cartr2\" id=\"tie_cartr2ID\" onChange=\" BlurNumeric(this);", 1, 2, 3, "", $matriz[0][tie_cartr2] );
	...
	function Insertar()
 {
  ...
  // die();
   ...
   $GLOBALS[ind_calcon] = $GLOBALS[ind_calcon] == '1' ? '1' : '0';

   #$_REQUEST['ind_excala'] = $_REQUEST['ind_excala'] == '1' ? '1' : '0';
   ...
   $query = "INSERT INTO ".BASE_DATOS.".tab_transp_tipser
             ...
             VALUES ('$nuevo_consec','$cod_transp','$GLOBALS[cod_tipser]', 
                     ...
                     '".$GLOBALS[ind_calcon]."', '".$_REQUEST[ind_segcar]."', '".$_REQUEST[ind_segtra]."', '".$_REQUEST[ind_segdes]."', '".$_REQUEST[val_regist]."', 
