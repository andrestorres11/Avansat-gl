...
class insAreasGescit
{
	...
	public function __construct($co = null, $us = null, $ca = null)
	{
		...
		switch($_REQUEST['Option'])
		{
			...
			break;
			case 'cargar':
				self::cargarArea( $_REQUEST['Option'] );
			break;
	...
	public function formInput()
	{
		try 
		{
			echo "<script src='../" . DIR_APLICA_CENTRAL . "/js/ins_areasx_gescit.js'></script>\n";
			      		...
			      		$mHtml->OpenDiv("id:tabla; class:accordion"); #Div2
				        	$mHtml->SetBody("<h2 style='padding:6px;'><B>Área</B></h2>");
								$mHtml->OpenDiv("id:sec2"); #Div3
									$mHtml->Table('tr');
										$mHtml->Label("* Nombre del área: ", array('for' =>'nom_areaxxID', 'width' => '15%', 'align' => 'right', 'maxlength'=>'50' ) );
										$mHtml->Input( array('name' =>'nom_areaxx', 'width' => '15%', 'align' => 'left', 'end'=>'yes','size' => 90) );
			    ...
			    $mHtml->Row("td");
			        $mHtml->OpenDiv("id:tablaID; class:contentAccordion");
				        $mHtml->OpenDiv("id:tabla; class:accordion");
				        	$mHtml->SetBody("<h2 style='padding:6px;'><B>LISTADO DE ÁREAS</B></h2>");
				        		$mHtml->OpenDiv("id:sec2");
				        			$mHtml->OpenDiv("id:form3; class:contentAccordionForm");
				        				 $mSql = "SELECT cod_areasx, nom_areasx, ind_estado								                          
								                    FROM ".BASE_DATOS.".tab_genera_areasx 
								                    ";
									      ...
									      $list->SetHeader("Área"		, "field:cod_areasx; width:1%;  ");
									      $list->SetHeader("Nombre"		, "field:nom_areasx; width:1%");
									      // $list->SetHeader("Estado"		, "field:ind_estado; width:1%");
									      $list->SetOption("Opciones"	, "field:ind_estado; width:1%; onclikDisable:editarArea( 2, this ); onclikEnable:editarArea( 1, this ); onclikEdit:editarArea( 99, this )" );

									      $list->SetHidden("cod_areasx"	, "0" );
									      $list->SetHidden("nom_areasx"	, "1" );
			...
			$mHtml->Hidden(array( "name" => "cod_areasx", "id" => "cod_areasxID", 'value'=>'nan')); 
			$mHtml->Hidden(array( "name" => "nom_areasx", "id" => "nom_areasxID", 'value'=>'nan')); 
	 ...
	 *  \return:
	 */
	public function cargarArea( $mData = NULL)
	{
		try 
		{
			$mQuery = "SELECT cod_areasx, nom_areasx 
						 FROM ".BASE_DATOS.".tab_genera_areasx 
						WHERE cod_areasx = '{$_REQUEST["cod_areaxx"]}' ";
	...
	{	
		try 
		{
			...
			switch ($mOption) {
				case 'registrar':
					$mQuery = "INSERT INTO 
							  ".BASE_DATOS.".tab_genera_areasx 
							  ( cod_areasx, nom_areasx, ind_estado, usr_creaci, fec_creaci )
							  VALUES
							  ( '', '{$_REQUEST["nom_areaxx"]}', '1', '{$_SESSION["datos_usuario"]["cod_usuari"]}', NOW() )
							  ";

				break;
				case 'activar':
				case 'inactivar': 
					$mQuery = "UPDATE ".BASE_DATOS.".tab_genera_areasx 
							   SET ind_estado = '".($mOption == 'activar' ? '1' : '0')."' 
							   WHERE cod_areasx = '{$_REQUEST["cod_areasx"]}'
							   ";
					
				break;				
				case 'editar':
					$mQuery = "UPDATE ".BASE_DATOS.".tab_genera_areasx 
							   SET nom_areasx = '{$_REQUEST["nom_areaxx"]}' 
							   WHERE 
			...
			if(!new Consulta( $mQuery, self::$cConexion )) {
				$mReturn = ['status' => false, 'message' => 'Error al '.$mOption.' el area: '.$_REQUEST["nom_areaxx"] ];
			}else{
				$mReturn = ['status' => true, 'message' => 'Exito al '.$mOption.' el area: '.$_REQUEST["nom_areaxx"] ];
			}
...
if($_REQUEST['Ajax'] === 'on' )
	$_INFORM = new insAreasGescit();
else
	$_INFORM = new insAreasGescit($this -> conexion, $this -> usuario_aplicacion, $this-> codigo);
