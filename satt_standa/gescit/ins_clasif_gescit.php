...
class insClasifiGescit
{
	...
	public function __construct($co = null, $us = null, $ca = null)
	{
		...
		switch($_REQUEST['Option'])
		{
			...
			break;
			case 'cargar':
				self::cargarClasificacion( $_REQUEST['Option'] );
			break;
	...
	public function formInput()
	{
		try 
		{
			echo "<script src='../" . DIR_APLICA_CENTRAL . "/js/ins_clasif_gescit.js'></script>\n";
				        	...
				        	$mHtml->SetBody("<h2 style='padding:6px;'><B>LISTA DE CLASIFICACIONES</B></h2>");
				        		$mHtml->OpenDiv("id:sec2");
				        			$mHtml->OpenDiv("id:form3; class:contentAccordionForm");
				        				 $mSql = "SELECT cod_clasif, nom_clasif, ind_estado								                          
								                    FROM ".BASE_DATOS.".tab_genera_clasif 
								                    ";
									      ...
									      $list->SetHeader("Clasificación"		, "field:cod_clasif; width:1%;  ");
									      $list->SetHeader("Nombre"		, "field:nom_clasif; width:1%");
									      ...
									      $list->SetHidden("cod_clasif"	, "0" );
									      $list->SetHidden("nom_clasif"	, "1" );
	 ...
	 *  \return:
	 */
	public function cargarClasificacion( $mData = NULL)
	{
		try 
		{
			$mQuery = "SELECT cod_clasif, nom_clasif 
						 FROM ".BASE_DATOS.".tab_genera_clasif 
						WHERE cod_clasif = '{$_REQUEST["cod_clasif"]}' ";
	...
	{	
		try 
		{
			...
			switch ($mOption) {
				case 'registrar':
					$mQuery = "INSERT INTO 
							  ".BASE_DATOS.".tab_genera_clasif 
							  ( cod_clasif, nom_clasif, ind_estado, usr_creaci, fec_creaci )
							  VALUES
							  ( '', '{$_REQUEST["nom_clasif"]}', '1', '{$_SESSION["datos_usuario"]["cod_usuari"]}', NOW() )
							  ";

				break;
				case 'activar':
				case 'inactivar': 
					$mQuery = "UPDATE ".BASE_DATOS.".tab_genera_clasif 
				...
				case 'editar':
					$mQuery = "UPDATE ".BASE_DATOS.".tab_genera_clasif 
							   SET nom_clasif = '{$_REQUEST["nom_clasif"]}' 
							   WHERE 
			...
			if(!new Consulta( $mQuery, self::$cConexion )) {
				$mReturn = ['status' => false, 'message' => 'Error al '.$mOption.' la clasificacion: '.$_REQUEST["nom_clasif"] ];
			}else{
				$mReturn = ['status' => true, 'message' => 'Exito al '.$mOption.' la clasificacion: '.$_REQUEST["nom_clasif"] ];
			}
...
if($_REQUEST['Ajax'] === 'on' )
	$_INFORM = new insClasifiGescit();
else
	$_INFORM = new insClasifiGescit($this -> conexion, $this -> usuario_aplicacion, $this-> codigo);
