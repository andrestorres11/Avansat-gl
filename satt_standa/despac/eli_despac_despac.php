...
class Proc_despac
{
 ...
 function Datos()
 {
   ...
   if($manredes)
   {
    ...
    if($liremdes)
    {
     for($i = 0; $i < sizeof($liremdes); $i++)
     {
      ...
      $formulario -> linea($liremdes[$i][5],0,"i");
	  $formulario -> linea("\$ ".number_format($liremdes[$i][9]).$liremdes[$i][12],0,"i");
	  $formulario -> linea("\$ ".number_format($liremdes[$i][10]),0,"i");
	  $formulario -> linea($liremdes[$i][11],0,"i");
	  $formulario -> linea($liremdes[$i][6],0,"i");
	  $formulario -> linea($liremdes[$i][8],1,"i");
     }
    }
   ...
   $formulario -> nueva_tabla();
   $formulario -> botoni("Eliminar","if(confirm('Esta Seguro de Eliminar Este Despacho.?')){form_item.submit()}",0);
 ...
 function Eliminar()
 {
  $query = "DELETE FROM ".BASE_DATOS.".tab_despac_despac
  ...
  {
   $link_a = "<br><b><a href=\"index.php?&window=central&cod_servic=".$_REQUEST[cod_servic]." \"target=\"centralFrame\">Eliminar Otro Despacho</a></b>";

   $mensaje =  "El Despacho # <b>".$_REQUEST[despac]."</b> Se Elimino con Exito".$link_a;
   $mens = new mensajes();
   $mens -> correcto("ELIMINAR DESPACHOS",$mensaje);
  }
 } 
