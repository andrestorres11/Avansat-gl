...
class Ajax
{
  ...
  function Ajax()
  {
    ...
    switch( $_AJAX["Case"] )
    {
      case 'Insert_Data_Despac_GPS' :
          ...
          $mInsertDataGps = $this -> InsertDataGps( $_AJAX );
          //echo "<pre>"; print_r( $mInsertDataGps); echo "</pre>";
          if( $mInsertDataGps != true)
            $html = "<b><span style='font-size:17px; color:#009900;'>Ha ocurrido un error en la actualización de los datos del GPS para el despacho ".$_AJAX['num_despac']."</span></b>";
          else
          {
           $html = "<b><span style='font-size:17px; color:#009900;'>Se ha Insertado los datos GPS del despacho: ".$_AJAX['num_despac'].", de manera Exitosa.</span></b>
             <br>
             <input type='button' class='crmButton small save' value='Regresar' onclick='location.href=\"?window=central&cod_servic=".$_AJAX['cod_servic']."&num_despac=".$_AJAX['num_despac']."\"' />
             ";
          }
      ...
      case 'Update_Data_Despac_GPS' :
       ...
       $mDataGps = $this -> GetDataDespacGps( $_AJAX['num_despac']);
        ...
        if($mDataGps == NULL)
        $html = "<b><span style='font-size:17px; color:#009900;'>No se ha encontrado el despacho ".$_AJAX['num_despac']."en ruta con GPS</span></b>";
        else
        {
          ...
          if( $mUpdateDataGps != true)
            ...
            $html = "<b><span style='font-size:17px; color:#009900;'>Se ha actualizado los datos del GPS para el Despacho: ".$_AJAX['num_despac'].", de manera Exitosa.</span></b>";
        }
  ...
  {
    $mCodConsec = "SELECT MAX(cod_consec + 1) FROM ".BASE_DATOS.".tab_despac_gpsxxx  ";
    $consulta = new Consulta($mCodConsec, $this -> conexion); 
    $mMaxCons = $consulta -> ret_matriz();     
    
    $mFlag = false;
    $mGpsID = $this -> getOperadGpsReq ();
    for($h = 0; $h <= count($mGpsID); $h ++):
      if( $mData['cod_operad'] == $mGpsID[$h]['cod_operad'] )
        $mFlag = true;
    endfor;
    
    
    if( $mFlag == true )
      $mData['idx_gpsxxx'] = $mData['idx_gpsxxx'];
    else
      $mData['idx_gpsxxx'] = '';
        
    
    $mQuery = "INSERT ".BASE_DATOS.".tab_despac_gpsxxx 
                      ( num_despac, cod_consec, idx_gpsxxx, cod_opegps, nom_usrgps, 
                        clv_usrgps, usr_creaci, fec_creaci) 
               VALUES 
                      ( '".$mData['num_despac']."', '".$mMaxCons[0][0]."'     , '".$mData['idx_gpsxxx']."', '".$mData['cod_operad']."', '".$mData['nom_usuari']."', 
                        '".base64_encode($mData['clv_usrgps'])."', '".$this -> cUsrApl."', NOW())
                      ";
