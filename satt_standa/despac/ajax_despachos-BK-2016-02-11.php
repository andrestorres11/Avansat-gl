	...
	class AjaxDespachos{
		...
		function __construct(){
			...
			switch ($mHost[0]) {
				...
				case 'web10': 	$mBD = "bd10.intrared.net"; break;
			}
		...
		function SaveFechaAdicio( $mData )
		{
			...
			$mUpdate = "UPDATE ".BASE_DATOS.".tab_despac_destin
					     ...
					     WHERE num_despac = '".$mData['num_despac']."'
					  	   AND num_docume IN ( '".str_replace( "|", "','",$mData['num_docume'])."')";
			

			$consulta  = new Consulta($mUpdate, $this -> conexion);
			mail("miguel.romero@intrared.net", "tiempos Adicionales", $mUpdate."<br>". var_export($consulta,true));
 		...
 		/*! \fn: Style
		*  \brief: pinta los estilos del htmk
		...
		*/
		...
		function SetCitaCargue(){
            ...
            if( $_SESSION['datos_usuario']['cod_usuari'] == 'soporte' ){
	            echo "<pre> _REQUEST: ";	print_r($_REQUEST);	echo "</pre>";
	            echo "<pre> regist: ";	print_r($regist);	echo "</pre>";        	
            }
		...
		function SetCitaDescargue(){
			...
			$mSqlU = "UPDATE ".BASE_DATOS.".tab_despac_destin
		  	...
		  	$consulta  = new Consulta($mSqlU, $this -> conexion, "BRC"); 
         	mail("miguel.romero@intrared.net", "grilla de descargue", var_export($mSqlU,true));
		    ...
		    $consulta = new Consulta($query, $this->conexion, "C");
            $protocol = $consulta->ret_matriz();
	                ...
	                # remesa es cuando la factura es igual en los campos num_docume y num_docalt de la misma tabla (tab_despac_destin, tab_despac_cordes)
	       			...
	       			$mListDocumen = "SELECT a.num_dessat AS num_despac, 
									   ...
									   WHERE
											...
											b.num_docume IN ({$_REQUEST[num_docume]})";					 
