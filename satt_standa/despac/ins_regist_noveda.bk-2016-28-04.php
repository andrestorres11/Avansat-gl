...
class Proc_despac
{
	...
	function Formulario()
	{
		...
		if(!$_REQUEST['fecnov']) $_REQUEST['fecnov']= $fec_actual;
		if(!$_REQUEST['hornov']) $_REQUEST['hornov']= $hor_actual;
		if(!$_REQUEST['ind_despac']) $_REQUEST['ind_despac'] = 0;
		...
		echo "<tr>";
            ...
            echo "<td class='celda_titulo' style='padding:4px;' width='50%' >
                    <input class='campo_texto' type='text' value='".$_REQUEST['cod_manifi']."' 
			...
			if( $_REQUEST[placa] != $_REQUEST[placa_a] && $_REQUEST[placa_a] )
			{
      		...
      		$query = "SELECT a.num_despac, a.cod_manifi, 
					  ...
					  WHERE 
							...
							b.num_placax = '".$_REQUEST[placa]."' AND
							(a.cod_conult != '9999' 
