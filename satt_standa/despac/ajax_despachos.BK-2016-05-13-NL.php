        ...
        if ($RESPON[0]["indica"]){
			$mSqlU = "UPDATE ".BASE_DATOS.".tab_despac_destin
				  ...
				  WHERE num_despac = '{$_REQUEST['num_despac']}' 
					...
					AND num_docume IN ({$_REQUEST['num_docum2']}) ";
			mail('fabian.salinas@intrared.net', $_SERVER['HTTP_HOST'].'-Grilla Descargue', var_export($mSqlU, true));
			$consulta  = new Consulta($mSqlU, $this -> conexion);
			...
			if( $_REQUEST['otr_destin'] != '' ){
				...
				mail('fabian.salinas@intrared.net', $_SERVER['HTTP_HOST'].'-Grilla Descargue Multi Destin', var_export($mSqlU, true));
				$consulta  = new Consulta($mSqlU, $this -> conexion); 
			}
        }
