...
function validateInicial(){
    ...
    var validator = $("#InsSolici").validate({ignore: []
    });
...
function InsSoliciValidate() {
    $("#InsSolici").validate({
        ...
        messages: {
            ...
            num_docume: {
                required: "El nÔøΩmero de documento es requerido",
                number: "Solo se aceptan nÔøΩmeros"
            },
            nom_person:{
            ...
            dir_emailx:{
                ...
                email: "El correo no es vÔøΩlido" 
            },
            num_placax:{
                minlength: "Debe tener almenos 6 caracteres",
                maxlength: "Solo es permitido maximo 6 caracteres",
...
function almacenarFase1(){
    ...
    $.ajax({
        ...
        processData: false,
        beforeSend: function() {
            cargando("Guardando la informaciÛn. Por favor espere.");
        },
        success: function(data) {
            swal.close();
            if (data['status'] == 200) {
...
function preguardadoF1(){
    ...
    $.ajax({
        ...
        processData: false,
        beforeSend: function() {
            cargando("Pre Guardando la informaciÛn. Por favor espere.")
        },
        success: function(data) {
            swal.close();
            if (data['status'] == 200) {
...
function rellenaTablas(indicador){
    ...
    var table = $(tabla).DataTable({
        "ajax": {
            "url": "../" + standa + "/estseg/ajax_genera_estseg.php",
            "data": ({ opcion: 'getRegistros', tip_inform: indicador, transportadora: transportadora, estado: estado,
                       fecha_inicio: fecha_inicio, fecha_final: fecha_final, num_solici: num_solici}),
            "type": 'POST'
        },
        'processing': true,
        "deferRender": true,
