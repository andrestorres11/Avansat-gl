    ...
    if (document.getElementById(obj)) {
        if (min >= 0 && max >= 0) {
            ...
            if (!expreg.test(ele) && tam > 0) {
                msg = "Este campo debe contener únicamente letras";
                inc_alerta(obj, msg);
...
function inc_numero(obj, min, max, obl) {
    ...
    if ($('#' + obj)) {
        if (min >= 0 && max >= 0) {
            ...
            if (!expreg.test(ele) && tam > 0) {
                msg = "Este campo debe contener únicamente numeros";
                inc_alerta(obj, msg);
                return 1; //suma al error
            } else {
                var cantidad = inc_cantidad(obj, tam, min, max, obl);
    ...
    if (document.getElementById(obj)) {
        if (min >= 0 && max >= 0) {
            ...
            if (cantidad == true) {
                ...
                if (!expreg.test(ele) && tam > 0) {
                    msg = "Este campo debe contener únicamente numeros y letras";
                    inc_alerta(obj, msg);
                    return 1; //suma al error
                } else {
                    return 0;
    ...
    if (document.getElementById(obj)) {
        if (min >= 0 && max >= 0) {
            ...
            if (cantidad == true) {
                ...
                if (!expreg.test(ele) && tam > 0) {
                    msg = "Este campo contiene caracteres no validos";
                    inc_alerta(obj, msg);
                    return 1; //suma al error
                } else {
                    return 0;
...
function inc_placa(obj, min, max) {
    ...
    if (!expreg2.test(fin) || !expreg.test(inicio) || fin.length != 3) {
        msg = "La placa introducida no es válida";
        inc_alerta(obj, msg);
        return 1;
    }else{
      return 0;
...
function inc_date(obj, min, max, obl) {
    ...
    var tam = ele.length;
    /*
     if(obl==true && (tam<10 || tam>10)){
     
     msg = "Ingrese una fecha valida";
     inc_alerta(obj,msg);
     return 1; //suma al error
     }else if(obl==false && tam>0){
     inc_date(obj,min,max,true)
     }else{
    ...
    /*
     ...
     if(min>= 0 && max>=0){
     console.log("LLego con 10 10"+obj)
     var ele = $.trim($('#'+obj).val());
     ...
     var expreg = /^[0-9]{4}\-(0[1-9]|[12][0-9]|3[01])\-(0[1-9]|[12][0-9]|3[01])/;
     if(!expreg.test(ele) && tam>0){
     msg = "Ingrese una fecha valida";
     inc_alerta(obj,msg);
     return 1; //suma al error
     ...
     var es_mes31 = parseInt(meses31.indexOf(mes));
     var es_mes30 = parseInt(meses30.indexOf(mes));
     
     
     if(es_mes31 >= 0 ){ //si esta
     if(dia>31){
     msg = "Ingrese una fecha valida";
     inc_alerta(obj,msg);
     return 1;
...
function inc_cantidad(obj, tam, min, max, obl) {
    var estado = true;
    var msg = 'Este campo debe contener';
    if (obl == true) {
...
//        }
    ...
    var error = '<span class="inc_alert" style="display:none" id="inc_val_' + obj + '" ' + evento + '>' + msg + '</span>';
    if (elemento == null)
        $('#' + obj).after(error);
    else
        $('#' + obj).html(msg);
