...
function aceptar_ins() {
	try {
		...
		if (formulario.ind_calcon.value == '1') {
			...
			if (num_califi.value == '') {
				alert("Debe seleccionar una calificaci�n para el conductor!");
				return false;
			}
			if (obs_califi.value == '') {
				alert("Debe digitar una observaci�n para el conductor!");
				return false;
			}
		}
...
function SaveAsignaciones() {
	try {
		...
		if (validator) {
			LoadPopupJQ3('close');
			//$("#PopupAsiID").dialog("destroy").remove();
...
function UbicaGPS(num_despac) {
	try {
		...
		console.log("Error Funci�n UbicaGPS: " + e.message + "\nLine: " + Error.lineNumber);
		return false;
	}
}
