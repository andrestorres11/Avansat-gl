 ...
 */
 ...
 /*function getDetails( num_despac, Standa )
 {
   try
   {
     $.ajax({
      type: "POST",
      url: "../"+ Standa +"/desnew/ajax_despac_detail.php",
      data: "num_despac="+num_despac,
      async: false,
      beforeSend: function()
      {
...
$(document).ready(function(){
  ...
  ///------------------------------------------------------------------------------------------------
  $('input:radio[name^=ind_cumdes]').click(function(){
...
/*
  if(NumCheckCarga > 0 ){
    $("input[name^=ind_cliente_cumdes]").each(function(){
  ...
  if( TotalRadio > 0)
  {
    var Obj = $("body").find("tr a span ");
    if(Obj.html()== 'LUGAR ENTREGA' )
    {
      Obj.parent().removeAttr("href");
      Obj.parent().click(function(){
        alert("Primero Debe cumplir las citas de descargue"); 
      }); 
    }
  }
  

    var NumCheckCargue= $("input[name^=ind_cumpli]").length;
    if( NumCheckCargue > 0)
    {
     
       $("body").find("tr a span ").each(function(i, o){
         if(i != 0) {
            
           console.log("a"+$(this));
             $(this).parent().removeAttr("href");
             $(this).parent().click(function(){
               alert("Primero Debe cumplir la cita de cargue"); 
             });
...
function SaveCumpli(){
  ...
  $.ajax({
    url: '../'+dir_aplica.value+'/despac/ajax_despachos.php?option=SetCitaCargue',
    type:'POST',
    async: true,
    data: atributes,
