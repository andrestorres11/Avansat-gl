...
function getInforme(tipo) {
	...
	if (errores == false) {
		...
		if (con) {
			...
			var tip_despac = Array();
			var poseedores = Array();
			var box_checke = $("input[type=checkbox]:checked");
			var x = 0;
			var y = 0;
			box_checke.each(function(i, o) {
				if ($(this).attr("name") == 'multiselect_tip_despacID') {
					tip_despac[x] = $(this).val();
					x++;
				}
			});
			box_checke.each(function(i, o) {
				if ($(this).attr("name") == 'multiselect_cod_poseedID') {
					poseedores[y] = $(this).val();
					y++;
				}
			});

			dataForm += "&tip_despac=" + tip_despac + "&cod_poseed=" + poseedores,
				$.ajax({
...
function getDetalleEal(fec_inicia, fec_finali, ind) {
	...
	if (conn) {
		...
		var tip_transp = $("#tip_transpID").val();
		var tip_despac = Array();
		var poseedores = Array();
		var box_checke = $("input[type=checkbox]:checked");
		var x = 0;
		...
		console.log(tip_despac);
		console.log(poseedores);
		$.ajax({
			url: "../" + standa + "/inform/ajax_inform_inform.php",
			data: "Option=getDetalleEal&Ajax=on&fec_inicia=" + fec_inicia + "&fec_finali=" + fec_finali + "&tipo=" + ind + "&cod_transp=" + cod_transp + "&tip_despac=" + tip_despac + "&num_despac=" + num_despac +"&num_manifi="+cod_manifi+ "&num_viajex=" + num_viajex + "&tip_transp=" + tip_transp + "&cod_poseed=" + poseedores,
			type: "POST",
			async: true,
			beforeSend: function(obj) {
				$("#PopUpID").html('');
...
function verificar() {
	var tip_transp = parseInt($("#tip_transpID").val());
	if (tip_transp == 3) {
		var conn = checkConnection();
		if (conn) {
			var standa = $("#standaID").val();
			...
			if (cod_transp) {
				$.ajax({
					url: "../" + standa + "/inform/ajax_inform_inform.php",
					data: "Option=getPoseedores&Ajax=on&cod_transp=" + cod_transp,
					type: "POST",
					async: true,
					beforeSend: function(obj) {
						$.blockUI({
							...
							title: 'Poseedores',
							draggable: false,
							message: '<center><img src="../' + standa + '/imagenes/ajax-loader2.gif" /><p>Cargano Poseedores</p></center>'
						});
					},
					success: function(data) {
						$.unblockUI();
						$("#poseedores").html("");
						$("#poseedores").append(data);
						$(".multi").multiselect().multiselectfilter();
					}
				});
			} else {
				setTimeout(function() {
					inc_alerta("notify", "Por Favor Primero ingresa una transportadora");
				}, 510);
			}
		} else {
			setTimeout(function() {
		...
		$("#poseedores").html("");
		$("#poseedores").append("<b>NO APLICA</b>");
	}
}