...
function preFunction(){
  ...
  $(':file').change(function(){
      ...
      switch(file.substring(file.lastIndexOf('.') + 1).toLowerCase()){
        case 'jpg' : case 'jpeg': case 'bmp' : case 'tiff' : case 'png' : case 'pdf' : case 'doc' : case 'docx' : case 'xls' : case 'xlsx' : case 'csv' : case 'zip' : case 'rar' :
          //alert("ok");
        break;
        default:
...
function validarKey(min,max,type,campo) {
      ...
      {
        $('#'+campo).focus().after('<span class="error" onclick="cerrarAlert()">Valor minimo no cumple</span>');
        //$('#'+campo).val("");
      }
      if($('#'+campo).val().length>max)
      {
        $('#'+campo).focus().after('<span class="error" onclick="cerrarAlert()">Valor maximo superado</span>');
        $('#'+campo).val("");
      }
      if(type=="num")
      {
          ...
          {
            $('#'+campo).focus().after('<span class="error" onclick="cerrarAlert()">Campo Numerico</span>');
            $('#'+campo).val("");
          }
      }
      if(campo=="num_horlabID")
      {
          ...
          if(hl>24)
          {
            $('#'+campo).focus().after('<span class="error" onclick="cerrarAlert()">Maximo 24 Hrs</span>');
            $('#'+campo).val("");
          }
      }
}
