...
function UpdateNoveda(o, d, c, p, n, f, t, u) {
	try {
		...
		var OldDesc = $("#old_descriID").val();
		var ObsMoti = $("#obs_motivoID").val();
		var bandera = true;

		if( ObsMoti == '' ){
			setTimeout(function(){
				inc_alerta('obs_motivoID', 'Este campo es Obligatorio');
			}, 510);
			bandera = false;
		}

		if( NewNomNoveda == '' && OldDesc == desc ){
			setTimeout(function(){
				inc_alerta('novedadListID', 'Digite una novedad o cambie la Observacion de la novedad')
			}, 510);
			bandera = false;
		}
		...
		param += '&obs_novold=' + OldDesc;
		param += '&obs_motivo=' + ObsMoti;
		...
		$.ajax({
			...
			data: param,
			beforeSend: function() {
				$(".ui-dialog-buttonset").children().next().remove();
			},
			success: function(data) {
				$("#DialogNovedaID").html(data);
				$(".ui-dialog").css({
