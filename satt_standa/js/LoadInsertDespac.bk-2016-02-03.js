...
function ClearAllAccordion()
{
  try
  {
    ...
    var cod_agenci = $( "#cod_agenciID" );
    var fec_despac = $( "#fec_despacID" );
    var cod_ciuori = $( "#cod_ciuoriID" );
    var cod_ciudes = $( "#cod_ciudesID" );
    var cod_rutaxx = $( "#cod_rutaxxID" );
    var cod_tipdes = $( "#cod_tipdesID" );
    var val_declar = $( "#val_declarID" );
    var val_pesoxx = $( "#val_pesoxxID" );
    
    if( cod_manifi.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_manifi.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( cod_agenci.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_agenci.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( fec_despac.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      fec_despac.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( cod_ciuori.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_ciuori.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( cod_ciudes.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_ciudes.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( cod_rutaxx.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_rutaxx.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( cod_tipdes.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      cod_tipdes.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( val_declar.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      val_declar.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( val_pesoxx.val() == '' )
    {
      $( "#accbasID" ).accordion({active:0});
      val_pesoxx.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    // *************************************
    
    // ****** PESTAÑA 2 ********************
    var cod_cenope = $( "#cod_cenopeID" );
    var cod_operad = $( "#cod_operadID" );
    var fec_citcar = $( "#fec_citcarID" );
    var hor_citcar = $( "#hor_citcarID" );
    var sit_cargue = $( "#sit_cargueID" );
    
    // if( cod_cenope.val() == '' )
    // {
      
      // $( "#accopeID" ).accordion({active:0});
      // cod_cenope.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      // return false;
    // }
    // else if( cod_operad.val() == '' )
    // {
      // $( "#accopeID" ).accordion({active:0});
      // cod_operad.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      // return false;
    // }
    // else 
    if( fec_citcar.val() == '' )
    {
      $( "#accopeID" ).accordion({active:0});
      fec_citcar.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( hor_citcar.val() == '' )
    {
      $( "#accopeID" ).accordion({active:0});
      hor_citcar.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( sit_cargue.val() == '' )
    {
      $( "#accopeID" ).accordion({active:0});
      sit_cargue.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    // *************************************
    ...
    if( num_placax.val() == '' )
    {
      $( "#accvehID" ).accordion({active:0});
      num_placax.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else if( $("#cod_remolqID").val() == '')
    {
      $( "#accvehID" ).accordion({active:0});
      $("#cod_remolqID").focus().after("<span id='loading'>-> Campo Obligatorio</span>");
      return false;
    }
    else
    {
      var cod_conduc = $( "#cod_conducID" );
      if( cod_conduc.val() == '' )
      {
        $( "#accvehID" ).accordion({active:0});
        cod_conduc.focus().after("<span id='loading'>-> Campo Obligatorio</span>");
        return false;
      }
      else
      {
        ...
        $.ajax({
          type: "POST",
          url: "../"+ Standa +"/desnew/ajax_desnew_despac.php",
          data: "option=verifyConduc&standa="+Standa+"&cod_tercer="+cod_conduc.val(),
          async: false,
          success: function( datos )
          {
            if( datos == 'no' )
            {
              $( "#accvehID" ).accordion({active:0});
              cod_conduc.focus().after("<span id='loading'>-> El Conductor no Existe</span>");
              return false;
            }
            else
            {
...
function SaveDespacho()
{
  try
  {
    ...
    var cod_transp = $("#transpID").val();
    params += '&cod_transp=' + cod_transp;
    /*******************************************/
    var cod_manifi = $( "#cod_manifiID" ).val();
    params += '&cod_manifi=' + cod_manifi;
    
    var cod_desext = $( "#cod_desextID" ).val();
    params += '&cod_desext=' + cod_desext;
    
    var cod_agenci = $( "#cod_agenciID" ).val();
    params += '&cod_agenci=' + cod_agenci;
    
    var fec_despac = $( "#fec_despacID" ).val();
    params += '&fec_despac=' + fec_despac;
    ...
    var val_declar = $( "#val_declarID" ).val();
    params += '&val_declar=' + val_declar;
    
    var val_pesoxx = $( "#val_pesoxxID" ).val();
    params += '&val_pesoxx=' + val_pesoxx;
    /*******************************************/
    var cod_cenope = $( "#cod_cenopeID" ).val();
    params += '&cod_cenope=' + cod_cenope;
    
    var cod_operad = $( "#cod_operadID" ).val();
    params += '&cod_operad=' + cod_operad;
    
    var fec_citcar = $( "#fec_citcarID" ).val();
    params += '&fec_citcar=' + fec_citcar;
    
    var hor_citcar = $( "#hor_citcarID" ).val();
    params += '&hor_citcar=' + hor_citcar;
    
    var sit_cargue = $( "#sit_cargueID" ).val();
    params += '&sit_cargue=' + sit_cargue;
    /*******************************************/
    var cod_opegps = $( "#cod_opegpsID" ).val();
    params += '&cod_opegps=' + cod_opegps;
    
    var nom_opegps = $( "#cod_opegpsID" ).text();
    params += '&nom_opegps=' + nom_opegps;
    
    var usr_gpsxxx = $( "#usr_gpsxxxID" ).val();
    params += '&usr_gpsxxx=' + usr_gpsxxx;
    
    var clv_gpsxxx = $( "#clv_gpsxxxID" ).val();
    params += '&clv_gpsxxx=' + clv_gpsxxx;
    
    var nom_asegur = $( "#nom_asegurID" ).val();
    params += '&nom_asegur=' + nom_asegur;
    
    var num_poliza = $( "#num_polizaID" ).val();
    params += '&num_poliza=' + num_poliza;
    /*******************************************/
    var num_placax = $( "#num_placaxID" ).val();
    params += '&num_placax=' + num_placax;
    
    var cod_conduc = $( "#cod_conducID" ).val();
    params += '&cod_conduc=' + cod_conduc;
    
    var cod_remolq = $( "#cod_remolqID" ).val();
    params += '&cod_remolq=' + cod_remolq;
    /*******************************************/
    var counter = $( "#counterID" ).val();
    ...
    for( var i = 0; i <= counter; i++ )
    {
      ...
      params += '&num_factur'+i+'=' + num_factur;
      
      num_docalt = $( "#num_docalt"+i+"ID" ).val();
      params += '&num_docalt'+i+'=' + num_docalt;
      
      cod_genera = $( "#cod_genera"+i+"ID" ).val();
      params += '&cod_genera'+i+'=' + cod_genera;
      
      nom_destin = $( "#nom_destin"+i+"ID" ).val();
      params += '&nom_destin'+i+'=' + nom_destin;
      
      cod_ciudad = $( "#cod_ciudad"+i+"ID" ).val();
      params += '&cod_ciudad'+i+'=' + cod_ciudad;
      
      dir_destin = $( "#dir_destin"+i+"ID" ).val();
      params += '&dir_destin'+i+'=' + dir_destin;
      
      nom_contac = $( "#nom_contac"+i+"ID" ).val();
      params += '&nom_contac'+i+'=' + nom_contac;
      
      fec_citdes = $( "#fec_citdes"+i+"ID" ).val();
      params += '&fec_citdes'+i+'=' + fec_citdes;
      
      hor_citcar = $( "#hor_citcar"+i+"ID" ).val();
      params += '&hor_citcar'+i+'=' + hor_citcar;
    }
    /*******************************************/
    //alert( params );
    //resultID
...
function SetVehiculo( num_placax, nom_marcax, nom_lineax, nom_colorx, nom_carroc, num_modelo, num_config, cod_tenedo, cod_conduc, nom_tenedo, nom_conduc, cod_propie, nom_propie )
{
  try
  {
    ...
    $("#nom_codconID").html( '<input type="text" value="'+cod_conduc+'" name="cod_conduc" id="cod_conducID" onchange="ValidateExistConduc( this.value );" size="15" onfocus="this.className=\'campo_texto_on\'" onblur="this.className=\'campo_texto\'" />' );
    ...
    if( num_config.match( expr ) )
    {
      ...
      $("#nom_numremID").html( '<input type="text" name="cod_remolq" id="cod_remolqID" maxlength="7" size="10">' );
    }
    else
    {
      $("#des_numremID").html( "&nbsp;&nbsp;&nbsp;" );
...
function InsertVehiculo()
{
  ...
  else if( !tip_perpro && validatePropietario( des_perpro ) && !tip_perten && validateTenedor( des_perten ) && validateConductor() )
  {
    SaveNewVehiculo();
  }
}
