...
$(function() {
    ...
    var table = $("#contenedor #tablaRegistros").DataTable({
        "ajax": {
            "url": "../" + standa + "/maestr/ajax_genera_tipcta.php",
            "data": ({option:'setRegistros'}),
            "type": 'POST'
        },
        "ajax": "../" + standa + "/maestr/ajax_genera_tipcta.php?option=setRegistros",
        'processing': true,
        "deferRender": true, 
        "autoWidth": false,     
        "search": {
...
function formRegistro(modulo, tam, cod_tipcta = null){
    try {
        var boton = cod_tipcta == null ? 'Crear' : 'Actualizar';
        Swal.fire({
          ...
          confirmButtonText: 'Si, confirmar'
            ...
            if (result.value) {
                $.ajax({
                    url: "../" + standa + "/maestr/ajax_genera_tipcta.php",
                    type: "post",
                    data: ({option: modulo, tam: tam, cod_tipcta:cod_tipcta}),
          ...
          confirmButtonText: 'Si, confirmar'
            ...
            if (result.value) {
                $.ajax({
                    url: "../" + standa + "/maestr/ajax_genera_tipcta.php",
                    type: "post",
                    data: ({option: 'updEst', estado: $(objet).attr('data-estado'), cod_tipcta:$(objet).val()}),
                    dataType: "json",
                    beforeSend: function()
                    {
                        Swal.fire({
