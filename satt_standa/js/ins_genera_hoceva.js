...
$(function() {
    ...
    var table = $("#contenedor #tablaRegistros").DataTable({
        "ajax": {
            "url": "../" + standa + "/maestr/ajax_genera_hoceva.php",
        ...
        fnInitComplete: function() {
            $("#crear").html('<div><a  class="small-box-footer btn btn-success btn-sm" data-toggle="modal" data-target="#regService" aria-controls="tablaRegistros"><span>Crear nuevo servicio</span></a></div><br>');
        }
...
function almacenarDatos() {
    ...
    $.ajax({
        url: "../" + standa + "/maestr/ajax_genera_hoceva.php?" + dataString,
        method: 'POST',
...
function edit(cod_hoceva) {
    var standa = 'satt_standa';
    var opcion = 'edit-forminput';
    var dataString = 'option=' + opcion + '&cod_hoceva=' + cod_hoceva;
    $.ajax({
        url: "../" + standa + "/maestr/ajax_genera_hoceva.php?" + dataString,
        method: 'POST',
        data: '',
...
function updEst(objet) {
    ...
    try {
                ...
                $.ajax({
                    url: "../" + standa + "/maestr/ajax_genera_hoceva.php",
                    type: "post",
                    data: ({ option: 'updEst', estado: $(objet).attr('data-estado'), cod_hoceva: $(objet).val() }),
                    dataType: "json",
                    beforeSend: function() {
                        Swal.fire({
                            title: 'Cargando',
