...
function registrar(accion, DLRow)
{
    try 
    {
        ...
        switch(accion)
        {
            case 'activar': 
            case 'inactivar': 
                cod_areasx = $("input[id^=cod_areasx"+DLRow+"ID]").val();
                nom_areasx = $("input[id^=nom_areasx"+DLRow+"ID]").val();

            break;
            case 'editar': 
                cod_areasx = $("input[id=cod_areasxID]").val();
                nom_areasx = $("input[id=nom_areaxxID]").val();   
            break;            
            case 'registrar': 
                cod_areasx = '0';
                nom_areasx = $('#nom_areaxxID').val();
                if( nom_areasx == '' )
                {
                    return alert('Digite el nombre de la nueva área!');
                }
        ...
        $.ajax({
          url: "../"+ Standa +"/gescit/ins_areasx_gescit.php",
          data : "Ajax=on&Option="+accion+"&Standa="+Standa+"&nom_areaxx="+nom_areasx+"&cod_areasx="+cod_areasx,
          method : 'POST',
...
function confirmar(operacion, data){
    ...
    var area = data.nom_areasx;
    var onclick = "onclick='registrar(\""+operacion+"\", "+data.row+" )'";
    var msj = "<div style='text-align:center'>¿Está seguro de <b>"+operacion+"</b> el área: <b>" +area+ "?</b><br><br><br><br>";
...
function editarArea(tipo, objeto)
{
    try 
    {
        var DLRow = $( objeto ).parent().parent();
        var cod_areasx = DLRow.find("input[id^=cod_areasx]").val();
        var nom_areasx = DLRow.find("input[id^=nom_areasx]").val();
        $("#cod_areasxID").val(cod_areasx);
        $("#nom_areasxID").val(nom_areasx);

        var row = DLRow.find("input[id^=cod_areasx]").attr('name').substr(10, 11); 
        $("#row_dinamiID").val(row);
        //alert(cod_tercer+" - "+nom_transp);
 

        if(tipo == 1){
            confirmar('activar', {'cod_areasx':cod_areasx, 'nom_areasx': nom_areasx, 'row': row } );
        }else if(tipo == 2){
            confirmar('inactivar', {'cod_areasx':cod_areasx, 'nom_areasx': nom_areasx, 'row': row } );
        }else{
            LoadPopupJQNoButton( 'open', 'Confirmar Operación', 'auto', 'auto', false, false, true );
            var popup = $("#popID");
            var area = nom_areasx;
            var msj = "<div style='text-align:center'>¿Está seguro de <b>editar</b> el área: <b>" +area+ "?</b><br><br><br><br>";
                msj += "<input type='button' name='si' id='siID' value='Si' style='cursor:pointer' onclick='cargarArea( "+cod_areasx+", "+row+" )' class='crmButton small save'/> &nbsp;&nbsp;&nbsp;&nbsp";
                msj += "<input type='button' name='no' id='noID' value='No' style='cursor:pointer' onclick='closePopUp()' class='crmButton small save'/><div>";
    ...
    {
        alert("Error en editarArea, mensaje" + e.message+"\nEn la linea: "+e.lineNumber);
    }
}

function cargarArea(cod_areasx, DLRow)
{
    try
    {
        var Standa     = $("#standaID").val();
        ...
        $.ajax({
          url: "../"+ Standa +"/gescit/ins_areasx_gescit.php",
          data : "Ajax=on&Option=cargar&Standa="+Standa+"&cod_areaxx="+cod_areasx,
          method : 'POST',
          success : 
            function ( data ) 
            { 
                ...
                {
                    $("#nom_areasxID, #nom_areaxxID").val(result.data.nom_areasx);
                    $("#cod_areasxID").val(result.data.cod_areasx);
