...
function getInforme() {
	...
	var cod_ciuori = Array();
	var box_checke = $("input[type=checkbox]:checked");
	var x = 0;
	box_checke.each(function(i, o) {
		if ($(this).attr("name") == 'multiselect_cod_ciuoriID') {
			cod_ciuori[x] = $(this).val();
			x++;
		}
	});
	if (errores == false) {
		...
		if (con) {
			...
			dataForm+="&tip_transp="+tip_transp+"&cod_ciuori="+cod_ciuori;
			$.ajax({
...
function getDetalleSalDes(fec_inicia, fec_finali, indicador) {
	...
	if (conn) {
		...
		$.ajax({
			url: "../" + standa + "/inform/ajax_inform_inform.php",
			data: "Option=getDetalleSalDes&Ajax=on&fec_inicia=" + fec_inicia + "&indicador=" + indicador + "&fec_finali=" + fec_finali + "&cod_transp=" + cod_transp + "&tip_despac=" + tip_despac + "&num_despac=" + num_despac + "&num_viajex=" + num_viajex + "&cod_manifi=" + cod_manifi + "&cod_ciuori=" + cod_ciuori + "&tip_transp=" + tip_transp,
			type: "POST",
			async: true,
			beforeSend: function(obj) {
				$("#PopUpID").html('');
...
function getOrigenes() {
	...
	if (conn) {
		...
		if (cod_transp) {
			$.ajax({
				url: "../" + standa + "/inform/ajax_inform_inform.php",
				data: "Option=getOrigenes&Ajax=on&cod_transp=" + cod_transp,
				type: "POST",
				async: true,
				beforeSend: function(obj) {
					$.blockUI({
						...
						title: 'Ciudad Origen',
						draggable: false,
						message: '<center><img src="../' + standa + '/imagenes/ajax-loader2.gif" /><p>Cargando Ciudades de Origen</p></center>'
					});
				},
				success: function(data) {
					$.unblockUI();
					$("#origenes").html('');
					$("#origenes").append(data);
					$(".multi").multiselect().multiselectfilter();
				}
			});
		} else {
			$("#origenes").html('<b>Ingrese Primero una Transportadora</b>');
		}
	} else {
		setTimeout(function() {
			inc_alerta("notify", "Por favor verifica tu conexión a internet");
