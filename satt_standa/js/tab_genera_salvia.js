...
$(function() {
    ...
    var table = $("#contenedor #tablaRegistros").DataTable({
        "ajax": {
            "url": "../" + standa + "/rutas/ajax_genera_salvia.php",
        ...
        fnInitComplete: function() {
            $("#crear").html('<div><a  class="small-box-footer btn btn-success btn-sm" data-toggle="modal" data-target="#regService" aria-controls="tablaRegistros"><span>Crear nueva salida vial</span></a></div><br>');
        }
...
function almacenarDatos() {
    ...
    $.ajax({
        url: "../" + standa + "/rutas/ajax_genera_salvia.php?" + dataString,
        method: 'POST',
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    url: "../" + standa + "/rutas/ajax_genera_salvia.php",
                    type: "post",
                    data: ({ option: 'updEst', estado: $(objet).attr('data-estado'), cod_salvia: $(objet).val() }),
                    dataType: "json",
                    beforeSend: function() {
                        Swal.fire({
                            title: 'Cargando',
