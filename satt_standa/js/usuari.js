...
function mostrarOcultos() {
    ...
    $.ajax({
        type: "POST",
        url: "../" + standa + "/seguridad/ajax_perfil_perfil.php",
        data: "&Ajax=on&Option=getDatosFiltro&standa=" + standa + "&cod_perfil=" + cod_perfil,
        async: true,
        success: function(data) {
            if (parseInt(data) !== 99) {
...
function validarLetras(e) { // 1
    ...
    if(tecla == 241 || tecla == 209){
        alert("La letra �/� no es valida para el Nombre de Usuario");
    }
    patron =/[A-Za-z\s]/; // 4
...
function registrar(ind) {
    ...
    if ($("#ali_usuari").val().length < 1) { 
            setTimeout(function() { 
                inc_alerta("ali_usuari", "Por favor diligenciar.");
            }, 530);
        return false; 
    };
