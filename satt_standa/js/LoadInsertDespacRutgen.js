$('body').ready(function() {
  ...
  $("#nom_transpID").autocomplete({
      source: "../" + standa + "/desnew/ajax_desnew_rutgen.php?option=getTransp&standa=" + standa + "&filter=" + filter,
      minLength: 3,
      delay: 100,
    ...
    if ($("#num_factur" + counter + "ID").val() == '') {
      ...
      $.ajax({
        ...
        success: function(datos) {
          ...
          $("input[name^='nom_destinVal']").autocomplete({
            source: "../" + standa + "/desnew/ajax_desnew_rutgen.php?option=getNomDestin&standa=" + standa,
            minLength: 3,
            delay: 100,
            select: function(event, ui) {
              var postion = $(this).attr("id").substr(-3);
              $(this).val(ui.item);
              $("#nom_destin"+postion).val(ui.item.id);
              $("#dir_destin"+postion).val(ui.item.dir);
              $("#cod_ciudad"+postion+" option[value="+ui.item.ciu+"]").attr("selected", "selected");
              $("body").removeAttr("class");
            }
          });
        }
      ...
      success: function(datos) {
        ...
        if (datos.split('-')[0] == 'yes') {
          ...
          if (radio == 'N') {
            ...
            var est_nom= datos.split('-')[4] != "" ? true : false;
            $("#nom_ter" + tip_tercer + "ID").attr('readonly', est_nom);
            $("#ape_ter" + tip_tercer + "ID").val(datos.split('-')[5]);
            var est_ape= datos.split('-')[5] != "" ? true : false;
            $("#ape_ter" + tip_tercer + "ID").attr('readonly', est_ape);
          }
          $("#cel_ter" + tip_tercer + "ID").val(datos.split('-')[6]);
          var est_cel= datos.split('-')[6] != "" ? true : false;
          $("#cel_ter" + tip_tercer + "ID").attr('readonly', est_cel);
          $("#ema_ter" + tip_tercer + "ID").val(datos.split('-')[7]);
          var est_ema= datos.split('-')[7] != "" ? true : false;
          $("#ema_ter" + tip_tercer + "ID").attr('readonly', est_ema);
        } else {
    ...
    $.ajax({
      ...
      success: function(datos) {
        ...
        if (datos.split('/')[0] == 'yes') {
          ...
          var est_lic= datos.split('/')[1] != "" ? true : false;
          $("#lic_conducID").attr('readonly', est_lic);
          $("#fec_venlicID").val(datos.split('/')[2]);
          var est_veh= datos.split('/')[2] != "" ? true : false;
          $("#fec_venlicID").attr('readonly', est_veh);
        } else {
          $("#lic_conducID").val("");
    ...
    if (conn == true) {
      $.ajax({
        ...
        success: function(datos) {
          ...
          complete: function(){
            $("#sit_cargueVal").autocomplete({
              source: "../" + standa + "/desnew/ajax_desnew_rutgen.php?option=getSitCar&standa=" + standa,
              minLength: 3,
              delay: 100,
              select: function(event, ui) {
                $(this).val(ui.item.value);
                $("#sit_cargue").val(ui.item.id);
                $("body").removeAttr("class");
              }
          });
...
function ClearAllAccordion() {
  try {
    ...
    if (conn == true) {
      ...
      var sit_cargue = $('#sit_cargue');
      if(sit_cargue.val() == '' ){
        
        setTimeout(function() {
            inc_alerta("sit_cargue", "Debe seleccionar un lugar para el sitio de cargue.");
          }, 510);
        val = false;
      }
        ...
        $.ajax({
          ...
          success: function(datos) {
            if (datos == 'no') {
              ...
              if(validaDespacho($("#cod_manifiID")) == true){
                SaveDespacho();
              }
...
function validaDespacho( obj ) {
  try {
    ...
    $.ajax({
      url: "../" + standa + "/desnew/ajax_desnew_rutgen.php",
      data: 'standa=' + standa + '&option=validaViaje&cod_transp=' + cod_transp+'&cod_manifi='+$(obj).val(),
      method: 'POST',
      beforeSend: function() {
        
      },
      success: function(dataRsp) {
        if(dataRsp.status == false)
        {
          alert( dataRsp.message );
          $(obj).focus();
          flag = false;
          return false; 
        }
