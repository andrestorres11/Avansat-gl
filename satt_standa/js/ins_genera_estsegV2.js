...
$(document).ready(function() {
    ...
    $("input[type=file]").on("change", (e) => {
        ...
        const maxSize = 90 * 1024; // Tama�o m�ximo permitido en bytes (90KB)
        ...
        if (!sinLimite && archivo.size > maxSize) {
            alert("El tama�o del archivo es mayor a 90KB. Por favor, elige un archivo m�s peque�o.");
            $(e.target).val('');
            return;
        }
    
        if (!sinLimiteExtension && !extensiones.includes(extension)) {
            alert("Extensi�n NO permitida (Solo se aceptan jpg, png, jpeg)."); 
            $(e.target).val('');
        }
