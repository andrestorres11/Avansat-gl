...
function ajaxEliminarFranja(info) {
    ...
    hor_inicio = moment(info.event.start).format("YYYY-MM-DD HH:mm");
    hor_finalx = moment(info.event.end).format("YYYY-MM-DD HH:mm");
    var standa = "satt_standa"
    var parametros = "Option=EliminarFranja&hor_inicio=" + hor_inicio + "&hor_finalx=" + hor_finalx + "&cod_bahia=" + cod_bahia;
    $.ajax({
        url: "../" + standa + "/config/ajax_agenda_calend.php?" + parametros,
        data: { hor_inicio: hor_inicio, hor_finalx: hor_finalx },
        type: "POST",
        dataType: "json",
        ...
        success: function(data) {
            if (data['status']) {
                Swal.fire({
                    ...
                    text: 'La franja ha sido eliminada correctamente',
                }).then((result) => {
                    if (result.value) {
                        location.reload();
                    }
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se ha podido eliminar la franja, verifique que no tenga pedidos agendando dentro de ella.',
                })
            }
        }
    });
