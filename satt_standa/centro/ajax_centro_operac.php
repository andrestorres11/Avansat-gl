...
class AjaxCentroOperacion
{
  ...
  {  
    ...
    {
      case 'update':
        $mUpdate = "UPDATE ".BASE_DATOS.".tab_genera_cenope 
                       SET nom_cenope = '".$_AJAX['nom_cennot']."', 
                           des_cenope = '".$_AJAX['des_cennot']."',
                           usr_modifi = '".$_SESSION['datos_usuario']['cod_usuari']."',
                           fec_modifi = NOW()
                     WHERE cod_transp = '".trim($_AJAX['transp'])."'
                       AND cod_cenope = '".$_AJAX['notify']."'";
        
        $consulta = new Consulta( $mUpdate, $this -> conexion );
      break;      
      
      case 'delete':
        $mDelete = "DELETE FROM ".BASE_DATOS.".tab_contac_cenope 
                     WHERE cod_transp = '".trim($_AJAX['transp'])."'
                       AND cod_cenope = '".$_AJAX['notify']."'";

        $consulta = new Consulta( $mDelete, $this -> conexion );
        
        $mDelete = "DELETE FROM ".BASE_DATOS.".tab_genera_cenope 
                     WHERE cod_transp = '".trim($_AJAX['transp'])."'
                       AND cod_cenope = '".$_AJAX['notify']."'";
      ...
      case 'insert':
        $mSql = "SELECT MAX( cod_cenope ) FROM ".BASE_DATOS.".tab_genera_cenope WHERE cod_transp = '".trim($_AJAX['transp'])."'";
        $consulta = new Consulta( $mSql, $this -> conexion );
        $num_consec = $consulta -> ret_matriz();
        $consecutivo = $num_consec[0][0] + 1; 
        
        
        $mInsert = "INSERT INTO ".BASE_DATOS.".tab_genera_cenope
                              ( cod_transp, cod_cenope, nom_cenope, 
                                des_cenope, usr_creaci, fec_creaci ) 
  ...
  protected function FormManageNotify( $_AJAX )
  {
    ...
    {
      $mHtml .= '<tr>';
      $mHtml .= '<td align="center" width="20%" style="padding-right:3px; padding-top:15px; font-family:Trebuchet MS, Verdana, Arial; font-size:12px;">Si Realmente desea Eliminar el Operador <<b>'.utf8_encode( $_CENTRO[0]['nom_cenope'] ).'</b>> Haga click en \'CONTINUAR\', de lo Contrario Presione \'CERRAR\'</td>';
      $mHtml .= '</tr>';
    }
    else
    {
      ...
      $mHtml .= '<td align="left" width="80%" style="padding-right:3px; padding-top:15px; font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"><input type="text" id="nom_cennotID" size="50" name="nom_cennot" onblur="ClearForm();" value="'.utf8_encode( $_CENTRO[0]['nom_cenope'] ).'"/></td>';
      $mHtml .= '</tr>';
      ...
      $mHtml .= '<td align="left" width="80%" style="padding-right:3px; padding-top:15px; font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"><textarea id="des_cennotID"  onblur="ClearForm();"  rows="4" cols="47" name="des_cennot" >'.utf8_encode( $_CENTRO[0]['des_cenope'] ).'</textarea></td>';
      $mHtml .= '</tr>';
    }
          $mHtml .= '<tr>';
          $mHtml .= '<td>
                     <input type="hidden" id="notifyID" value="'.$_CENTRO[0]['cod_cenope'].'"/>
  ...
  protected function LoadNotify( $_AJAX )
  {
    ...
    $Message = '<br>Si desea un nuevo Operador haga click <a onclick="Manage(\'insert\', \'0\');" style="text-decoration:none; cursor:pointer;">aqu&iacute;</a></p>';
    
    if( sizeof( $_CENTROS ) > 0 )
    {
     echo "<link rel='stylesheet' href='../" . DIR_APLICA_CENTRAL . "/estilos/informes.css' type='text/css'>\n";
      $mHtml = '<p align="left" style="font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"> A continuaci&oacute;n usted encuentra una lista de todos los Operadores asignados a la Transportadora.';
      $mHtml .= $Message;
      $mHtml .= '
        <script>
          $(function() {
      ...
      {
        $mHtml .= '<li><a href="#centro-'.$centro['cod_cenope'].'">'.utf8_encode( $centro['nom_cenope'] ).'</a></li>';
      }
      $mHtml .= '</ul>';
      
      foreach( $_CENTROS as $centro )
      {
        $mHtml .= '<div id="centro-'.$centro['cod_cenope'].'">';
          $mHtml .= '<div style="overflow: hidden;">';
            ...
            $mHtml .= '<table width="100%" border="0" cellpadding="0" cellspacing="0">';
            $mHtml .= '<tr>';
            $mHtml .= '<td width="100%" style="border-bottom:1px solid #000000; font-family:Trebuchet MS, Verdana, Arial; font-size:15px; font-weight:bold; padding-bottom:5px; color: #285C00;" align="center">'.utf8_encode( $centro['nom_cenope'] ).'</td>';
            $mHtml .= '</tr>';
            ...
            $mHtml .= '<tr>';
            $mHtml .= '<td width="20%" style="text-align:justify; padding-right:3px; padding-top:3px; font-family:Trebuchet MS, Verdana, Arial; font-size:12px;">'.utf8_encode( $centro['des_cenope'] ).'</td>';
            $mHtml .= '</tr>';
            
            $mHtml .= '<tr>';
            $mHtml .= '<td width="20%" style="text-align:right; padding-right:3px; padding-top:3px; font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"><a href="#" style="color:#285C00; text-decoration:none;" onclick="Manage(\'update\', \''.$centro['cod_cenope'].'\');" >Editar</a>     |    <a href="#" style="color:#285C00; text-decoration:none;" onclick="Manage(\'delete\', \''.$centro['cod_cenope'].'\');" >Eliminar</a></td>';
            $mHtml .= '</tr>';
            ...
            $MessCon = '<br>Si desea un nuevo Contacto haga click <a onclick="Contact(\'insert\', \''.$_AJAX['cod_transp'].'\', \''.$centro['cod_cenope'].'\', \'0\');" style="color:#285C00; text-decoration:none; cursor:pointer;">aqu&iacute;</a></p><br></td></tr>';
            $_CONTAC = $this -> getContactos( $_AJAX['cod_transp'],$centro['cod_cenope'] );

            if( sizeof( $_CONTAC ) > 0 )
            {
              $mHtml .= '<tr><td colspan="8"><p align="left" style="font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"> A continuaci&oacute;n usted encuentra una lista de todos los Contactos asigandos al Operador <<b>'.utf8_encode( $centro['nom_cenope'] ).'</b>>.';
              $mHtml .= $MessCon;
              ...
              foreach( $_CONTAC as $contacto )
              {
                ...
                $mHtml .= '<tr>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.utf8_encode( $contacto['nom_contac'] ).'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.utf8_encode( $contacto['car_contac'] ).'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.utf8_encode( $contacto['ema_contac'] ).'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.utf8_encode( $contacto['dir_contac'] ).'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.$contacto['tel_contac'].'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.$tipdes[0]['nom_tipdes'].'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap>'.utf8_encode( $ciudad[0]['nom_ciudad'] ).'</td>';
                $mHtml .= '<td class="'.$style.'" align="center" nowrap><a onclick="Contact(\'update\', \''.$_AJAX['cod_transp'].'\', \''.$centro['cod_cenope'].'\', \''.$contacto['cod_contac'].'\');" href="#" style="color:#285C00; text-decoration:none;">Editar</a> | <a onclick="Contact(\'delete\', \''.$_AJAX['cod_transp'].'\', \''.$centro['cod_cenope'].'\', \''.$contacto['cod_contac'].'\');" href="#" style="color:#285C00; text-decoration:none;">Eliminar</a></td>';
                $mHtml .= '</tr>';
                $i++;
              }
            ...
            {
              $mHtml .= '<tr><td colspan="8"><p align="left" style="font-family:Trebuchet MS, Verdana, Arial; font-size:12px;">El Operador <<b>'.utf8_encode( $centro['nom_cenope'] ).'</b>> no tiene Contactos asignados.';
              $mHtml .= $MessCon;
            }
    ...
    {
      $mHtml = '<p align="left" style="font-family:Trebuchet MS, Verdana, Arial; font-size:12px;"> La transportadora no tiene Operadores asociados.';
      $mHtml .= $Message;
    }
    echo $mHtml;
  }
  ...
  protected function InsertContact( $_AJAX )
  {
    ...
    {
      case 'update':
        echo $mUpdate = "UPDATE ".BASE_DATOS.".tab_contac_cenope
                  ...
                  WHERE cod_transp = '".$_AJAX['transp']."'
                    AND cod_cenope = '".$_AJAX['cod_cennot']."'
      ...
      case 'delete':
        echo $mDelete = "DELETE FROM ".BASE_DATOS.".tab_contac_cenope
                  WHERE cod_transp = '".$_AJAX['transp']."'
                    AND cod_cenope = '".$_AJAX['cod_cennot']."'
                    AND cod_contac = '".$_AJAX['contact_id']."'";
      ...
      case 'insert':
        $mSql = "SELECT MAX( cod_contac ) FROM ".BASE_DATOS.".tab_contac_cenope WHERE cod_transp = '".$_AJAX['transp']."' AND cod_cenope = '".$_AJAX['cod_cennot']."'";
        $consulta = new Consulta( $mSql, $this -> conexion );
        $num_consec = $consulta -> ret_matriz();
        $consecutivo = $num_consec[0][0] + 1; 
        
        
        $mInsert = "INSERT INTO ".BASE_DATOS.".tab_contac_cenope 
                              ( cod_transp, cod_cenope, cod_contac,
  ...
  private function getContactos( $cod_transp, $cod_cennot, $cod_contac = NULL )
  {
    $mQuery = "SELECT cod_contac, nom_contac, car_contac, 
                 ...
                 FROM ".BASE_DATOS.".tab_contac_cenope 
                WHERE cod_transp  = '". $cod_transp ."' 
                  AND cod_cenope = '".$cod_cennot."'";
    if( $cod_contac != NULL )
    {
  ...
  {
    $mQuery = "SELECT cod_cenope, nom_cenope, des_cenope 
                 FROM ".BASE_DATOS.".tab_genera_cenope 
                WHERE cod_transp = '".$cod_transp."'";
    if( $id_notify != NULL )
    {
      $mQuery .= " AND cod_cenope = ".$id_notify;
    }
    $consulta = new Consulta( $mQuery, $this -> conexion );
		return $consulta -> ret_matriz();
  }
...
$proceso = new AjaxCentroOperacion();
 ?>