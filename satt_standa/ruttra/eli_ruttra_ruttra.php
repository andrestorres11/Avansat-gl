<?php
/*! \file: eli_ruttra_ruttra.php
 *  \brief: Elimina la asignacion de rutas a transportadoras
 *  \author: 
 *  \author: 
 *  \version: 1.0
 *  \date: dia/mes/año
...
class Proc_rutas
{
 ...
 function Datos()
 {
   ...
   $formulario -> nueva_tabla();
   $formulario -> linea("Transportadora(s) Relacionada(s) :: Al Seleccionar la Transportadora se Desasignara Automaticamente.",0,"t2");
   ...
   for($i = 0; $i < sizeof($transpors); $i++)
   {
    ...
    $transpors[$i][1] = "<a href=\"index.php?cod_servic=$_REQUEST[cod_servic]&window=central&ruta=".$_REQUEST[ruta]."&trpt=".$transpors[$i][0]."&opcion=4 \"target=\"centralFrame\">".$transpors[$i][1]."</a>";
 ...
 function Eliminar()
 {
  ...
  {
   $link_a = "<br><b><a href=\"index.php?&window=central&cod_servic=".$_REQUEST[cod_servic]." \"target=\"centralFrame\">Desasignar Otra Transportadora</a></b>";

   $mensaje =  "La Transportadora Se Desasigno de la Ruta con Exito".$link_a;
   $mens = new mensajes();
   $mens -> correcto("DESASIGNAR TRANSPORTADORA",$mensaje);
  }
 }
