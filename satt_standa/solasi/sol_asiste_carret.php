...
class SpgPanelControl
{
	...
	function setArrDatSol(){
		...
		*/
        ...
        ";
  		$consulta = new Consulta($sql, $this->cConexion);
		$info = $consulta->ret_matriz('a')[0];

		$sql="SELECT 
	        *,b.nom_ciudad
        FROM 
		  ".BASE_DATOS.".tab_tercer_tercer a
		INNER JOIN ".BASE_DATOS.".tab_genera_ciudad b ON a.cod_paisxx = b.cod_paisxx
		AND b.cod_depart = a.cod_depart AND b.cod_ciudad = a.cod_ciudad
		WHERE a.cod_tercer = '".$info['cod_tercer']."';
		";
		
  		$consulta = new Consulta($sql, $this->cConexion);
		$data = $consulta->ret_matriz('a')[0];
		...
		$this->arr_datsol=array(
			...
			"cod_transp"=>$data['cod_tercer'],
			"nom_transp"=>$data['nom_tercer'],
			"url_direc_aplica"=>defined("URL_DIREC_APLICA") ? URL_DIREC_APLICA : "",
			"server_aplica"=>defined("URL_DIREC_APLICA") ? URL_DIREC_APLICA : "",
			"url_faro"=>defined("URL_INTERF_FAROXX") ? URL_INTERF_FAROXX : "",
			"pwd_logfar"=>defined("PWD_INTERF_FAROXX") ? PWD_INTERF_FAROXX : "",
			"usr_logfar"=>defined("USR_INTERF_FAROXX") ? USR_INTERF_FAROXX : "",
			"nom_aplica"=>defined("BASE_DATOS") ? BASE_DATOS : "",
			//"url_aplica"=>$_SERVER["PHP_SELF"],//url de retorno para respuesta */
			"url_aplica"=>'ctorres/sat-gl-2015/'.$nom_aplica,//Prueba en dev */
	...
	function loadSpg(){
		...
		header("Location: https://central.intrared.net/ap/dev/sol_asistencia_carretera/public/?token=".$this->encrypt(json_encode($this->arr_datsol)));
		//header("Location: http://localhost:8080/Solicitud-Asistencia/public/?token=".$this->encrypt(json_encode($this->arr_datsol)));
	}
