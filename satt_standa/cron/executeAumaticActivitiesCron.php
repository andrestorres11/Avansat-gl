...
class ajaxCalendAgendamiento
{
  ...
  */  
  function viewAgenda(){
      ...
      $fec_anteri = date("Y-m-d",strtotime($fecha_actual."-2 months")); 
      $fec_anteri = $fec_anteri." 00:00:00";
      $mSql = "SELECT a.cod_pedido,b.nit_cliente,
      				  CONCAT ('(',UPPER (a.cod_pedido), ', ', UPPER (b.nombre_cliente) ,')') AS dat_client,
                      a.fec_inicio,
                      a.fec_finalx,
                      a.cod_colorx
                FROM  ".BASE_DATOS.". tab_agenda_pedido a
          INNER JOIN  ".BASE_DATOS.". tab_genera_pedido b
                  ON  a.cod_pedido = b.num_pedido
                  AND a.num_lineax = b.num_lineax
               WHERE  a.fec_inicio >= '".$fec_anteri."'";
/*
     if($filtro -> listar($this -> conexion)>0){
      $datos_filtro = $filtro -> dar_filtro_multiple($this -> conexion);
      $ultima = count($datos_filtro)-1;
      $mSql .= " AND b.nit_cliente IN (";

      foreach($datos_filtro as $key => $value){
        $mSql .= " ".$value['clv_filtro'];
        if($key!=$ultima){
          $mSql .= ", ";  
        }
      }
...
*/
      ...
      $mSql = "SELECT 'Cliente Retira'AS cod_pedido,
                'Intervalo de agendamiento Cliente Retira' AS dat_client,
                      a.fec_inicio,
                      a.fec_finalx,
                      a.cod_bahia,
                    'rgb(241, 196, 27)' AS cod_colorx
                FROM  ".BASE_DATOS.". tab_agenda_cliret a
               WHERE  1";

      $consulta = new Consulta( $mSql, $this -> conexion );
      $clienteRetira = $consulta->ret_matrix('a');
      ...
      foreach ($agendamientos as $key => $value) {
        if($filtro -> listar($this -> conexion)>0){
          $datos_filtro = $filtro -> dar_filtro_multiple($this -> conexion);
          if($this->busquedaArray($datos_filtro,$value["nit_cliente"])){
              $agenda[$key]["title"] = utf8_encode($value["dat_client"]);
          ...
          $agenda[$key]["cliente"] = $value["nit_cliente"];
          $agenda[$key]["id"] = $value["cod_pedido"];
          $agenda[$key]["start"] = $value["fec_inicio"];
      ...
      foreach ($clienteRetira as $key => $value) {
        $agenda[$cont]["id"] = $value["cod_pedido"];
        $agenda[$cont]["title"] = utf8_encode($value["dat_client"]);
        $agenda[$cont]["start"] = $value["fec_inicio"];
        $agenda[$cont]["end"] = $value["fec_finalx"];
        $agenda[$cont]["backgroundColor"] = $value["cod_colorx"];
        $agenda[$cont]["borderColor"] = $value["cod_colorx"];
        $agenda[$cont]["textColor"] = "#ffffff";
        $agenda[$cont]["codBahia"] = $value["cod_bahia"];
        $cont++;
      }
  ...
  */  
  function busquedaHora(){
    ...
    //retorna el horario disponible para agendar.
    $horario = $this->retornaHorarioDisp($fec_inicia,$fec_finalx, $tiempo_cargue, $bahia);
    $hora_inicia = $horario['fec_inicar'];
    $hora_finalx = $horario['fec_fincar'];
    $validate = $horario['validate'];
    
    $respuesta['estado']=2;
    if($validate){

    $hor_inicar = date('H:i:s',strtotime($hora_inicia));
    $hor_fincar = date('H:i:s',strtotime($hora_finalx)); 

    //segunda validacion
    $v1=$this->verifica_rango($hor_inicia,$hor_finalx,$hor_inicar);
    $v2=$this->verifica_rango($hor_inicia,$hor_finalx,$hor_fincar);
    ...
    //validacion horario de la bahia
    $v3 = $this->verifica_rango($hor_inibah, $hor_finbah, $hor_inicar);
    $v4 = $this->verifica_rango($hor_inibah, $hor_finbah, $hor_fincar);
    if($v3 && $v4){
      $respuesta['estado']=0;
      $respuesta['comparacion_prueba']=$hora_finalx;
      if($v1 && $v2){
        $respuesta['estado']=1;
        $respuesta['inicio']=$hora_inicia;
        $respuesta['final']=$hora_finalx;
        $respuesta['tiecar']=$tiempo_cargue;
        $respuesta['dia']=$fec_dia;
  ...
  */  
  function registraCitacion(){
    ...
    //retorna el horario disponible para agendar.
    $horario = $this->retornaHorarioDisp($fec_inicia,$fec_finalx, $tiempo_cargue, $bahia);
    //Fechas disponible para agendar
    $fec_disini = $horario['fec_inicar'];
    $fec_disfin = $horario['fec_fincar'];
    $validate = $horario['validate'];

    if($validate){

        $hor_inicar = date('H:i:s',strtotime($fec_disini));
        $hor_fincar = date('H:i:s',strtotime($fec_disfin)); 
        //segunda validacion
        $v1=$this->verifica_rango($hor_inicia,$hor_finalx,$hor_inicar);
        $v2=$this->verifica_rango($hor_inicia,$hor_finalx,$hor_fincar);
        ...
        if($v3 && $v4){
          $respuesta['estado']=0;
          if($v1 && $v2){
            $this->registraCita($fec_dia, $fec_disini, $fec_disfin,$num_pedido,$num_lineax,$bahia);
            $respuesta['estado']=1;
          }
        }
    }
  ...
  *  \return n/a
  */  
  function registraCita($fecha, $fec_inicial, $fec_finalx,$num_pedido,$num_lineax,$bahia){

    $sql="INSERT INTO ".BASE_DATOS.".tab_agenda_pedido 
            (cod_pedido, num_lineax,fec_inicio, fec_finalx, tip_pedido,
            cod_colorx, usr_creaci, fec_creaci)
          VALUES  ('".$num_pedido."', '".$num_lineax."','".$fec_inicial."', '".$fec_finalx."', 1,
     'rgb(0,131,38)', '".$_SESSION['datos_usuario']['cod_usuari']."', NOW())";
    ...
    $hora_inicial = date("H:i:s",strtotime($fec_inicial));
    $hora_finalx = date("H:i:s",strtotime($fec_finalx));
  ...
  */ 
  ...
  function verifica_rango($date_inicio, $date_fin, $date) {
    // Your code here!
  ...
  *  \return hora libre
  */ 
  function retornaHorarioDisp($fec_inicia,$fec_finalx, $tiempo, $bahia){
    $tiempo_maximo =  date("Y-m-d H:i:s",strtotime( '+60 minute' , strtotime (date("Y-m-d H:i:s"))));

    $ini = strtotime($fec_inicia);
    $fin = strtotime($fec_finalx);
    $dia = date("Y-m-d",strtotime($fec_inicia));
    $datos = array();
    //tiempo maximo de agendamiento. Para que no se salga del rango
    $max = strtotime( '-'.$tiempo.' minute' , strtotime ($fec_finalx));
    //Registros de las fechas del dia.
    $mSql = "SELECT 
              b.fec_inicio, 
              b.fec_finalx 
            FROM 
              ".BASE_DATOS.".tab_genera_pedido a 
              INNER JOIN ".BASE_DATOS.".tab_agenda_pedido b ON b.cod_pedido = a.num_pedido 
              AND b.num_lineax = a.num_lineax 
            WHERE 
              a.bahia_cargue = '".$bahia."' 
              AND a.fec_cargue = '".$dia."';";
    
    $consulta = new Consulta( $mSql, $this->conexion );
    $registros = $consulta->ret_matriz();
    $ran_inicio = '';
    $ran_finalx = '';
    $min_ocupados = $this->minutosOcupados($registros);
    $prueba = array();
    $disponibilidad = array();
    $c = 0;
    $flag = false;
    for($i=$ini; $i<=$max; $i+=60){
      $ran_inicio = date("Y-m-d H:i:s", $i);
      $ran_finalx = date("Y-m-d H:i:s", strtotime ( '+'.$tiempo.' minute' , strtotime ($ran_inicio)));
      $inix = strtotime($ran_inicio);
      $finx = strtotime($ran_finalx);
      $validad = true;
      $x = 0;
      $disponibilidad[$c]['inicio'] = $ran_inicio;
      $disponibilidad[$c]['final'] = $ran_finalx;
      $disponibilidad[$c]['valido'] = true;
      for($j=$inix;$j<=$finx;$j+=60){
        $min_busqueda = date("Y-m-d H:i:s", $j);
        if(in_array($min_busqueda, $min_ocupados) OR ($tiempo_maximo >= $min_busqueda)){
          $validad = false;
          $prueba[$c][$x]['encontrado'] = $min_busqueda;
          $disponibilidad[$c]['valido'] = false;
        }
        $x++;
      }
      /*
    ...
    //almacena retorna el primera rango valido para agendamiento
    $rangevalido = $this->retornaValidaRango($disponibilidad);
    $datos['fec_inicar'] = $rangevalido['inicio'];
    $datos['fec_fincar'] = $rangevalido['final'];
    $datos['validate'] = $rangevalido['valido'];
    return $datos;
  }

  function retornaValidaRango($ranges){
  ...
  function minutosOcupados($registros){
    $ocupados = array();
    foreach($registros as $key=>$registro){
      $ini = strtotime($registro['fec_inicio']);
      $fin = strtotime($registro['fec_finalx']);
      for($i=$ini; $i<=$fin; $i+=60){
        $minuto = date("Y-m-d H:i:s", $i);
        if(!in_array($minuto, $ocupados)){
          $ocupados[] = $minuto;
        }
      }
    }
