...
class Solici_solici
{
	...
	function Main()
	{
		echo "<hr>".get_class($this)."::Main > <hr>";
		exit;
...
png, jpeg, zip, rar)
	...
	*/
	...
	function checkTableInterfParame($table){
		//campos minimos requeridos
		$requiredField=array("cod_operad","cod_transp","nom_operad","nom_usuari","clv_usuari","ind_estado","url_webser");
		$sql='desc '.BASE_DATOS.'.'.$table.';';
		$consulta = new Consulta( $sql, $this->conexion );
		$fields = $consulta->ret_matriz( 'a' );
		$nf=0;
		if(!empty($fields) && is_array($fields) && sizeof($requiredField)>0 && sizeof($fields)>0){
			foreach ($fields as $k => $v) {
				if(in_array($v["Field"], $requiredField)){
					$nf++;
				}
			}
			if($nf!=sizeof($requiredField)){
				return false;
			}
		}else{
			return false;
	...
	function setInterfParame(){		
		...
		if(sizeof($rtbn)>0){
			foreach($rtbn as $c => $table){
				if($this->checkTableInterfParame($table)){
	...
	function onCreateForm(){
		if($this->validateInterfParame()){
			...
			if(!file_exists($filejs)){
				$filejs="../js/mod_solici_solici.js";
				$filecss="../estilos/mod_solici_solici.css";
				$dirsolifa='../solifa/';
			}
