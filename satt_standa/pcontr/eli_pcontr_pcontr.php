...
/*   $interfaz = new Interfaz_SAT(BASE_DATOS,NIT_TRANSPOR,$this -> usuario_aplicacion,$this -> conexion);
   ...
   for($i = 0; $i < $interfaz -> totalact; $i++)
   {
    ...
    $homolo = $interfaz -> getHomoloTranspPC($interfaz -> interfaz[$i]["operad"],$interfaz -> interfaz[$i]["usuari"],$interfaz -> interfaz[$i]["passwo"],$_REQUEST[contro]);
...
*//*
   //Manejo de la Interfaz GPS
    ...
    if($interf_gps -> cant_interf > 0)
    {
	    ...
	    $zona_homolo = $interf_gps -> getHomozonasxx($interf_gps -> cod_operad[$i][0],NIT_TRANSPOR,$_REQUEST[contro]);
...
/*  $interfaz = new Interfaz_SAT(BASE_DATOS,NIT_TRANSPOR,$usuario,$this -> conexion);
  ...
  for($i = 0; $i < $interfaz -> totalact; $i++)
  {
   ...
   if($homolo["PCHomolo"] != -2)
   {
    $resultado_sat = $interfaz -> eliHomoloPC($interfaz -> interfaz[$i]["operad"],$interfaz -> interfaz[$i]["usuari"],$interfaz -> interfaz[$i]["passwo"],$_REQUEST[contro],$homolo["PCHomolo"]);
...
*/
  ...
  {
   $link = "<b><a href=\"index.php?cod_servic=".$this -> servic."&window=central&cod_servic=".$_REQUEST[cod_servic]." \"target=\"centralFrame\">Eliminar Otro Puesto de Control</a></b>";

   $mensaje = "El Puesto de Control <b>".$_REQUEST[nombre]."</b> se Elimino con Exito<br>".$link;
   $mens = new mensajes();
