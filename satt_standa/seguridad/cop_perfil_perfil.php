...
class Proc_perfil
{
    ...
    function Formulario1()
    {
        ...
        $formulario = new Formulario("index.php", "post", "COPIAR PERFIL", "form_princi");
        $formulario->nueva_tabla();
        ...
        $formulario->oculto("cod_perfil_nue", $consecut[0][0], 0);
        $formulario->oculto("cod_servic", $_REQUEST["cod_servic"], 0);
    ...
    function Formulario2()
    {
        ...
        $formulario = new Formulario("index.php", "post", "COPIAR PERFILES", "form_ins");
        ...
        $formulario->nueva_tabla();
        $formulario->linea("Codigo", 0, "t");
        $formulario->linea($_REQUEST[cod_perfil_nue], 1, "i");
        ...
        for ($i = 0; $i < sizeof($matriz); $i++)
        {
            ...
            if ($matriz[$i][2])
            {
                ...
                if ($matriz[$i][0] != COD_FILTRO_EMPTRA && $matriz[$i][0] != COD_FILTRO_CONTRO && $matriz[$i][0] != COD_FILTRO_ASEGUR && $matriz[$i][1] != 'Etapa' )
        ...
        $formulario->oculto("opcion", $_REQUEST[opcion], 0);
        $formulario->oculto("cod_servic", $_REQUEST["cod_servic"], 0);
        //$formulario->oculto("cod_perfil", $_REQUEST[cod_perfil_nue], 0);
        $formulario->oculto("cod_perfil", $_POST['cod_perfil_nue'], 0);
        $formulario->oculto("cod_perfil_nue", $_POST['cod_perfil_nue'], 0);
    ...
    function Actualizar()
    {
        ...
        $query = "INSERT INTO " . BASE_DATOS . ".tab_genera_perfil
   						 (cod_perfil,nom_perfil,usr_creaci,fec_creaci)
        ...
        {
            $link_a = "<br><b><a href=\"index.php?&window=central&cod_servic=" . $_REQUEST[cod_servic] . " \"target=\"centralFrame\">Copiar Otro Perfil</a></b>";

            $mensaje = "El Perfil <b>" . $_REQUEST[nom] . "</b> Se Inserto con Exito" . $link_a;
            $mens = new mensajes();
            $mens->correcto("COPIAR PERFILES", $mensaje);
        }
    }
