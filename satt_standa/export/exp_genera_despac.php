...
";
    ...
    function expListadoExcel($archivo, $desenrutafinal, $porllegadfinal, $ind_enrutas, $ind_porlleg, $alarmas, $manredes, $desurb) {
        ...
        $formulario->linea("Manifiesto", 0, "i");
        $formulario->linea("No. Viaje", 0, "i");
        $formulario->linea("No. Solicitud", 0, "i");
        if ($manredes) {
        ...
        for ($i = 0; $i < sizeof($desenrutafinal); $i++) {
            $mDataExt = self::getDataExtra( $desenrutafinal[$i][0] );
            ...
            $formulario->linea($mDataExt['num_desext'], 0, "i");
            $formulario->linea($mDataExt['num_solici'], 0, "i");
            if ($manredes) {
        ...
        if ($ind_porlleg && !$_REQUEST[des_retras]) {
            ...
            for ($i = 0; $i < sizeof($porllegadfinal); $i++) {
                $mDataExt = self::getDataExtra( $porllegadfinal[$i][0] );

                if ($porllegadfinal[$i][12]) {
                    echo "<td bgcolor=\"#99CC99\">" . $porllegadfinal[$i][0] . "</td>";
                    echo "<td bgcolor=\"#99CC99\">" . $porllegadfinal[$i][1] . "</td>";
                    echo "<td bgcolor=\"#99CC99\">$mDataExt[num_desext]</td>";
                    echo "<td bgcolor=\"#99CC99\">$mDataExt[num_solici]</td>";
                    if ($manredes) {
     ...
     *  \return: Array
     */
    private function getDataExtra( $mNumDespac ){
        $mSql = "SELECT a.num_desext, a.num_solici 
                   FROM ".BASE_DATOS.".tab_despac_sisext a 
                  WHERE a.num_despac = '$mNumDespac' ";
        $mConsul = new Consulta($mSql, $this->conexion);
        $mResult = $mConsul->ret_matrix('a');
        return $mResult[0];
    }
}
