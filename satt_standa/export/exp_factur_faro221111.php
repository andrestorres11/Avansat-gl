...
class Proc_Factur_Faro
{
 ...
 {ini_set('memory_limit','128M');
  ...
  $query  = "SELECT a.num_despac AS 'DESPACHO',a.fec_salida AS 'FECHA DE SALIDA',  d.nom_ciudad AS 'CIUDAD ORIGEN',
				   ...
				   f.cod_tercer AS 'CEDULA',f.num_telef1 AS 'TELEFONO'	   
			...
			WHERE a.num_despac = b.num_despac AND
				  ...
				  a.num_despac NOT IN (SELECT j.num_despac FROM ".CENTRAL.".tab_factur_factur j 
				  WHERE j.num_despac = a.num_despac AND c.cod_tercer = j.cod_tercer) AND
				  a.fec_salida >= '".$GLOBALS["fecini"]."' AND 
				  a.fec_salida <= '".$GLOBALS["fecfin"]."' AND 
				 (a.cod_conult ='9999' OR a.fec_llegad IS NOT NULL) 
			GROUP BY 1";
	...
	{
		$query  = "SELECT COUNT(a.usr_creaci)  
				  FROM ".BASE_DATOS.".tab_despac_noveda a  
				  WHERE a.num_despac = ".$facturas[$i][0]." ";
		$consulta = new Consulta($query, $this -> conexion);
		$nove = $consulta -> ret_matriz();
		...
		$nov = (int)$contro[0][0] +(int)$nove[0][0]; 
	...
	for($i = 0; $i < sizeof($bases); $i++)
	{
		...
		if($x==1){
			...
			$query  = "SELECT a.num_despac AS 'DESPACHO',a.fec_salida AS 'FECHA DE SALIDA',  d.nom_ciudad AS 'CIUDAD ORIGEN',
					   e.nom_ciudad AS 'CIUDAD DESTINO',a.cod_manifi AS 'MANIFIESTO',c.abr_tercer AS EMPRESA,
					   a.fec_llegad AS 'FECHA DE LLEGADA' ,b.num_placax AS 'PLACA',f.abr_tercer AS 'CONDUCTOR',
					   f.cod_tercer AS 'CEDULA',if(a.ind_segfar='1','SI','NO') AS 'SEGUIMIENTO FARO',f.num_telef1 AS 'TELEFONO'				FROM ".$bases[$i][0].".tab_despac_despac a, ".$bases[$i][0].".tab_despac_vehige b,".$bases[$i][0].".tab_tercer_tercer c,
				...
				WHERE a.num_despac = b.num_despac AND
					  ...
					  f.cod_tercer = b.cod_conduc AND 
					  a.num_despac NOT IN (SELECT j.num_despac FROM ".CENTRAL.".tab_factur_factur j 
					  WHERE j.num_despac = a.num_despac AND ".$aux1." = j.cod_tercer) AND
					  a.fec_salida >= '".$GLOBALS["fecini"]."' AND 
					  a.fec_salida <= '".$GLOBALS["fecfin"]."' AND
			...
			{
				$query  = "SELECT COUNT(a.usr_creaci)  
					FROM ".$bases[$i][0].".tab_despac_noveda a  
					WHERE a.num_despac = ".$facturas[$z][0]." AND
						  (a.usr_creaci LIKE '%faro%' OR a.usr_creaci LIKE '%Admin-americ%')";
				$consulta = new Consulta($query, $this -> conexion);
				$nove = $consulta -> ret_matriz();
				
				$query  = "SELECT COUNT(a.usr_creaci)  
					FROM ".$bases[$i][0].".tab_despac_contro a  
					WHERE a.num_despac = ".$facturas[$z][0]." AND
						  (a.usr_creaci LIKE '%faro%' OR a.usr_creaci LIKE '%Admin-americ%')";
				$consulta = new Consulta($query, $this -> conexion);
				$contro = $consulta -> ret_matriz();	
