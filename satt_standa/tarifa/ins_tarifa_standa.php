    ...
    function formulario()
    {
        ...
        echo "<script language=\"JavaScript\" src=\"../".DIR_APLICA_CENTRAL."/js/tarifa.js\"></script>\n";
			  echo "<script language=\"JavaScript\" src=\"../".DIR_APLICA_CENTRAL."/js/min.js\"></script>\n";
				...
				$formulario = new Formulario("index.php\" enctype=\"multipart/form-data\"", "post", "INSERTAR TARIFA STANDA", "form_insert\" id=\"formularioID");
        ...
        $formulario -> lista( "Tipo de Cobro:","tip_tarifa\" id=\"tip_tarifaID\" onChange=\"form_insert.submit()",$tipos,1);
    			...
    			$consulta = new Consulta($query, $this -> conexion);
					...
					$formulario -> texto ("Valor Despacho","text","val_tarifa\" onchange=\"BlurNumeric(this);\" onkeypress=\"return EvalMoney( event, this,',' );\" id=\"val_tarifaID",1,9,11,"",number_format($can_minim[0][0]) );
    ...
    function insert()
	  {
	      ...
	      $insercion = new Consulta($query, $this -> conexion,"BR");
    		...
    		$consulta = new Consulta($query, $this -> conexion);
				...
				$max = $max[0][0]+1;
				$query =  "INSERT INTO ".BASE_DATOS.".tab_genera_tarifa(
	                      cod_tarifa, tip_tarifa, val_minimo,
	                      ind_estado, usr_creaci,fec_creaci,
												fec_inifac, fec_finfac)
	                 VALUES('".$max."', 'D', '".$this -> moneyToDouble($_POST['val_minim'])."',
	                         1,'".$datos_usuario['cod_usuari']."',NOW(),
													 '".$_POST['fec_inifac']."', '".$_POST['fec_finfac']."')";
	      $insercion = new Consulta($query, $this -> conexion,"R");
    		...
    		$consulta = new Consulta($query, $this -> conexion);
				...
				$consec = $consec[0][0]+1;
				$query =  "INSERT INTO ".BASE_DATOS.".tab_detall_tarifa(
	      ...
	      $insercion = new Consulta($query, $this -> conexion,"R");
		     ...
		     $mens = new mensajes();
		     $mens -> correcto("INSERTAR TARIFA STANDA",$mensaje);
				}
	  	}
	      ...
	      $insercion = new Consulta($query, $this -> conexion,"BR");
    		...
    		$consulta = new Consulta($query, $this -> conexion);
				...
				$query =  "INSERT INTO ".BASE_DATOS.".tab_genera_tarifa(
	                      ...
	                      ind_estado, usr_creaci,fec_creaci,
												fec_inifac, fec_finfac)
	                 VALUES('".$max."', 'N', '".$this -> moneyToDouble($_POST['val_minim'])."',
	                         1,'".$datos_usuario['cod_usuari']."',NOW(),
													 '".$_POST['fec_inifac']."', '".$_POST['fec_finfac']."')";
	      $insercion = new Consulta($query, $this -> conexion,"R");
				$query= "SELECT MAX(cod_consec)
...
$service = new TarStanda($this -> conexion, $this -> usuario_aplicacion, $this-> codigo);
?> 
