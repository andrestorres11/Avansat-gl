...
class DashBoard_Seguim_Table1 {
    ...
    function __construct() {
        ...
        echo '
           ...
           echo "<script>
            ...
            {
                $table='.tab_genera_noveda';
                $column='cod_noveda';
                $whereColum='c.nov_especi = 1';
            }
            else{
                $table='.tab_parame_novseg';
                $column='cod_ultnov';
                $whereColum='c.ind_novesp = 1 and c.cod_transp=b.cod_transp';
            }
            ...
            $mSql="select a.num_despac, a.cod_manifi, b.num_placax, d.nom_tercer, e.usr_creaci,b.cod_transp
                ...
                foreach($despachos as $value){
                    ...
                    //$mDespac =$this->getDataDespac($value['num_despac']);
           ...
           echo '
            <table id="tablaRegistros" class="table table-striped table-bordered table-sm" style="width: 90vw;font-size:10px;">
                ...
                foreach($sorted as $value){
					if(!in_array($uniques,$value['cod_manifi']) && !in_array($uniques,$value['num_despac']) && !in_array($uniques,$value['num_placax']) && !in_array($uniques,$value['nom_tercer']) && !in_array($uniques,$value['usr_creaci']))
					{
						$i++;
						echo "<tr>";
						echo "<td>".$i."</td>";
						echo "<td>".$value['cod_manifi']."</td>";
						echo "<td class='classCell bt ".$value['color']."'>".$value['link']."</td>";
						echo "<td>".$value['num_placax']."</td>";
						echo "<td>".$value['nom_tercer']."</td>";
						echo "<td>".$value['usr_creaci']."</td>";
						echo "</tr>";
						echo $html;
						array_push($uniques,$value['cod_manifi'],$value['num_despac'],$value['num_placax'],$value['nom_tercer'],$value['usr_creaci']);
					}
                }
            echo '</tbody>
            </table>';
    ...
    public function getDataDespac($num_despac){
        $mNovDespac = getNovedadesDespac( $this -> conexion, $num_despac, 1 ); # Novedades del Despacho -- Script /lib/general/function.inc
		...
		$mResult[sig_pcontr] = getNextPC( $this -> conexion, $num_despac );
		$mResult[pla_rutaxx] = getControDespac( $this -> conexion, $num_despac ); # Plan de Ruta del Despacho -- Script /lib/general/function.inc
        return $mResult;
    }
...
$_INFORM = new DashBoard_Seguim_Table1();
?>