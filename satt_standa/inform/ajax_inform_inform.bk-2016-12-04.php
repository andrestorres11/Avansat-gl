  ...
  *  \date modified: dia/mes/año
  ...
  private function getDataDetalleEal($datos){
   ...
   if($datos->tipo){
    ...
    $sql = "SELECT a.num_despac, a.cod_manifi, b.num_desext, c.nom_tipdes,
                   $adicional poseedor, g.nom_ciudad origen, h.nom_ciudad destino, e.num_placax,
  ...
  private function getDataSalDes($datos){
    ...
    $sql = "SELECT x.num_despac, x.cod_manifi, x.num_desext, x.nom_tipdes, x.poseedor, x.origen, x.destino,
                   ...
                   x.diferencia, x.fec_creaci, x.fec_citcar, x.ind_cumpli FROM
                   (SELECT a.num_despac, a.cod_manifi, b.num_desext, c.nom_tipdes,
                       ...
                       i.abr_tercer conductor, i.cod_tercer cedula, i.num_telmov celular, a.fec_salida,
                       a.fec_llegad, j.abr_tercer transporta,  TIMESTAMPDIFF(MINUTE, a.fec_creaci, CONCAT(a.fec_citcar, ' ', a.hor_citcar)) diferencia, 
                       a.fec_creaci,  CONCAT(a.fec_citcar, ' ', a.hor_citcar) fec_citcar,
                       IF(
                            (TIMESTAMPDIFF(MINUTE, a.fec_creaci, CONCAT(a.fec_citcar, ' ', a.hor_citcar)) >= 
                              (CASE a.cod_tipdes when 1 then ".$tipSer[0]['tie_carurb']."
                                                 when 2 then ".$tipSer[0]['tie_carnac']."
  ...
  private function getPoseedores(){
    $datos = (object) $_POST;
   $sql = "SELECT a.cod_poseed, a.nom_poseed FROM ".BASE_DATOS.".tab_despac_corona a 
                                       ...
                                       WHERE b.cod_activi = 1 AND c.cod_transp = '$datos->cod_transp' GROUP BY cod_poseed ";
