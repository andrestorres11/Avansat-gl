...
class inform {
  ...
  private function getDataEal($datos){
    ...
    $sql = "SELECT count(DISTINCT x.num_despac) cant_despac, x.nom_contro, x.fec_creaci, x.cod_contro, 
                    count(x.cod_contro) can_regist, sum(IF(x.cod_noveda IS NOT NULL, 1, 0)) can_cumpli, sum(IF(x.cod_noveda IS  NULL, 1, 0)) can_nocump
                  FROM
                     (SELECT a.num_despac, d.cod_noveda, 
                             date_format(a.fec_creaci,'%Y-%m-%d') fec_creaci, c.nom_contro, c.cod_contro
                          ...
                          INNER JOIN ".BASE_DATOS.".tab_despac_vehige e ON e.num_despac = a.num_despac
                           ...
                           WHERE c.ind_virtua = 0 $and 
                           AND e.cod_transp = '$datos->cod_transp'
                           AND b.ind_estado != 2
                           AND a.fec_creaci BETWEEN '$datos->fec_inicia 00:00:00' AND '$datos->fec_finali 23:59:59' ) x $group ";
  ...
  ?>
    ...
    <div id="acordeon2ID" class="col-md-12 accordion">
      <label><b>Eal Cumplidas en Ruta</b></label>
      <div id="contenido2">
              ...
              foreach ($data as $key => $value) {
                ...
                ?> 
                  <tr class="Style2DIV">
                    ...
                    <th class="cellInfo onlyCell" style="text-align:center"><?php if($despachos > 0){ ?><a onclick="getDetalleEal('<?= $value['fec_creaci'] ?> 00:00:00', '<?= $value['fec_creaci'] ?> 23:59:59', 0 )" style="cursor:pointer;color:green"><?= $despachos ?></a> <?php }else{ echo $despachos; } ?></th>
  ...
  private function getEal($datos, $data){// para mostrar el informe por EAL
        ...
        <div id="ch" class="Style2DIV">
          <table width="100%" cellspacing="0" cellpadding="0">
           <?php if($data){ ?>
            ...
            <tr class="Style2DIV">
              <td class="cellInfo onlyCell" style="text-align:center" id="despachos"><a onclick="getDetalleEal('<?=$datos->fec_inicia ?> 00:00:00', '<?= $datos->fec_finali ?> 23:59:59', 0)" style="cursor:pointer; color:green" ><?= self::$cTotalDespac ?></a></td>
  ...
  *  \date modified: dia/mes/año
            ...
            <?php
              foreach ($data as $key => $value) {
               ...
               <tr class="Style2DIV">
                  ...
                  <td class="cellInfo onlyCell" style="text-align:center"><a style="cursor:pointer; color:green" target="_BLANK"  href="index.php?cod_servic=3302&window=central&despac=<?=$value->num_despac?>&opcion=1"><?= $value->num_despac ?></a></td>
  ...
  private function getDataDetalleEal($datos){
   ...
   if($datos->tipo){
    ...
    $sql = "SELECT a.num_despac, a.cod_manifi, b.num_desext, c.nom_tipdes,
                   f.abr_tercer poseedor, g.nom_ciudad origen, h.nom_ciudad destino, e.num_placax,
                   i.abr_tercer conductor, i.cod_tercer cedula, i.num_telmov celular, a.fec_salida,
              ...
              LEFT  JOIN ".BASE_DATOS.".tab_despac_noveda m ON m.num_despac = a.num_despac AND l.cod_contro = m.cod_contro AND m.fec_creaci = ( SELECT MAX(x.fec_creaci) FROM ".BASE_DATOS.".tab_despac_noveda x WHERE x.num_despac = m.num_despac AND x.cod_contro = m.cod_contro )
              WHERE a.fec_creaci BETWEEN '$datos->fec_inicia' AND '$datos->fec_finali'
              AND l.ind_virtua = 0  AND d.cod_transp = '$datos->cod_transp' $and AND k.ind_estado != 2 GROUP BY a.num_despac";
    $consulta = new Consulta($sql, self::$cConexion);
  ...
  private function getDataInd($datos){
    ...
    $sql = "SELECT  SUM(
                        IF(
                            (TIMESTAMPDIFF(MINUTE, a.fec_salsis, CONCAT(a.fec_citcar, ' ', a.hor_citcar)) <= 
        ...
        <div id="contenido2">
          <div id="ch" class="Style2DIV">
            ...
            <tr class="Style2DIV">
              ...
              <th class="CellHead" style="text-align:center"> Cédula </th>
              <th class="CellHead" style="text-align:center"> Celular </th>
              <th class="CellHead" style="text-align:center"> Fecha de Salida del Despacho </th>
            ...
            <?php
              foreach ($data as $key => $value) {
               ...
               <tr class="Style2DIV">
                  ...
                  <td class="cellInfo onlyCell" style="text-align:center"><?= $value->cedula     ?></td>
                  <td class="cellInfo onlyCell" style="text-align:center"><?= $value->celular    ?></td>
                  <td class="cellInfo onlyCell" style="text-align:center"><?= $value->fec_salsis ?></td>
  ...
  private function getDataSalDes($datos){
    ...
    $sql = "SELECT x.num_despac, x.cod_manifi, x.num_desext, x.nom_tipdes, x.poseedor, x.origen, x.destino,
                   ...
                   x.diferencia, x.fec_salsis, x.fec_citcar, x.ind_cumpli FROM
                   (SELECT a.num_despac, a.cod_manifi, b.num_desext, c.nom_tipdes,
                       f.abr_tercer poseedor, g.nom_ciudad origen, h.nom_ciudad destino, e.num_placax,
                       i.abr_tercer conductor, i.cod_tercer cedula, i.num_telmov celular, a.fec_salida,
                       a.fec_llegad, j.abr_tercer transporta,  TIMESTAMPDIFF(MINUTE, a.fec_salsis, CONCAT(a.fec_citcar, ' ', a.hor_citcar)) diferencia, 
                       a.fec_salsis,  CONCAT(a.fec_citcar, ' ', a.hor_citcar) fec_citcar,
                       IF(
                            (TIMESTAMPDIFF(MINUTE, a.fec_salsis, CONCAT(a.fec_citcar, ' ', a.hor_citcar)) <= 
                              (CASE a.cod_tipdes when 1 then ".$tipSer[0]['tie_carurb']."
                                                 when 2 then ".$tipSer[0]['tie_carnac']."
