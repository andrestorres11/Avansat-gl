...
class InformDespacSeguim
{
  ...
  function GetDespacTiempoPropio( $num_despac ) {
    ...
    {
          $_NUEVO = array();
          $_NUEVO = $_DESPAC_TIEPRO;
          $tot_tiempo = $size_tiepro;
          $lab_alarma =  $tot_tiempo == 1 ? 'SE ENCONTRÓ ' : 'SE ENCONTRARON ';
          $lab_alarma .= $tot_tiempo. " ";
          $lab_alarma .= $tot_tiempo == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
			
          $tiempo_propio = explode(', ',$_TRANSP['can_tiepro']);
      
      
        if ($mData['tip_alarma'] == 'can_sinret')
        {
            $sin_retraso = explode(', ',$_TRANSP['can_sinret']);
            $cont = 0;
            /*------------------------------------------------*/
            for ( $k = 0; $k <= count($tiempo_propio)-1; $k++ )
            {
              for( $l = 0; $l <= count($sin_retraso)-1; $l++ )
              {
                if( $tiempo_propio[$k] == $sin_retraso[$l] )
                {
                  $_NUEVO1[$cont] = $sin_retraso[$l];
                  $cont++;
                }
              }
            }
            /*------------------------------------------------*/
            $lab_alarma =  count($_NUEVO1)  == 1 ? 'SE ENCONTRÓ ' : 'SE ENCONTRARON ';
            $lab_alarma .= count($_NUEVO1) . " ";
            $lab_alarma .= count($_NUEVO1)  == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= 'Y SIN RETRASO';
        }
        if ($mData['tip_alarma'] == 'can_30mins')
        {
           $can_30mins = explode(', ',$_TRANSP['can_30mins']);
            $cont = 0;
            /*------------------------------------------------*/
            for ( $k = 0; $k <= count($tiempo_propio)-1; $k++ )
            {
              for( $l = 0; $l <= count($can_30mins)-1; $l++ )
              {
                if( $tiempo_propio[$k] == $can_30mins[$l] )
                {
                  $_NUEVO1[$cont] = $can_30mins[$l];
                  $cont++;
                }
              }
            }
            ...
            $lab_alarma .= count($_NUEVO1). " ";
            $lab_alarma .= count($_NUEVO1) == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= count($_NUEVO1) == 1 ? 'Y RETRASADO ENTRE 1 Y 30 MINUTOS' : 'Y RETRASADOS ENTRE 1 Y 30 MINUTOS';
        }
        if ($mData['tip_alarma'] == 'can_60mins')
        {
            $can_60mins = explode(', ',$_TRANSP['can_60mins']);
            $cont = 0;
            /*------------------------------------------------*/
            for ( $k = 0; $k <= count($tiempo_propio)-1; $k++ )
            {
              for( $l = 0; $l <= count($can_60mins)-1; $l++ )
              {
                if( $tiempo_propio[$k] == $can_60mins[$l] )
                {
                  $_NUEVO1[$cont] = $can_60mins[$l];
                  $cont++;
                }
              }
            }
            ...
            $lab_alarma .= count($_NUEVO1). " ";
            $lab_alarma .= count($_NUEVO1) == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= count($_NUEVO1) == 1 ? 'Y RETRASADO ENTRE 31 Y 60 MINUTOS' : 'Y RETRASADOS ENTRE 31 Y 60 MINUTOS';
        }
        if ($mData['tip_alarma'] == 'can_90mins')
        {
          $can_90mins = explode(', ',$_TRANSP['can_90mins']);
            $cont = 0;
            /*------------------------------------------------*/
            for ( $k = 0; $k <= count($tiempo_propio)-1; $k++ )
            {
              for( $l = 0; $l <= count($can_90mins)-1; $l++ )
              {
                if( $tiempo_propio[$k] == $can_90mins[$l] )
                {
                  $_NUEVO1[$cont] = $can_90mins[$l];
                  $cont++;
                }
              }
            }
            ...
            $lab_alarma .= count($_NUEVO1). " ";
            $lab_alarma .= count($_NUEVO1) == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= count($_NUEVO1) == 1 ? 'Y RETRASADO ENTRE 61 Y 90 MINUTOS' : 'Y RETRASADOS ENTRE 61 Y 90 MINUTOS';
        }
        if ($mData['tip_alarma'] == 'can_120mins')
        {
            $can_120mins = explode(', ',$_TRANSP['can_120mins']);
            $cont = 0;
            /*------------------------------------------------*/
            for ( $k = 0; $k <= count($tiempo_propio)-1; $k++ )
            {
              for( $l = 0; $l <= count($can_120mins)-1; $l++ )
              {
                if( $tiempo_propio[$k] == $can_120mins[$l] )
                {
                  $_NUEVO1[$cont] = $can_120mins[$l];
                  $cont++;
                }
              }
            }
            ...
            $lab_alarma .= count($_NUEVO1). " ";
            $lab_alarma .= count($_NUEVO1) == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= count($_NUEVO1) == 1 ? 'Y RETRASADO EN MAS DE 91 MINUTOS' : 'Y RETRASADOS EN MAS DE 91 MINUTOS';
        }
        if ($mData['tip_alarma'] == 'can_alaEAL')
        {

            $lab_alarma =  $size_tipala == 1 ? 'SE ENCONTRÓ ' : 'SE ENCONTRARON ';
            $lab_alarma .= $size_tipala. " ";
            $lab_alarma .= $size_tipala == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= 'Y CON ALARMA EAL';
        }
        if ($mData['tip_alarma'] == 'can_fuepla')
        {
            ...
            $lab_alarma .= $size_tipala. " ";
            $lab_alarma .= $size_tipala == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= 'Y FUERA DE PLATAFORMA';
        }
        ...
        if ($mData['tip_alarma'] == 'can_especi')
        {
            ...
            $lab_alarma .= $size_tipala. " ";
            $lab_alarma .= $size_tipala == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= 'Y CON NOVEDAD ESPECIAL';
        }
        if ($mData['tip_alarma'] == 'can_penlle')
        {
            ...
            $lab_alarma .= $size_tipala. " ";
            $lab_alarma .= $size_tipala == 1 ? 'DESPACHO CON TIEMPO MODIFICADO ' : 'DESPACHOS CON TIEMPO MODIFICADO ';
            $lab_alarma .= $size_tipala == 1 ? 'Y PENDIENTE POR LLEGAR' : 'Y PENDIENTES POR LLEGAR';
        }
        $html .= '<div class="classTitle">' . $lab_alarma . '</div>';
        //--------------------------
        $rowspan = $mData['tip_alarma'] == 'can_despac' ? $size_tipala + 3 +$size_tiepro : $size_tipala + $size_penlle + $size_tiepro + 3;
        //--------------------------
        $html .= '<th nowrap class="classMenu" align="center" rowspan="' . $rowspan . '" onclick="Hide();">&nbsp;</th>';
        $html .= '<th nowrap class="classHead" align="center" width="15">#</th>';
        $html .= '<th nowrap class="classHead" align="center" width="75">DESPACHO</th>';
        $html .= '<th nowrap class="classHead" align="center" width="25">TIEMPO</th>';
        $html .= '<th nowrap class="classHead" align="center">A C. EMPRESA</th>';
        $html .= '<th nowrap class="classHead" align="center">NO. TRANSPORTE</th>';
        $html .= '<th nowrap class="classHead" align="center">NOVEDADES</th>';
        $html .= '<th nowrap class="classHead" align="center">ORIGEN</th>';
        $html .= '<th nowrap class="classHead" align="center">DESTINO</th>';
        $html .= '<th nowrap class="classHead" align="center">TRANSPORTADORA</th>';
        $html .= '<th nowrap class="classHead" align="center">PLACA</th>';
        $html .= '<th nowrap class="classHead" align="center">CONDUCTOR</th>';
        $html .= '<th nowrap class="classHead" align="center">CELULAR</th>';
        $html .= '<th nowrap class="classHead" align="center">UBICACI&Oacute;N</th>';
        $html .= '<th nowrap class="classHead" align="center">FECHA SALIDA</th>';
        $html .= '<th nowrap class="classHead" align="center">ULTIMA NOVEDAD</th>';
			$html .= '</tr>';
      $i = 1;
      $_GRAL = !$_NUEVO1 && !$sin_retraso && !$can_30mins && !$can_60mins && !$can_90mins && !$can_120mins ? $_NUEVO  : $_NUEVO1 ;

          if($_GRAL)
          {
          foreach ($_GRAL as $num_despac)
            {
                //----------------------------------------------------------
                $obj_destra = new DespacTransi($this->conexion, $mData);
                //----------------------------------------------------------
                ...
                $can_alaEAL = $_DESPAC['can_alaEAL'];
                  ...
                  $fec_salida = $_DESPAC['fec_salsis'];
                  
                  $get_ultnov = $this -> getLastNoveda( $num_despac );
                  ...
                  $line .= $this->set_link($num_despac, $dif_minuto);
                  $line .= '</th>';
                  $line .= '<th nowrap class="classCell" align="left">';
                ...
                $line .= '<td nowrap class="classCell" align="left">' . $fec_salida. '</td>';
                $line .= '<td nowrap class="classCell" align="left">' . $get_ultnov. '</td>';
                $line .= '</tr>';
                //-------------
                $html .= $line;
                //-------------
        ...
        $html2 .= '<th nowrap class="classHead" align="center">FECHA SALIDA</th>';
        $html2 .= '<th nowrap class="classHead" align="center">ULTIMA NOVEDAD</th>';
        $html2 .= '</tr>';
        ...
        { 
            ...
            $html3 .= '<th nowrap class="classHead" align="center">FECHA SALIDA</th>';
            $html3 .= '<th nowrap class="classHead" align="center">ULTIMA NOVEDAD</th>';
            $html3 .= '</tr>';
    ...
    //----------------------------------------------------------------------------------------------------------------------------------|
    //@method: getLastNoveda                                                                                                            |
    //@brief : Retorna la ultima novedad de un despacho                                                                                 |                                   
    //----------------------------------------------------------------------------------------------------------------------------------|
    function getLastNoveda( $num_despac )
    {
        $mSelect = "SELECT ln.* FROM 
					(
					  ...
					  UNION
   					  ...
   					  SELECT a.cod_noveda, a.fec_contro, b.nom_noveda 
   					    FROM ".BASE_DATOS.".tab_despac_contro a, 
   					         ".BASE_DATOS.".tab_genera_noveda b 
   					   WHERE a.cod_noveda = b.cod_noveda 
   					     AND a.num_despac = '".$num_despac."'
