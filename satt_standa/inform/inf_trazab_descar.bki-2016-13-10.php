			...
			$mHtml->OpenDiv("id:secID");
				...
				$mHtml->OpenDiv("id:formID; class:Style2DIV");
					...
					$mHtml->Table('tr');
						...
						$mHtml->Select2( $mTransp, array("name"=>"cod_transp", "id"=>"cod_transpID", "width"=>"100%", "class"=>"cellInfo1 multiSel", "end"=>true) );
					$mHtml->CloseTable('tr');
		...
		if ( $tipoInforme != NULL ) {
			...
			$mSql = "SELECT x.*
						FROM (
							SELECT a.num_despac, c.nom_destin,
									DATE(a.fec_despac) AS fec_despac,
									IF(c.fec_cumdes IS NULL, 0, 1) AS fec_cumdex,
									IF(c.fec_inides IS NULL, 0, 1) AS fec_inides,
									IF(c.fec_findes IS NULL, 0, 1) AS fec_findes,
									d.num_desext,
									e.nom_tipdes,
