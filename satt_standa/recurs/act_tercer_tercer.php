...
class Act_tercer_tercer
{
 ...
 function Datos()
 {
  ...
  $matriz = $consulta -> ret_matriz();
   ...
   $formulario -> texto("Nro. Documento:","text","tercer\" readonly onChange=\"form_tercero.submit()",1,10,10,"",$_REQUEST[tercer],"","",NULL,1);
   ...
   {
   $formulario -> lista ("Tipo Doc.:", "tipdoc", $tipdoc, 0,1);
   $formulario -> texto ("No. Documento:","text","nit\" disabled ",1,15,11,"","$_REQUEST[tercer]","","",NULL,1);
 ...
 function Actualizar()
 {
  ...
  if($_REQUEST[tipdoc] == 'N')
    $query = "UPDATE ".BASE_DATOS.".tab_tercer_tercer
               SET num_verifi = '$_REQUEST[dijver]',
                   ...
                   dir_urlweb = '$_REQUEST[web]',
                   obs_tercer = '$_REQUEST[obs]',
                   usr_modifi = '$_REQUEST[usuario]',
                   fec_modifi = NOW()
             WHERE cod_tercer = '$_REQUEST[nit]'";
  else
  {
  ...
  $activi = $_REQUEST[activi];

  //query que borra
  $query = "DELETE FROM ".BASE_DATOS.".tab_tercer_activi
             WHERE cod_tercer = '".$_REQUEST[nit]."' AND
  ...
  for($i = 0; $i < $_REQUEST[maxactivi]; $i++)
  {
   if($activi[$i] != Null)
   {
    $query = "INSERT INTO ".BASE_DATOS.".tab_tercer_activi
                   VALUES ('".$_REQUEST[nit]."',".$activi[$i].")";
  ...
  {
   $link_a = "<br><b><a href=\"index.php?&window=central&cod_servic=".$_REQUEST[cod_servic]." \"target=\"centralFrame\">Actualizar Otro Tercero</a></b>";

   $mensaje =  "El Tercero <b>".$_REQUEST[abr]."</b> Se Actualizo con Exito".$link_a;
   $mens = new mensajes();
