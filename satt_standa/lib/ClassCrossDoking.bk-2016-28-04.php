...
class ClassCrossDoking
{
	...
	private function generateReport2()
	{
		#<Titulares>
			$mTittle[0] = array("num_despac" => "N&uacute;mero Despacho",
								...
								"fec_entreg" => "Fecha Lugar Entrega",
								"time_entre" => "Tiempo Entrega Total (Calendario)"
							   );
			...
			$mHtml->SetBody('<table id="'.$mIdxTablex.'" width="100%" align="center" cellspacing="1" cellpadding="3" border="0">');
				...
				foreach ($mData as $row)
				{
					#<Tratamiento de datos>
						...
						$mFec1 = $row['fec_entreg'] != "" ? $row['fec_entreg'] : $row['fec_llegad'];
						$mFec2 = $row['fec_salpla'] != "" ? $row['fec_salpla'] : $row['fec_citcar'];

						$mFec1 = new DateTime($mFec1);
						$mFec2 = new DateTime($mFec2);

						$mDiff = date_diff($mFec1, $mFec2);
						$row['time_entre'] = $mDiff->format('%a Dias %H Horas %I Min');

						/*$mDiff = ceil((strtotime($mFec) - strtotime($mFec2)) / 60);
						$mDiff = self::formatDayHour( $mDiff );
	...
	private function getDataDespac( $mCodTipdes = null )
	{
		$mSql = "SELECT a.num_despac, a.cod_manifi, a.fec_llegad, 
						c.num_solici, c.num_pedido, a.fec_despac, 
						a.val_pesoxx, c.num_despac AS num_viajex, 
						l.nom_produc, m.nom_tipdes, 
						n.fec_creaci AS fec_entreg, 
			 ...
			 INNER JOIN ".BASE_DATOS.".tab_genera_tipdes m 
			  ...
			  LEFT JOIN ".BASE_DATOS.".tab_despac_noveda n 
					 ON a.num_despac = n.num_despac 
					AND n.cod_contro = '9999' 
					AND n.fec_creaci = ( SELECT MIN(x.fec_creaci) 
										   FROM ".BASE_DATOS.".tab_despac_noveda x 
										  WHERE x.num_despac = n.num_despac 
											AND x.cod_contro = '9999'  )
