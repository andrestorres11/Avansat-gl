...
class suspensiones {
  ...
  function SetSuspensiones($cod_tercer = null, $cod_usuari = null, $eje_noveda = null, $ajax = null)
  {
    ...
    $segSusp = [];
    foreach ($cReturn as $key => $value) {
     //si el cliente tiene mas de $diasPorSuspender + 5 dias suspende
      if ($value['fec_suspen'] <= date("Y-m-d")){
        $segSusp['suspendido'][] = $value;
      }else{
        $inicio = date("Y-m-d H:i:s");
        $fin = $value['fec_suspen'] . " 15:30:00";
        $startDate = DateTime::createFromFormat('Y-m-d', date("Y-m-d", strtotime($inicio . "+ 0 days")));
        $endDate = DateTime::createFromFormat('Y-m-d', date("Y-m-d", strtotime($fin . "+ 0 days")));
        $dif2 = date_diff($startDate, $endDate);
        $diff2 = $dif2->format('%a');
        $dias = $diff2;
        $dif_datexx = $dias;
        $tiempo = date("H:i:s", strtotime("00:00:00") + strtotime($fin) - strtotime($inicio));
        $dif = explode(":", $tiempo);
        $value['contador'] = $dif_datexx . ' dias ' . $dif[0] . ' hrs ' . $dif[1] . ' mins';
