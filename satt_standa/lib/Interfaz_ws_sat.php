<?php
 ...
 $tiposEnvEstadoInterfValue = array(
    ...
    "intind"  => array('name' => "intind", 'type' => 'xsd:string')
                          );
 ...
 function FuncEnvDespacValue($schema)
 {
  ...
  if($schema["usuari"] && $schema["passwo"])
   $valog = $interf_sat -> Inicializar_client($schema["transp"],$schema["usuari"],$schema["passwo"]);
  else
  {
   $valog = $interf_sat -> Inicializar_client_infoTT($schema["usuari"],$schema["passwo"]);
   $interf_sat -> transp = $schema["transp"];
  }
 ...
 $tiposEnvDetinatValue = array(
		"usuari"  => array('name' => "usuari", 'type' => 'xsd:string'),
        "passwo"  => array('name' => "passwo", 'type' => 'xsd:string'),
        "transp"  => array('name' => "transp", 'type' => 'xsd:string'),
		"despac"  => array('name' => "despac", 'type' => 'xsd:string'),
		"codest"  => array('name' => "codest", 'type' => 'xsd:string'),
		"nomdes"  => array('name' => "nomdes", 'type' => 'xsd:string'),
		"nrodes"  => array('name' => "nrodes", 'type' => 'xsd:string'),
		"pesdes"  => array('name' => "pesdes", 'type' => 'xsd:string'),
		"cdesti"  => array('name' => "cdesti", 'type' => 'xsd:string'),
		"dirdes"  => array('name' => "dirdes", 'type' => 'xsd:string'),
		"camdes"  => array('name' => "camdes", 'type' => 'xsd:string'),
		"narchi"  => array('name' => "narchi", 'type' => 'xsd:string')
                          );

 $server -> wsdl -> addComplexType("envDetinatValue",'complexType','struct','all','',$tiposEnvDetinatValue);

 $tiposEnvDetinatReturn = array(
        'Confirmacion' => array('name' => 'Confirmacion', 'type' => 'xsd:string')
                             );

 $server -> wsdl -> addComplexType('envDetinatReturn','complexType','struct','all','',$tiposEnvDetinatReturn);

 $server -> register("FuncEnvDetinatValue",
    array("envDetinatValue" => "tns:envDetinatValue"),
    array("return"           => "tns:envDetinatReturn"),
    'urn:InterfazAplicaSAT',
    'urn:envDetinatReturnwsdl#envDetinatReturn',
    'rpc',
    'encoded',
    'Funcion Que Inserta Destinatarios'
                                        );

 function FuncEnvDetinatValue($schema)
 {
  $interf_sat = new Interfaz_SAT_client();
  ...
  if($valog)
  {
   $resultado = $interf_sat -> envDespacDetinat($schema);
