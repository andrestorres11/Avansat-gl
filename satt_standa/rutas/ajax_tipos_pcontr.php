    ...
    foreach ($mMatriz as $key => $datos) {
      foreach ($datos as $campo => $valor) {
    		...
    		if($campo == "ind_estado"){
    			...
    			if($valor == 1){
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_tpcont'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></button>&nbsp;';
          		}else{
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_tpcont'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i></button>&nbsp;';
          		}
          			$html .='<button onclick="edit('.$datos['cod_tpcont'].')" value="'.$datos['cod_tpcont'].'" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></button>';
          		$data[$key][] = $html;	
    		}else{
    			$data[$key][] = $valor;	
    		}
   ...
   */
    function updEst(){
      try {
        ...
        //Geera query
        $mQuery = "UPDATE ".BASE_DATOS.".tab_tipos_pcontr
                      SET ind_estado = $estado,
                          usr_modifi = '".$_SESSION['datos_usuario']['cod_usuari']."',
                          fec_modifi = NOW()
                    WHERE cod_tpcont = ".$_REQUEST['cod_tpcont'];
