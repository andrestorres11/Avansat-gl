  ...
  */
  private function setRegistros() {
    $mSql = " SELECT  a.cod_viasxx,
                      ...
                      a.ind_estado
                FROM  ".BASE_DATOS.".tab_genera_viasxx a";
    ...
    foreach ($mMatriz as $key => $datos) {
      foreach ($datos as $campo => $valor) {
    		...
    		if($campo == "ind_estado"){
    			...
    			if($valor == 1){
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_viasxx'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i></button>&nbsp;';
          		}else{
          			$html ='<button onclick="updEst(this)" value="'.$datos['cod_viasxx'].'" data-estado="'.$datos['ind_estado'].'" class="btn btn-success"><i class="fa fa-check" aria-hidden="true"></i></button>&nbsp;';
          		}
          			$html .='<button onclick="edit('.$datos['cod_viasxx'].')" value="'.$datos['cod_viasxx'].'" class="btn btn-info"><i class="fa fa-edit" aria-hidden="true"></i></button>';
          		$data[$key][] = $html;	
    		}else{
    			$data[$key][] = $valor;	
    		}
