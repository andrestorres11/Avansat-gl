 ...
 ***********************************************************************************/
    ...
    private function inactivar(){
      $cod_extenc = $_POST['cod_extenc'];
        ...
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se insctiv贸 la extensi贸n exitosamente.";
           ...
           $mens = new mensajes();
           $mens -> correcto2("INACTIVAR EXTENSI脫N",$mensaje);
        }
    }
    ...
    private function inactivarOperacion(){
      $cod_operac = $_POST['cod_operac'];
        ...
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se inactiv贸 la operaci贸n exitosamente.";
           $mensaje .= "<br><br><input type='button' name='cerrar' id='closeID' value='cerrar' onclick='closed()' class='crmButton small save ui-button ui-widget ui-state-default ui-corner-all'/><br><br>";
           $mens = new mensajes();
           $mens -> correcto2("INACTIVAR OPERAC脫N",$mensaje);
        }
    }
      ...
      $cod_operac = $_POST['cod_operac'];
        ...
        $query = "UPDATE ".BASE_DATOS.".tab_callce_operac 
                        SET ind_estado = 1,
                            ...
                            WHERE cod_operac = '$cod_operac' ";
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se activ贸 la operaci贸n exitosamente.";
           $mensaje .= "<br><br><input type='button' name='cerrar' id='closeID' value='cerrar' onclick='closed()' class='crmButton small save ui-button ui-widget ui-state-default ui-corner-all'/><br><br>";
           $mens = new mensajes();
           $mens -> correcto2("INACTIVAR OPERAC脫N",$mensaje);
        }
    ...
    private function inactivarGrupo(){
      $cod_grupox = $_POST['cod_grupox'];
        ...
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se inactiv贸 el grupo exitosamente.";
           $mensaje .= "<br><br><input type='button' name='cerrar' id='closeID' value='cerrar' onclick='closed()' class='crmButton small save ui-button ui-widget ui-state-default ui-corner-all'/><br><br>";
           $mens = new mensajes();
      ...
      $cod_grupox = $_POST['cod_grupox'];
        ...
        $query = "UPDATE ".BASE_DATOS.".tab_callce_grupox 
                        SET ind_estado = 1,
                            ...
                            WHERE cod_grupox = '$cod_grupox' ";
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se activ贸 el grupo exitosamente.";
           $mensaje .= "<br><br><input type='button' name='cerrar' id='closeID' value='cerrar' onclick='closed()' class='crmButton small save ui-button ui-widget ui-state-default ui-corner-all'/><br><br>";
           $mens = new mensajes();
           $mens -> correcto2("INACTIVAR GRUPO",$mensaje);
        }
      ...
      if($info){
       if($datos->pestana == "generaID"){
        ...
        <div class="col-md-12 Style2DIV">
          <label style="color:black">No se encontr贸 informaci贸n para los parametros de busqueda especificados.</label>
        </div>
        <?php 
      }
    ...
    private function GetDetalle(){
      ...
      if($post->tipo == 'todas'){
        $detalle = "Detallado General de llamadas entrantes para el d铆a $post->fec_inicia al $post->fec_finali";
      }else if ($post->tipo == 'ANSWER'){
        $detalle = "Detallado de llamadas entrantes contestadas para el d铆a $post->fec_inicia al $post->fec_finali";
      }else{
        $detalle = "Detallado de llamadas entrantes no contestadas para el d铆a $post->fec_inicia al $post->fec_finali";
      }
      ?>
      <div class="col-md-12">
        <br>
        ...
        <br>
        <label>Se encontr贸 un total de <?= count($data) ?> registros</label>
        <div id="padre">
    ...
    private function LoadCallPlay(){
      ...
      if($mAudio["cod_respon"] != '1000') {
        $mObjetAudio = $mAudio["msg_respon"];
         ...
         # Crea el elemento de reproduccion del audio que se descarg贸 de S3 -----------------------------------------
