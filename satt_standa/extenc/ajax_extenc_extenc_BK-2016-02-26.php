 ...
 ***********************************************************************************/
  ...
  /*  private function activar(){
        ...
        if($insercion = new Consulta($query, self::$cConexion, "R")) {
           $mensaje = "Se activó la extensión exitosamente.";
           ...
           $mens = new mensajes();
           $mens -> correcto2("ACTIVAR EXTENSIÓN",$mensaje);
        }
    }*/
    ...
    private function getInfomr($fec_inicia, $fec_finali, $cod_operac, $num_celula, $pestana){
      $fec_inicia = $fec_inicia." 00:00:00";
      $fec_finali = $fec_finali." 23:59:59";
      if($num_celula){
        $num_celula = "AND num_telefo LIKE '%$num_celula%'";
      }
     

        $sql = "SELECT x.cantidad, x.estado,  x.fecha FROM ((SELECT COUNT(a.num_telefo) cantidad, 'ANSWER' AS estado, DATE_FORMAT(a.fec_creaci, '%Y-%m-%d')  fecha FROM ".BASE_DATOS.".tab_despac_callin a 
                             INNER JOIN ".BASE_DATOS.".tab_callce_extenc b ON b.num_extenc = a.cod_extenc 
                             WHERE a.nom_estado = 'ANSWER' 
                             AND b.cod_operac = '$cod_operac'
                             AND a.fec_creaci BETWEEN '$fec_inicia' AND '$fec_finali' $num_celula 
                             GROUP BY fecha )
                             UNION ALL(
                               SELECT COUNT(c.num_telefo) cantidad, 'NOANSWER' AS estado, DATE_FORMAT(c.fec_creaci, '%Y-%m-%d') fecha FROM ".BASE_DATOS.".tab_despac_callin c 
                                   LEFT JOIN ".BASE_DATOS.".tab_callce_extenc d ON d.num_extenc = c.cod_extenc 
                                   WHERE c.nom_estado = 'NOANSWER' 
                                   AND c.cod_extenc = '0'
                                   AND c.fec_creaci BETWEEN '$fec_inicia' AND '$fec_finali' $num_celula  AND c.num_telefo IN 
                                   (SELECT DISTINCT(e.num_telefo)  FROM ".BASE_DATOS.".tab_despac_callin e INNER JOIN ".BASE_DATOS.".tab_callce_extenc f ON f.num_extenc = e.cod_extenc  WHERE e.fec_creaci BETWEEN '$fec_inicia' AND '$fec_finali' AND e.nom_estado = 'ANSWER' AND f.cod_operac = '$cod_operac'  $num_celula)
                                   GROUP BY fecha)) x";
/*echo "<pre>";
echo $sql;
echo "</pre>";*/
    ...
    private function pintarGeneral($datos,$post){
      ...
      ?>
      <div class="col-md-12 Style2DIV">
        <table class="table hoverTable" width="100%" cellspacing="0" cellpadding="2" border="0">
          <tr>
          ...
          <tr>
            ...
            <!-- <th class="cellInfo onlyCell" style="text-align:center"></th>
            <th class="cellInfo onlyCell" style="text-align:center"></th> -->
          </tr>
          <tr>
          ...
          <?php
          $fec_inicia = $post->fec_inicia;

          foreach ($mdata as $key => $value) {
            $img = "";
            $img2 = "";
            $img3 = "<a><img src='../".DIR_APLICA_CENTRAL."/imagenes/ver.png' style='cursor:pointer' width='14px' height='14px' onclick='detalle($cod_operac,\"$fec_inicia\",\"$fec_inicia\", \"todas\")' </a>";
            if($mdata[$fec_inicia]['contestadas'] > 0){
              $img = "<a><img src='../".DIR_APLICA_CENTRAL."/imagenes/ver.png' style='cursor:pointer' width='14px' height='14px' onclick='detalle($cod_operac,\"$fec_inicia\",\"$fec_inicia\", \"ANSWER\")' </a>";
            }
            if($mdata[$fec_inicia]['nocontestadas'] > 0){
              $img2 = "<a><img src='../".DIR_APLICA_CENTRAL."/imagenes/ver.png' style='cursor:pointer' width='14px' height='14px' onclick='detalle($cod_operac,\"$fec_inicia\",\"$fec_inicia\", \"NOANSWER\")' </a>";
            }

            $pcontestadas2 = round(($mdata[$fec_inicia]['contestadas']*100)/($mdata[$fec_inicia]['contestadas']+$mdata[$fec_inicia]['nocontestadas']),2);
            $pnocontestadas2 = round(($mdata[$fec_inicia]['nocontestadas']*100)/($mdata[$fec_inicia]['contestadas']+$mdata[$fec_inicia]['nocontestadas']),2);
            ?>
            <tr>
            <th class="cellInfo onlyCell" style="text-align:center"><?= $fec_inicia ?></th>
            <th class="cellInfo onlyCell" style="text-align:center"><?= ($mdata[$fec_inicia]['contestadas']+$mdata[$fec_inicia]['nocontestadas']).$img3 ?></th>
            <th class="cellInfo onlyCell" style="text-align:center"><?= $mdata[$fec_inicia]['contestadas'] . $img ?></th>
            <th class="cellInfo onlyCell" style="text-align:center"><?= $pcontestadas2 ?> %</th>
            <th class="cellInfo onlyCell" style="text-align:center"><?= $mdata[$fec_inicia]['nocontestadas'] . $img2 ?></th>
    ...
    private function GetDetalle(){
     ...
     /* print_r($post);
      ...
      echo "</pre>";*/
      if($post->tipo == 'todas'){
        $detalle = "Detallado General de llamadas entrantes para el día ". date ( 'Y-m-d' , strtotime($post->fec_inicia) );
      }else if ($post->tipo == 'ANSWER'){
        $detalle = "Detallado de llamadas entrantes contestadas para el día ". date ( 'Y-m-d' , strtotime($post->fec_inicia) );
      }else{
        $detalle = "Detallado de llamadas entrantes no contestadas para el día ". date ( 'Y-m-d' , strtotime($post->fec_inicia) );
      }
      ?>
      <div class="col-md-12">
        <br>
        <a onclick="getExcelLlamadas()" style="cursor:pointer"><img border="0" src="../<?= DIR_APLICA_CENTRAL ?>/imagenes/excel.jpg"></a>
        <br>
    ...
    private function getInfromacionDetallada($post){
      $and = "";
      if($post->fec_inicia == $post->fec_finali){
        $post->fec_inicia =  $post->fec_finali." 00:00:00"; ;
        $post->fec_finali =  $post->fec_finali." 23:59:59"; ;
      }
      if($post->tipo != 'todas'){
        $and = "WHERE x.nom_estado = '$post->tipo'";
      }
        $sql = "SELECT x.cod_consec, x.num_telefo, x.tie_duraci, x.idx_llamad, 
                       x.nom_estado, x.rut_audiox, x.cod_extenc, x.idx_servic, 
                      x.fec_creaci, x.estado  FROM ((SELECT a.cod_consec, a.num_telefo, a.tie_duraci, a.idx_llamad, 
                                                           a.nom_estado, a.rut_audiox, a.cod_extenc, a.idx_servic, 
                                                           a.fec_creaci, 'ANSWER' AS estado FROM ".BASE_DATOS.".tab_despac_callin a 
                             INNER JOIN ".BASE_DATOS.".tab_callce_extenc b ON b.num_extenc = a.cod_extenc 
                             WHERE a.nom_estado = 'ANSWER' 
                             AND b.cod_operac = '$post->cod_operac'
                             AND a.fec_creaci BETWEEN '$post->fec_inicia' AND '$post->fec_finali' 
                            )
                             UNION ALL(
                               SELECT  c.cod_consec, c.num_telefo, c.tie_duraci, c.idx_llamad, 
                                       c.nom_estado, c.rut_audiox, c.cod_extenc, c.idx_servic, 
                                       c.fec_creaci, 'NOANSWER' AS estado FROM ".BASE_DATOS.".tab_despac_callin c 
                                   LEFT JOIN ".BASE_DATOS.".tab_callce_extenc d ON d.num_extenc = c.cod_extenc 
                                   WHERE c.nom_estado = 'NOANSWER' 
                                   AND c.cod_extenc = '0'
                                   AND c.fec_creaci BETWEEN '$post->fec_inicia' AND '$post->fec_finali'   AND c.num_telefo IN 
                                   (SELECT DISTINCT(e.num_telefo)  FROM ".BASE_DATOS.".tab_despac_callin e INNER JOIN ".BASE_DATOS.".tab_callce_extenc f ON f.num_extenc = e.cod_extenc  WHERE e.fec_creaci BETWEEN '$post->fecha_inicial' AND '$post->fecha_final'  AND f.cod_operac = '$post->cod_operac'  $num_celula)
                                  )) x $and ";
