...
class cronAvansatTmsReportesUbicacion
{
	...
	*/
	private function getReportesPendientes()
	{
		...
		{ 
			$mQuery = "
							(
								SELECT
										...
										a.cod_contro, a.fec_noveda, a.des_noveda, a.tiem_duraci, a.cod_sitiox, c.nom_sitiox, a.cod_noveda, 
							 ...
							 INNER JOIN ".BASE_DATOS.".tab_interf_parame ee  ON cc.cod_transp = ee.cod_transp 
							 												...
							 												-- AND ee.cod_operad = 53 /*Debe tener interfaz Integrador*/
							...
							(
								SELECT
										...
										a.num_despac, aa.cod_manifi, bb.num_placax, 
										a.cod_contro, a.fec_contro AS fec_noveda, a.obs_contro AS des_noveda, a.tiem_duraci, a.cod_sitiox, c.nom_sitiox, a.cod_noveda,
			...
			//echo "<pre>"; print_r( self::$cPendientes );  echo "</pre>"; die();
	...
	private function envioReportes()
	{
		...
		try 
		{
			...
			{ 
				...
				{
					$mDesNoveda  =  ' ^{"latitud":"'.$mReporte['val_latitu'].'", "longitud":"'.$mReporte['val_logint'].'"}';
				}
				else
				{
				...
				$mParams =  [ 
								...
								'cod_transp' => $mReporte['nom_operad'],
								'num_manifi' => $mReporte['cod_manifi'],
								'num_placax' => $mReporte['num_placax'],
								'cod_noveda' => 9183, 
								'cod_contro' => $mReporte['cod_contro'], 
				...
				echo "<pre style='display:xx';> <hr>"; print_r( $mParams); echo "</pre>";  //die();
				...
				if( "1000" != $mCodResp[1] ){ 
					...
					echo "<pre>Error: ".$mReporte['num_despac']." ->  ".$mReporte['cod_manifi']."- ".$mReporte['url_webser']." -> "; print_r( var_dump($mMsgResp[1]) ); echo "</pre>"; 
					...
					if(in_array($mMsgResp[1], ["No se encuentra numero de despacho.", "Puesto de control no existente."])  ){
						self::$cReportes[] = $mReporte['num_despac'];
					}
				}
				else{
					self::$cReportes[] = $mReporte['num_despac'];
	...
	*/
	private function actualizarEnviados( $NumReport = NULL )
	{
		try 
		{
			$mUpdate = "UPDATE ".BASE_DATOS.".t_report_noveda SET ind_enviad = 1 WHERE num_despac IN ( ".join(', ',self::$cReportes).") ";
