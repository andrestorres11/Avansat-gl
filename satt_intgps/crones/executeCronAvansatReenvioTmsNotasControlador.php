...
class cronAvansatTmsReportesUbicacion
{
	...
	*/
	private function getReportesPendientes()
	{
		...
		{ 
			...
			$fecha_ayerxx = date('Y-m-d', strtotime('-1 day'));
			...
			$mQuery = "	(
							SELECT
									...
									a.cod_contro, g.nom_contro, a.fec_contro AS fec_noveda, a.obs_contro AS des_noveda, a.tiem_duraci, a.cod_sitiox, c.nom_sitiox, a.cod_noveda, f.nom_noveda, h.ind_manale as 'ind_alarma', h.ind_soltie as 'ind_tiempo', h.ind_novesp as 'nov_especi', h.ind_manale as 'ind_manala', 
						...
						LEFT JOIN ".BASE_DATOS.".tab_genera_contro g ON a.cod_contro = g.cod_contro
						LEFT JOIN ".BASE_DATOS.".tab_parame_novseg h ON a.cod_noveda = h.cod_noveda AND h.cod_transp = cc.cod_transp AND h.ind_status = 1
						...
						ORDER BY cod_server
							...
							SELECT
							'nc' AS metodo, cc.num_consec, cc.cod_transp, cc.cod_server, dd.nom_server, dd.url_webser, ee.nom_operad, ee.nom_usuari, ee.clv_usuari,
							a.num_despac, 1 as 'cod_connov', aa.cod_manifi, bb.num_placax, 
							a.cod_contro, g.nom_contro, a.fec_noveda AS fec_noveda, a.des_noveda AS des_noveda, a.tiem_duraci, a.cod_sitiox, c.nom_sitiox, a.cod_noveda, f.nom_noveda,  h.ind_manale as 'ind_alarma', h.ind_soltie as 'ind_tiempo', h.ind_novesp as 'nov_especi', h.ind_manale as 'ind_manala', 
		...
		{
			echo "<pre>Exception: "; print_r( $e -> getMessage()); echo "</pre>";  
		}
	}
	...
	private function envioReportes()
	{
		...
		try 
		{
			...
			{ 
				...
				echo "Despacho: ".$mReporte['num_despac']." - ".$mReporte['fec_creaci'];		
				echo "<pre>";
				print_r( $mParams);
				echo "</pre>";
				...
				if($mReporte['metodo']=='pc'){
					$mResponse = $mSoap -> __soapCall("setNovedadPC", $mParams);
				}else{
				...
				echo "Total Respuesta:</br>";
				echo "Servidor Consumo: ".$mReporte['url_webser']."<br>";
				echo "<pre>";
				print_r( $mResult );
				echo "</pre>";
				...
				if( "1000" != $mCodResp[1] ){ 
					echo "STATUS: ERROR ".$mMsgResp."</br>";
					echo "Servidor: ".$mReporte['url_webser']."</br>";
				}else{
					echo "STATUS: SUCCESS ".$mMsgResp."</br>";
					self::actualizaEstado( $mReporte );
				}
	...
	*  \return: none
	*/
	private function actualizaEstado( $mReporte = NULL )
	{
		try 
		{
